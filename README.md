...
> Twenty chapters covering the build, installing on a Raspberry Pi 4, the
...
You do not have to build anything. A compiled Pi 4 monitor package and its
boot-volume skeleton are both in this repository. The current package has
passed build and format checks; its exact image has not had a new hardware
acceptance run:

```text
Boards/RaspberryPi4/sdcard/
  kernel8.img            tracked Pi 4 monitor image
  armstub8.bin           required EL3 entry stub
  SHA256SUMS             check the package with one command
  config.txt             firmware settings, including armstub selection
  SETTINGS.TXT.example   optional; rename to SETTINGS.TXT
  README.md              the full card layout and first steps
```

**The images.** The card package includes both the Pi 4 monitor and the custom
EL3 entry stub. The stub is required: Anvil refuses to run below EL3. Check the
package hashes before copying it:
...
On **partition 1**, put the three Raspberry Pi firmware files (`start4.elf`,
`fixup4.dat`, `bcm2711-rpi-4-b.dtb`, which this repository does not
redistribute), `CONFIG.TXT`, `ARMSTUB8.BIN`, and `KERNEL8.IMG`. There is
**no `bootcode.bin` on a Pi 4** — it
...
hardware display failure. See the [DDC fixture issue](https://forum.ajtaji.com/topic/942/the-hdmi-ddc-gate-reads-the-edid-but-loses-its-preferred-timing-in-the-combined-fixture).
...
These are the compiled release candidates staged by the current build pass.
The hashes identify files, not hardware acceptance; see the target table above
for tested behavior and limits.

| Board package | Artifact | Build | Bytes | SHA-256 |
|---|---|---:|---:|---|
| `Boards/RaspberryPi3/sdcard/` | `kernel8.img` | 36 | 1,522,428 | `aeda023f58000d8415fbe215e53a90a489461d544d4c64507ff9e923a9600eab` |
| `Boards/RaspberryPi3/sdcard/` | `armstub8.bin` | — | 512 | `2a3bcc7e30c3ea80ecfcf168583272a24fa7907ab05678e5e88cdb84d8449909` |
| `Boards/RaspberryPi4/sdcard/` | `kernel8.img` | 212 | 4,200,128 | `d6d40968e176aba4d69501119d5a84c0f6d221763d7fbc3c4a285721c253294f` |
...
ROCK Pi U-Boot `Image` remain legacy experimental artifacts. The Rock Pi
...
python3 tools/build.py pi3
python3 tools/build.py pi4
python3 tools/build.py unoq
python3 tools/build.py rockpi4c
```
...
Target entries are
[`RaspberryPi3/Board/board.pi3`](RaspberryPi3/Board/board.pi3),
[`RaspberryPi4/Board/board.pi4`](RaspberryPi4/Board/board.pi4) and
[`ArduinoQ/Board/board.unoq`](ArduinoQ/Board/board.unoq), plus
[`RockPi4C/Board/board.rockpi4c`](RockPi4C/Board/board.rockpi4c). The normal
`pi3` target builds both its monitor and firmware stub; loader and updater
targets are explicitly diagnostic/recovery artifacts.

### Required EL3 firmware stub

The Pi 4 stub has a separate, explicit build using a compiler with
`--armstub` support:
...
This produces `build/pi4/armstub8.bin`. It is selected by the released Pi 4
card configuration and required because stock-stub EL2 is refused. It is not
part of `all`. Build 98 and the checked stub passed a witnessed
...
physical cold-power-cycle run, including cold leases, touch and Wi-Fi. Exact
historical artifacts and hardware boundaries are recorded in
