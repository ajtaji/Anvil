...
| `ARMSTUB8.BIN` | Optional, and read the warning below first. Built separately. |
...
`kernel8.img` here is **Anvil build 165** for the Raspberry Pi 4:

```text
3,019,188 bytes
sha256 0aef8f716baca0b6ba4d530711a82d232b0f47ba06d973f002546960ca1393e4
```
...
build is 166 or later and differs there. The hash above identifies *this* file.
...
> **Nothing loads the file for you after a reset.** Bringing the medium up is
> slow and noisy, so it is not done unasked. Type `settings load`.
...
`ARMSTUB8.BIN` is optional. It makes the firmware enter Anvil at EL3 — the
highest privilege level on this processor — instead of the EL2 the firmware
would otherwise drop it to. It is 512 bytes, a source translation of Raspberry
Pi's own stub pinned to a named upstream commit, and it keeps its upstream
BSD-3-Clause notice. Build it with `python3 tools/build.py armstub`; it is not
part of `all` and is not installed automatically.
...
> restoring files from a host. The EL3-to-EL1 handoff is also still incomplete.
> Do not change boot configuration unless you can physically get at the board
> and its medium.

The ordering is the whole safety argument, and chapter 3 of the guide walks it
step by step. In short: prove the image boots at EL2 with no stub involved
first; then write `ARMSTUB8.BIN` and **do not reset** — a stub nothing points
at changes nothing; then add the single line `armstub=armstub8.bin` to
`CONFIG.TXT`, change nothing else, and reset. The revert is deleting that one
line.
