...
; The 12x20 opaque glyph cell is composed once from TrueType coverage;
; ordinary console rendering submits a hardware copy into its row buffer.
...
#ROCK_RGA_MAX_ROW_BYTES = 2560 * #ROCK_RGA_GLYPH_H * 4
...
Global Dim rock_rga_dst_pages.a[272]
...
Procedure.i RockRgaMapPages(table.i,buffer.i,bytes.i)
  ...
  If pages<1 Or pages>64 : ProcedureReturn -1 : EndIf
...
Procedure.i RockRgaInit(normalFg.i,promptFg.i,bg.i,rowBuffer.i,pitch.i,width.i)
  ...
  If rowBuffer=0 Or pitch<width*4 Or pitch*#ROCK_RGA_GLYPH_H>#ROCK_RGA_MAX_ROW_BYTES Or width<320 Or width>2560
  ...
  If source<0 Or prompt<0 Or rowBuffer<0 Or rock_rga_src_table<0 Or rock_rga_alt_table<0 Or rock_rga_dst_table<0 Or rock_rga_command_addr<0
    rock_rga_error=5 : ProcedureReturn 0
  EndIf
  If source>$FFFFFFFF Or prompt>$FFFFFFFF Or rowBuffer>$FFFFFFFF Or rock_rga_command_addr>$FFFFFFFF
  ...
  rock_rga_dst_offset=RockRgaMapPages(rock_rga_dst_table,rowBuffer,pitch*#ROCK_RGA_GLYPH_H)
