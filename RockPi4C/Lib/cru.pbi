...
Procedure.i RockCruVpllSet(pixelHz.i)
  Protected con2.i
  Protected rate.i
  If rock_uart_ready<>0 : RockUartLine("VPLL00 MODE PLAN") : EndIf
  If RockModeVpllPlan(pixelHz)=0 : rock_cru_error=66 : ProcedureReturn 0 : EndIf
  ; Follow Linux rockchip_rk3399_pll_set_params exactly: switch the mux to
  ; xin24m slow mode, update the live dividers/fraction/DSM state, wait for
  ; lock, then select the PLL output. The rate-change path does not power the
  ; PLL down; powerdown is a separate clock-disable operation in that driver.
  ; CON2's 24-bit fraction is an ordinary RMW field.
  If rock_uart_ready<>0 : RockUartLine("VPLL01 SELECT SLOW") : EndIf
  RockCruField($CC,$0300,$0000)
  If rock_uart_ready<>0 : RockUartLine("VPLL02 WRITE DIVIDERS") : EndIf
  ...
  If rock_uart_ready<>0 : RockUartLine("VPLL03 WAIT LOCK") : EndIf
  If RockCruWait($C8,$80000000,$80000000)=0
    rock_cru_error=63 : ProcedureReturn 0
  EndIf
  If rock_uart_ready<>0 : RockUartLine("VPLL04 SELECT NORMAL") : EndIf
  RockCruField($CC,$0300,$0100)
  If rock_uart_ready<>0 : RockUartLine("VPLL05 VERIFY RATE") : EndIf
  ...
  EndIf
  If rock_uart_ready<>0 : RockUartLine("VPLL06 READY") : EndIf
  ProcedureReturn 1
EndProcedure
...
; dclk_vop0 is CRU CLKSEL_CON49 (VOPB); dclk_vop1 is CON50 (VOPL).
; Both use the dedicated VPLL direct output and require exact readback.
Procedure.i RockCruVpllModeForVop(vopId.i)
  Protected offset.i
  ...
  If vopId=0
    offset=#ROCK_CRU_CLKSEL+$C4
    errorCode=86
  ElseIf vopId=1
    offset=#ROCK_CRU_CLKSEL+$C8
    errorCode=73
  Else
    rock_cru_error=66 : ProcedureReturn 0
  EndIf
  RockCruGate(10,12+vopId,0)
  If RockCruVpllSet(rock_mode_pixel_hz)=0 : ProcedureReturn 0 : EndIf
  RockCruField(offset,$0BFF,$0000)
  RockCruGate(10,12+vopId,1)
  If RockCruExpect(offset,$0BFF,$0000,errorCode)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKGATE+10*4,1 << (12+vopId),0,errorCode+1)=0 : ProcedureReturn 0 : EndIf
  ProcedureReturn 1
EndProcedure

Procedure.i RockCruVpllMode()
  ProcedureReturn RockCruVpllModeForVop(1)
EndProcedure

; HDMI uses VOPB on the Rock Pi 4C. This path owns only its clock, reset and
; power contract, leaving TCPHY/Cadence and MiniDP pin state untouched.
Procedure.i RockCruHdmiPrepare()
  Protected vioDiv.i
  Protected hdcpDiv.i
  Protected hclkDiv.i
  Protected pclkDiv.i
  Protected vopDiv.i
  Protected vopHclkDiv.i
  Protected clock42.i
  Protected clock43.i
  Protected clock47.i
  Protected gpll.i
  rock_cru_error=0
  RockCruReset(274,1) ; SRST_A_VOP0
  RockCruReset(278,1) ; SRST_H_VOP0
  RockCruReset(280,1) ; SRST_D_VOP0
  RockCruReset(247,1) ; SRST_P_HDMI_CTRL
  RockCruReset(260,1) ; SRST_HDMI_CTRL
  gpll=RockCruPllRate($80,1,47)
  rock_cru_gpll_rate=gpll
  ; The live direct-SD EL3 handoff measured GPLL at 400 MHz. This is also
  ; the RK3399 ceiling for ACLK_VIO/HDCP/VOPB, so the source-derived tree is
  ; direct GPLL/1 for those roots, then /2 HCLK and /4 PCLK. Keep the other
  ; two observed firmware profiles; do not infer support for arbitrary rates.
  If gpll<>400000000 And gpll<>594000000 And gpll<>800000000 : rock_cru_error=50 : ProcedureReturn 0 : EndIf
  vioDiv=RockCruCeilingDivider(gpll,400000000)
  hdcpDiv=RockCruCeilingDivider(gpll,400000000)
  vopDiv=RockCruCeilingDivider(gpll,400000000)
  hclkDiv=RockCruCeilingDivider(gpll/hdcpDiv,200000000)
  pclkDiv=RockCruCeilingDivider(gpll/hdcpDiv,100000000)
  vopHclkDiv=RockCruCeilingDivider(gpll/vopDiv,200000000)
  If vioDiv=0 Or hdcpDiv=0 Or vopDiv=0 Or hclkDiv=0 Or pclkDiv=0 Or vopHclkDiv=0
    rock_cru_error=51 : ProcedureReturn 0
  EndIf
  clock42=$0040 | (vioDiv-1) | $4000 | ((hdcpDiv-1) << 8)
  clock43=(RockCruCeilingDivider(gpll/vioDiv,100000000)-1) | ((hclkDiv-1) << 5) | ((pclkDiv-1) << 10)
  ; VOP0 ACLK mux order is dmyVPLL/CPLL/GPLL/NPLL; choose GPLL (2).
  clock47=$0080 | (vopDiv-1) | ((vopHclkDiv-1) << 8)
  RockCruField(#ROCK_CRU_CLKSEL+$A8,$DFDF,clock42)
  RockCruField(#ROCK_CRU_CLKSEL+$AC,$7FFF,clock43)
  RockCruField(#ROCK_CRU_CLKSEL+$BC,$1FDF,clock47)
  RockCruGate(11,0,1) : RockCruGate(11,1,1) : RockCruGate(11,3,1) : RockCruGate(11,10,1)
  RockCruGate(28,0,1) : RockCruGate(28,1,1) : RockCruGate(28,2,1) : RockCruGate(28,3,1)
  RockCruGate(29,0,1) : RockCruGate(29,3,1) : RockCruGate(29,4,1) : RockCruGate(29,5,1)
  RockCruGate(29,6,1) : RockCruGate(29,12,1)
  RockCruGate(10,8,1) : RockCruGate(10,9,1) : RockCruGate(10,12,0)
  RockCruGate(11,6,1) ; SCLK_HDMI_SFR from xin24m
  If RockCruExpect(#ROCK_CRU_CLKSEL+$A8,$DFDF,clock42,87)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKSEL+$AC,$7FFF,clock43,88)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKSEL+$BC,$1FDF,clock47,89)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKGATE+10*4,$1300,$1000,98)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKGATE+11*4,$044B,0,99)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKGATE+28*4,$000F,0,100)=0 : ProcedureReturn 0 : EndIf
  If RockCruExpect(#ROCK_CRU_CLKGATE+29*4,$1079,0,101)=0 : ProcedureReturn 0 : EndIf
  ; HDCP and VO are powered parents; VOPB is an idle-only child (index 7).
  If RockPmuPowerOn(14,90)=0 : ProcedureReturn 0 : EndIf
  If RockPmuIdleRelease(17,91)=0 : ProcedureReturn 0 : EndIf
  If RockPmuPowerOn(24,92)=0 : ProcedureReturn 0 : EndIf
  If RockPmuIdleRelease(11,93)=0 : ProcedureReturn 0 : EndIf
  If RockPmuPowerOn(20,94)=0 : ProcedureReturn 0 : EndIf
  If RockPmuIdleRelease(7,95)=0 : ProcedureReturn 0 : EndIf
  RockTimerWaitUs(1)
  If RockCruReset(247,0)=0 Or RockCruReset(260,0)=0
    rock_cru_error=97 : ProcedureReturn 0
  EndIf
  ProcedureReturn 1
EndProcedure
...
Procedure.i RockCruVopBRelease()
  ; RK3399 VOPB resets (A/H/D VOP0), kept distinct from VOPL's IDs.
  If RockCruReset(274,0)=0 : ProcedureReturn 0 : EndIf
  If RockCruReset(278,0)=0 : ProcedureReturn 0 : EndIf
  If RockCruReset(280,0)=0 : ProcedureReturn 0 : EndIf
  ProcedureReturn 1
EndProcedure
