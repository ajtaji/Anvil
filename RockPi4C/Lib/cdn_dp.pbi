...
Procedure.i RockCdnReadEdid()
  ...
  Protected block.i
  Protected sharedBlocks.i
  Protected sharedTruncated.i
  ...
  EndIf
  ; Feed the same bytes to Anvil's platform-neutral EDID parser. The Cadence
  ; firmware mailbox remains this board's block transport; native AUX here is
  ; used for DPCD, not for the EDID I2C-over-AUX transaction. Keep the RockPi
  ; capability collector/mode policy intact while exposing common EDID data.
  sharedBlocks=extensionCount+1
  sharedTruncated=0
  If sharedBlocks>#ANVIL_EDID_MAX_BLOCKS
