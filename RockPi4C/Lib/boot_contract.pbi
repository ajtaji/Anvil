; Direct SD Anvil entry contract for the original ROCK Pi 4C v1.2.
; Anvil's integrated entry reaches the runtime at EL3 with x0=FDT at
; 0x001C0000, x1..x3 zero,
; aligned SP=0x05000000, DAIF masked, and MMU/caches disabled. It must have
; trained DDR for the image/BSS/stack windows, configure UART2 at 1.5 Mbaud,
; and enable the architectural counter at exactly 24 MHz. Build 96 proved
; this handoff on the original ROCK Pi 4C v1.2.
...
ProcedureNaked RockCaptureSctlrEl3()
  ASM
    adrp x9, global_rock_sctlr_el3
    add x9, x9, #:lo12:global_rock_sctlr_el3
    mrs x10, sctlr_el3
    str x10, [x9]
    ret
  ENDASM
...
Procedure.i RockValidateDtb()
  ...
  ; The current loader embeds this exact DTB inside the one Anvil component.
  ; Its fixed address and the bounded end below are the ownership proof; an
  ; overlap with Anvil is required here, rather than an external-FDT failure.
  If rock_dtb_end > #ROCK_ANVIL_LIMIT
...
Procedure.i RockValidateEntry()
  ...
  EndIf
  ; The direct-SD loader starts only the boot CPU. Refuse a surprise secondary rather than
