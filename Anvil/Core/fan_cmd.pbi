...
Procedure FanBoot()
  ...
  ; ---- THE PIN. The settings store first, and the BOARD's own default
  ; behind it.
  ;
  ; HwPwmDefaultPin() - "the pin a board KNOWS its fan is on, or -1" - was
  ; in the HAL contract from the start and nothing called it. The gap only
  ; shows on a boot with NO SETTINGS MEDIUM: the Pi 4 keeps SETTINGS.TXT on
  ; the USB stick, so a monitor run from RAM, or a boot with the stick
  ; pulled or not enumerating, reached the prompt with no fan pin and never
  ; turned the fan. `fan pin 18` became the first thing to type after every
  ; RAM-run, and the boot where it is easiest to forget is the one where
  ; the storage failed - which is also the boot that then sits at a prompt
  ; being investigated for a long time.
  ;
  ; A stored pin still wins outright, so a board whose wiring has been
  ; stated is never overridden. A board that answers -1 keeps the old
  ; behaviour and says nothing.
  Define fromBoard.i
  fromBoard = 0
  ...
  If FanStartOn(pin, gFanHz) = 0
    ...
    If fromBoard <> 0
      PrintN("   That was this board's default fan pin, not a stored setting, so")
      PrintN("   nothing is misconfigured - the board simply has no fan control")
      PrintN("   this boot. Type fan to see the pins.")
    Else
  ...
  ; SAY WHERE THE PIN CAME FROM. A boot log has to tell a pin somebody
  ; chose from a pin the image assumed, because the two want opposite
  ; responses when the fan is on the wrong header: one is a setting to
  ; correct, the other is a default to turn off.
  If fromBoard <> 0
    PrintN("fan: no fan pin was stored, so this board's own default is being used.")
    PrintN("     Type fan pin <gpio> if the fan is on another pin, or fan mode off")
    PrintN("     to leave it alone, then settings save.")
  EndIf
