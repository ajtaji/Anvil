...
; ----------------------------------------------------------------------
;  WHEN AN OWNER STOPS BEING ONE - forum 888, 2026-09-16.
;
;  Ownership is released when a command finishes. A datagram that never
;  finishes a command - an empty keepalive, a lone line feed after the CR
;  that already ran a command, a half-typed line from a host that went away -
;  used to keep the console for good, and every other host was answered BUSY
;  until somebody touched the board. So, AT THE PROMPT ONLY (gConAtPrompt is
;  set when a command finishes and cleared when the next line is taken):
;
;    - an owner with nothing in progress is not an owner: nothing waits in
;      the keystroke ring and nothing is typed on the line;
;    - an owner that left a half-typed line and has sent nothing for
;      #NETCON_OWNER_IDLE_MS is not one either, and its line is discarded.
;
;  Either is decided when ANOTHER endpoint sends something, which is the only
;  moment ownership matters. A running command is never taken away: a long
;  silent payload keeps its console however quiet its host is.
; ----------------------------------------------------------------------
#NETCON_OWNER_IDLE_MS = 60000
Global gConAtPrompt.i = 0          ; 1 between a command's end and the next line
Global gConOwnerLastMs.i = 0       ; millis() of the owner's last datagram
Global gConOwnerLapsed.i = 0       ; owners released this way, for the gate
...
; ----------------------------------------------------------------------
;  NetConsoleOwnsCommand - is the command running now this peer's?
;
;  1 only when all three are true: the console is armed, a peer has been
;  seen, and the line being executed arrived from the network rather than
;  from the local port. gConOwner is set by NetConsoleTakeUdp when the
;  peer's line is taken and cleared by NetConsoleCommandDone, so this is
;  a fact about THIS command and not about the session.
; ----------------------------------------------------------------------
Procedure.i NetConsoleOwnsCommand()
  If gConOn = 0 Or gConPeerOk = 0
    ProcedureReturn 0
  EndIf
...
; ----------------------------------------------------------------------
;  NetConsoleWriteBulk - a TRANSFER'S payload straight to the peer.
;
;  1 if this datagram went, 0 if it did not. The caller decides ONCE,
;  before it starts, whether the peer is the destination (see
;  ConsoleWriteBulk in Anvil/Core/readback.pbi); this never silently
;  reroutes a block, because a block that went two ways would arrive
;  twice.
;
;  WHY THIS EXISTS AT ALL, and it is the whole of the 2026-09-16 console
;  readback measurement. Every other byte the monitor emits goes through
;  UartWrite, which SPINS ON THE PL011 TRANSMIT FIFO and only then copies
;  the byte into the screen mirror and this console's aux tap. That is
;  correct for a terminal - the three consoles then agree, character for
;  character, about what was printed, and a byte the serial port had to
;  drop never appears on the screen looking real. It also means every
;  console on this board runs at the serial port's baud rate. At 115200
;  that is 11,520 bytes a second, so `memory` - which spends 78 bytes of
;  hex and ASCII on every 16 bytes of memory - read back at 1,730 bytes
;  a second over a GIGABIT link: 1.76 s of the measured 2.36 s per 4 KB
;  chunk was the serial shift register, and the wire contributed nothing
;  a clock could see.
;
;  THE RULE THIS ESTABLISHES: a TRANSFER'S bytes belong to the console
;  that asked for them, and a DISPLAY'S bytes belong to every console.
;  `memory` renders bytes as text for a person and is unchanged - it goes
;  everywhere, at the slowest sink's rate, which is what a display is
;  for. `readback` moves bytes to a machine that named itself by sending
;  the command, and they go to that machine at its own wire's rate and
;  nowhere else. Nobody watching the serial port or the screen sees the
;  payload, and both see the command and its verdict line, because those
;  are the parts a person can read.
;
;  IT FLUSHES THE ORDINARY PATH FIRST. The header line above a stream was
;  printed through UartWrite and is sitting in the aux ring; sending this
;  datagram before that one would put the stream in front of its own
;  header on the wire.
; ----------------------------------------------------------------------
Procedure.i NetConsoleWriteBulk(*p, n.i)
  Define fn.i
  If NetConsoleOwnsCommand() = 0
    ProcedureReturn 0
  EndIf
  ...
  EndIf
  gConTxOk = gConTxOk + 1
  ; THE SAME PACE THE SCREENSHOT USES, and for the same reason: a host
  ; UDP socket buffer is finite and back-to-back full datagrams off a
  ; gigabit link overrun it. 250 us a datagram is a ceiling of about
  ; 4 MB/s, which is far above anything this path needs and far below
  ; what a receiver drops.
  delayMicroseconds(#NETCON_PACE_US)
  ProcedureReturn 1
EndProcedure
...
Procedure.i NetConsoleTakeUdp(*frame, kind.i)
  ...
  If NetUdpRxDstPort() <> #NETCON_PORT
    ProcedureReturn 0
  EndIf
  ; AN EMPTY DATAGRAM CARRIES NO COMMAND, SO IT CANNOT TAKE THE CONSOLE -
  ; 2026-09-16, forum 888. Ownership is released only when a command
  ; finishes, so a datagram with nothing in it, taken as a claim at an idle
  ; prompt, owned the console for good: a host tool's keepalive did exactly
  ; that from a socket that then closed, and every other host was answered
  ; BUSY until the board restarted. It is consumed here and does nothing -
  ; no claim, no BUSY, no keystroke - which is also what makes an empty
  ; datagram a safe keepalive for a host whose command is still running.
  If NetUdpRxLen() = 0
    If netcon_PeerMatches(*frame, kind) <> 0
      gConOwnerLastMs = millis()
    EndIf
    ProcedureReturn 1
  EndIf
  If gConOwner = #NETCON_OWNER_NET
    If netcon_PeerMatches(*frame, kind) = 0
      If netcon_OwnerLapsed() <> 0
        ; See WHEN AN OWNER STOPS BEING ONE. Whatever it left half typed
        ; is thrown away before this endpoint's first byte, so a line can
        ; never be finished by somebody who did not start it.
        If TextEditLength() > 0
          TextEditDiscard()
        EndIf
        netcon_ForgetOwner()
        gConOwnerLapsed = gConOwnerLapsed + 1
      EndIf
    EndIf
  EndIf
...
; The line has been taken and a command is about to run under whoever owns
; the console now. From here until NetConsoleCommandDone nothing lapses.
Procedure NetConsoleCommandBegin()
  gConAtPrompt = 0
EndProcedure

; 1 when the network owner is no longer one. See WHEN AN OWNER STOPS BEING
; ONE: at the prompt only, and either nothing in progress or a half-typed line
; left silent for #NETCON_OWNER_IDLE_MS.
Procedure.i netcon_OwnerLapsed()
  If gConOwner <> #NETCON_OWNER_NET Or gConAtPrompt = 0
    ProcedureReturn 0
  EndIf
  If gConInHead = gConInTail And TextEditLength() = 0
    ProcedureReturn 1
  EndIf
  If gConInHead = gConInTail And (millis() - gConOwnerLastMs) >= #NETCON_OWNER_IDLE_MS
    ProcedureReturn 1
  EndIf
  ProcedureReturn 0
EndProcedure
