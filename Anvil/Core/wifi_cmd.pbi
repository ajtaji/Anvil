...
Procedure WifiRxReadyStatus()
  ...
  Print("  frame latch inherited/new ") : PrintDec(WifiRxReadyInheritedDelta())
  Print("/") : PrintDec(WifiRxReadyNewDelta()) : PrintNl()
  Print("  quiet nonempty channel data/control/event/glom/unknown/malformed ")
  PrintDec(WifiRxReadyQDataDelta()) : Print("/") : PrintDec(WifiRxReadyQControlDelta())
  Print("/") : PrintDec(WifiRxReadyQEventDelta()) : Print("/") : PrintDec(WifiRxReadyQGlomDelta())
  Print("/") : PrintDec(WifiRxReadyQUnknownDelta()) : Print("/") : PrintDec(WifiRxReadyQMalformedDelta()) : PrintNl()
  Print("  quiet nonempty previous NEXTLEN ") : PrintDec(WifiRxReadyQPrevNextDelta())
  Print(", sequence continuous/gap ") : PrintDec(WifiRxReadyQSeqOkDelta())
  Print("/") : PrintDec(WifiRxReadyQSeqGapDelta()) : PrintNl()
  Print("  quiet nonempty post-read host pending/quiet/failure ") : PrintDec(WifiRxReadyQPostPendingDelta())
  Print("/") : PrintDec(WifiRxReadyQPostQuietDelta()) : Print("/") : PrintDec(WifiRxReadyQPostFailDelta()) : PrintNl()
  PrintN("  Post-read host status is correlation only; it does not prove which cause produced the frame.")
