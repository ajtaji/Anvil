...
Procedure.i RxTimeoutTicks()
  Define hz.i
  If gToTicks > 0
    ProcedureReturn gToTicks
  EndIf
  hz = TickHz()
  If hz < 1000 Or hz > 1000000000
    ProcedureReturn 0
  EndIf
  ; Direct firmware entry clears BSS. A zero deadline expired before a
  ; 115200-baud character could arrive, so select a two-second default.
  gToTicks = hz * 2
  ProcedureReturn gToTicks
EndProcedure
...
Procedure.i OutBreakByte()
  ...
  If gBreakNetPump = 0
    gBreakNetPump = 1
    ; THE PUMP IS A BACKGROUND SERVICE EVEN HERE. It takes in whatever
    ; frames are waiting - a mid-session rekey among them - and anything
    ; that says is a message, not part of the answer the running command
    ; is printing. See THE MESSAGE CONTEXT in RaspberryPi4/Lib/uart.pi4.
    UartMessageBegin()
    NetConsolePump()
    UartMessageEnd()
    gBreakNetPump = 0
