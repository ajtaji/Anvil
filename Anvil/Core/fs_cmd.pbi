...
;
; A PATH MAY NAME ITS PARTITION: 2:/folder/name is partition 2 of the
; medium, and a path with no N: is partition 1. The prefix is kept in the
; copy and resolved by the board's storage seam, so this parser - which
; also serves a board whose files are reached through firmware - never has
; to know what a partition is.
;
; AND A PATH MAY START data: - `data:/models/MODEL.PMW` - which is resolved
; HERE, against the `data.path` setting, before the seam sees it (see
; Anvil/Core/datapath.pbi). data.path absent is partition 1's root.
Global Dim gDataPathBuf.a[1024]
...
Procedure.i ParseStoragePath()
  ...
  If n = 0
    ProcedureReturn -1
  EndIf
  If DataPathIs(@gName[0]) <> 0
    base = 0
    If SettingsHas("data.path") <> 0
      base = SettingsGet("data.path")
    EndIf
    got = DataPathJoin(base, @gName[0], @gDataPathBuf[0], 1024)
    If got < 0
      If got = -2
        PrintN("!! the data.path setting itself begins data:, so a data: path would name")
        PrintN("   itself and nothing was resolved. Set it to a real location, for")
        PrintN("   example settings set data.path 2:/ - or remove it for partition 1.")
      Else
        PrintN("!! that data: path, joined to the data.path setting, is longer than the")
        PrintN("   1023 bytes a storage path may be, so it was not used.")
      EndIf
      gName[0] = 0
      ProcedureReturn -1
    EndIf
...
Procedure CmdFat()
  ...
  If HwFileOpen(@gName[0]) = 0
    ...
    PrintN("   contains spaces, and start it with N: for partition N (2:/name) -")
    PrintN("   with no N: it is partition 1. Type f to see the partitions.")
...
Procedure CmdSave()
  ...
  ; be `If gMedium <> 1` followed by FatSetRangeWriter(@MscWriteBlocks)
...
;  worse than none. A path lists that directory, and N: in front of it picks
;  partition N (2:/folder). FAT32 and exFAT partitions both list any folder
;  by long name (a FAT entry with no long name shows its short name in the
;  case its display flags ask for). Until 2026-09-17 a FAT partition listed
;  its root only, by 8.3 name (forum 898).
;
...
Procedure CmdFatls()
  ...
  If HwDirOpen(@gName[0]) = 0
    ; A directory that is not there, a FAT partition asked for a folder, a
    ; partition that will not mount: each is said, never an empty listing.
    Print("!! the directory ")
    UartWriteStr(@gName[0])
    PrintN(" could not be opened for listing, so nothing")
    PrintN("   was shown. Start a path with N: for partition N (2:/folder); with no")
    PrintN("   N: it is partition 1. Type f to see the partitions.")
    Print("   The medium said: ")
    UartWriteStr(HwFileErrorText())
    PrintNl()
    ProcedureReturn
  ...
  PrintN("  Case is not significant. ls N:/folder lists a folder on partition N.")
EndProcedure
...
;
;  IT TAKES A PATH THE WAY `load` DOES - quoted when it has spaces, N: for
;  partition N - on a FAT32 or an exFAT partition alike, and a missing
;  name, refused by the storage seam in its own words.
;
...
Procedure CmdCat()
  ...
  If n = 0
    ...
    PrintN("   cat CONFIG.TXT         or   cat 2:/notes/readme.txt")
    PrintN("   Case does not matter. Quote a path that contains spaces, and start")
    PrintN("   it with N: for partition N; with no N: it is partition 1.")
    ProcedureReturn
  EndIf
  If n < 0
    PrintN("!! cat needs a valid path. Put a path containing spaces in double")
    PrintN("   quotes; an exFAT path may be at most 1023 UTF-8 bytes.")
    ProcedureReturn
  EndIf
  nm = 0
  ...
  If HwFileOpen(@gName[nm]) = 0
    Print("!! the file ")
    UartWriteStr(@gName[nm])
...
; ======================================================================
;  mkdir / rm / rename - the directory-level changes
; ----------------------------------------------------------------------
;  WHY THEY EXIST. The exFAT filesystem underneath could always create a
;  directory, remove an entry and rename one, and nothing at the prompt
;  could ask it to, so the first save into a new folder was impossible and
;  a scratch file could only ever be overwritten. Each is a thin command
;  over one storage seam call (HwDirCreate, HwFileRemove, HwFileRename),
;  and the seam does what `save` already relies on: it refuses before
;  arming anything when the medium is not writable, arms the block writer
;  around the one change, flushes, and disarms on every path out.
;
;  THE PATHS ARE THE SAME AS load's: quoted when they contain spaces, N: in
;  front for partition N, no N: meaning partition 1. A rename keeps to one
;  partition. FAT32 and exFAT partitions take all four the same way.
;
;  rm REMOVES ONE FILE OR ONE EMPTY DIRECTORY and nothing recursive. A
;  directory that still holds entries is refused, which is the only safe
;  default for a command typed into a monitor that has no undo.
; ======================================================================
Global Dim gName2.a[1024]

Procedure.i fs_cmd_PathOrSay(*cmd, *usage)
  Define n.i
  n = ParseStoragePath()
  If n > 0
    ProcedureReturn 1
  ...
  UartWriteStr(*cmd)
  If n = 0
    PrintN(" needs a path, and none was given, so nothing was changed.")
  Else
    PrintN(" needs a valid path. Put a path containing spaces in double")
    PrintN("   quotes; an exFAT path may be at most 1023 UTF-8 bytes. Nothing was changed.")
  EndIf
  Print("   ")
  UartWriteStr(*usage)
  PrintNl()
  PrintN("   Start a path with N: for partition N (2:/folder/name); with no N: it is")
  PrintN("   partition 1.")
  ProcedureReturn 0
EndProcedure

Procedure.i fs_cmd_Writable(*cmd)
  If HwStorageUp() = 0
    ProcedureReturn 0
  EndIf
  If HwFileWritable() = 0
    Print("!! ")
    UartWriteStr(*cmd)
    PrintN(" changed nothing, because this board cannot write to its storage")
    PrintN("   medium right now. No writer was armed.")
    Print("   The medium said: ")
    UartWriteStr(HwFileErrorText())
    PrintNl()
    ProcedureReturn 0
...
Procedure fs_cmd_Refused(*cmd)
  Print("!! ")
  UartWriteStr(*cmd)
  PrintN(" did not complete.")
  Print("   The medium said: ")
  UartWriteStr(HwFileErrorText())
  PrintNl()
EndProcedure
...
Procedure CmdMkdir()
  If RequireCap(#CAP_STORAGE, "mkdir", "this board has no block storage medium Anvil can read") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_PathOrSay("mkdir", "mkdir <path>") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_Writable("mkdir") = 0
    ProcedureReturn
  EndIf
  If HwDirCreate(@gName[0]) = 0
    fs_cmd_Refused("mkdir")
    PrintN("   A parent directory that is not there is not created for you.")
    ProcedureReturn
  EndIf
  Print("Made the directory ")
  UartWriteStr(@gName[0])
  PrintN(". The volume was flushed and the block")
  PrintN("writer disarmed again.")
EndProcedure

Procedure CmdRemove()
  If RequireCap(#CAP_STORAGE, "rm", "this board has no block storage medium Anvil can read") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_PathOrSay("rm", "rm <path>       one file, or one EMPTY directory") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_Writable("rm") = 0
    ProcedureReturn
  EndIf
  If HwFileRemove(@gName[0]) = 0
    fs_cmd_Refused("rm")
    PrintN("   A directory must be empty before it can be removed; nothing inside")
    PrintN("   one is ever removed by this command.")
    ProcedureReturn
  EndIf
  Print("Removed ")
  UartWriteStr(@gName[0])
  PrintN(". Its space is free again, the volume was flushed")
  PrintN("and the block writer disarmed.")
EndProcedure

Procedure CmdRmdir()
  If RequireCap(#CAP_STORAGE, "rmdir", "this board has no block storage medium Anvil can read") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_PathOrSay("rmdir", "rmdir <path>    one EMPTY directory, never a file") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_Writable("rmdir") = 0
    ProcedureReturn
  EndIf
  If HwDirRemove(@gName[0]) = 0
    fs_cmd_Refused("rmdir")
    PrintN("   rmdir removes only an empty directory; a file is refused, and so is")
    PrintN("   a directory that still holds anything.")
    ProcedureReturn
  EndIf
  Print("Removed the directory ")
  UartWriteStr(@gName[0])
  PrintN(". The volume was flushed and the block")
  PrintN("writer disarmed.")
EndProcedure

Procedure CmdRename()
  Define i.i
  If RequireCap(#CAP_STORAGE, "rename", "this board has no block storage medium Anvil can read") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_PathOrSay("rename", "rename <old path> <new path>") = 0
    ProcedureReturn
  EndIf
  i = 0
  While gName[i] <> 0
  ...
  Wend
  gName2[i] = 0
  If fs_cmd_PathOrSay("rename", "rename <old path> <new path>, both on the same partition") = 0
    ProcedureReturn
  EndIf
  If fs_cmd_Writable("rename") = 0
    ProcedureReturn
  EndIf
  If HwFileRename(@gName2[0], @gName[0]) = 0
    fs_cmd_Refused("rename")
    PrintN("   The new name must not exist yet, and both paths must be on one")
    PrintN("   partition - a path with no N: is partition 1.")
    ProcedureReturn
  EndIf
  Print("Renamed ")
  UartWriteStr(@gName2[0])
  Print(" to ")
  UartWriteStr(@gName[0])
  PrintN(".")
  PrintN("The volume was flushed and the block writer disarmed.")
EndProcedure
