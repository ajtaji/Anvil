; The display connector lifecycle is shared policy with no hardware access.
; Boards opt in through the bounded action/result seam; without registration
; it is inert. Include it beside common state so Pi 3 and Pi 4 compositions and
; their reduced test fixtures see one contract rather than board copies.
XIncludeFile "Anvil/Core/screen_connection.pbi"
