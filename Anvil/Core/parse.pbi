...
Procedure ReadLine()
  ...
  Repeat
    ...
    If c >= 0
      ...
      Continue
    EndIf
    ; THE SECOND HALF OF THE SERVICES, IN THE SAME CONTEXT. See THE MESSAGE
    ; CONTEXT above; the bracket was closed only so the peer's byte could be
    ; taken in and applied as typing, which is not a message.
    UartMessageBegin()
    MouseTick()
    TouchTick()
