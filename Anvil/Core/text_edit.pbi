...
; ----------------------------------------------------------------------
;  TextEditDiscard - throw the line away, and take it off the screen.
;
;  For a line nobody will finish: the network console hands the prompt to
;  another host when the one that was typing has gone silent (forum 888),
;  and that host's first keystroke must not land on the end of somebody
;  else's half-typed command. It is the editor's own vocabulary, not a
;  message: the caret walks to the end by re-sending the tail, and every
;  character is then erased with backspace, space, backspace, so each
;  console shows the bare prompt the line started from. Nothing is run.
; ----------------------------------------------------------------------
