...
Procedure CmdBlock()
  ...
  If HitsMonitor(addr, hi) <> 0
    ...
    ; have been. A range at #MON_DATA_LO hits the DATA window and has
...
; The Wi-Fi block receiver is present only when the board has a network
; backend. Keeping this tied to the board capability lets the serial
; receive path compile on boards that provide no NetConsole procedures.
#ANVIL_WIFI_OTA = #CAP_NET
