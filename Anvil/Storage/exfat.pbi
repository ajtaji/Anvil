...
;
; This driver owns filesystem policy only.  A board supplies 512-byte block
; callbacks; no USB, SD, firmware or target register appears here.  All media
...
Global exfat_utcOffset.i = $80 ; unknown offset, per the format
...
Procedure ExFatSetBlockReader(address.i)
  *exfat_reader = address
EndProcedure

Procedure ExFatSetBlockWriter(address.i)
...
Procedure.i exfat_ReadBlock(lba.i, *dst)
  ...
  If exfat_partBlocks <> 0
    If exfat_BlockInPartition(lba) = 0
      ProcedureReturn exfat_Fail(#EXFAT_E_BOUNDS)
    EndIf
  EndIf
  exfat_lastLba = lba
  If exfat_reader(lba, *dst) = 0
    ProcedureReturn exfat_Fail(#EXFAT_E_READ)
  EndIf
  ProcedureReturn 1
EndProcedure

Procedure.i exfat_WriteBlock(lba.i, *src)
  If *exfat_writer = 0
...
Procedure.i exfat_ReadSector(sector.i, *dst)
  Define b.i
  Define lba.i
  If sector < 0 Or sector >= exfat_volumeBlocks / exfat_blocksPerSector
    ProcedureReturn exfat_Fail(#EXFAT_E_BOUNDS)
  EndIf
  lba = exfat_partLba + sector * exfat_blocksPerSector
  b = 0
  While b < exfat_blocksPerSector
    If exfat_ReadBlock(lba + b, *dst + b * #EXFAT_BLOCK_SIZE) = 0
      ProcedureReturn 0
    EndIf
    b = b + 1
  Wend
...
Procedure.i exfat_WriteSector(sector.i, *src)
  ...
  b = 0
  While b < exfat_blocksPerSector
    If exfat_WriteBlock(lba + b, *src + b * #EXFAT_BLOCK_SIZE) = 0
      ProcedureReturn 0
    EndIf
    b = b + 1
  Wend
...
Procedure.i exfat_FatEntry(cluster.i)
  ...
  If exfat_ReadSector(sector, @exfat_aux[0]) = 0
    ProcedureReturn -1
  EndIf
  ProcedureReturn exfat_U32(@exfat_aux[0], off) & $FFFFFFFF
EndProcedure
...
Procedure.i exfat_SetFatEntry(cluster.i, value.i)
  ...
  If exfat_ReadSector(sector, @exfat_aux[0]) = 0
    ProcedureReturn 0
  EndIf
  exfat_Put32(@exfat_aux[0], off, value)
  ProcedureReturn exfat_WriteSector(sector, @exfat_aux[0])
EndProcedure
...
Procedure.i exfat_StreamRead(first.i, flags.i, length.i, offset.i, *dst, count.i)
  ...
  While done < count
    ...
    cluster = exfat_StreamCluster(first, flags, clusterIndex)
    If cluster < 0
      ProcedureReturn -1
    EndIf
    sector = exfat_ClusterSector(cluster, sectorIn)
    If sector < 0 Or exfat_ReadSector(sector, @exfat_sector[0]) = 0
      ProcedureReturn -1
    EndIf
    ...
    If take > count - done
      take = count - done
    EndIf
    exfat_Copy(*dst + done, @exfat_sector[0] + inSector, take)
    done = done + take
  Wend
...
Procedure.i ExFatDirOpen(*path)
  If exfat_mounted = 0
    ProcedureReturn exfat_Fail(#EXFAT_E_NOT_MOUNTED)
  EndIf
  If *path = 0 Or PeekA(*path) = 0 Or (PeekA(*path) = 47 And PeekA(*path + 1) = 0) Or (PeekA(*path) = 92 And PeekA(*path + 1) = 0)
...
Procedure.i exfat_BitmapGet(cluster.i)
  ...
  bit = cluster - 2
  byteOff = bit >> 3
  b = exfat_StreamByte(exfat_bitmapCluster, 0, exfat_bitmapLength, byteOff)
  If b < 0
    ProcedureReturn -1
  EndIf
  ProcedureReturn (b >> (bit & 7)) & 1
EndProcedure
...
Procedure.i exfat_BitmapSet(cluster.i, used.i)
  ...
  Else
    b = b & ~(1 << (bit & 7))
  EndIf
  If exfat_StreamWriteFixed(exfat_bitmapCluster, 0, exfat_bitmapLength, byteOff, @b, 1) = 0
    ProcedureReturn 0
  EndIf
...
Procedure.i exfat_EnsureClusters(first.i, flags.i, oldLength.i, newLength.i)
  Define have.i
  Define need.i
  Define last.i
  Define nxt.i
  ...
  If have = 0
    ...
    have = 1
  ElseIf (flags & #EXFAT_STREAM_NO_FAT_CHAIN) <> 0
    If exfat_MaterializeContiguous(first, have) = 0
      ProcedureReturn 0
    EndIf
    exfat_growFlags = flags & ~#EXFAT_STREAM_NO_FAT_CHAIN
  EndIf
  last = exfat_StreamCluster(exfat_growFirst, exfat_growFlags, have - 1)
  If last < 0
    ProcedureReturn 0
  EndIf
  i = have
  While i < need
    nxt = exfat_AllocateOne()
    If nxt = 0
      ProcedureReturn 0
    EndIf
...
Procedure.i exfat_UpdateEntrySet(parentFirst.i, parentFlags.i, parentLength.i, setIndex.i, secondary.i, newFirst.i, newFlags.i, validLength.i, dataLength.i, attr.i)
  Define checksum.i
  If exfat_DirRead(parentFirst, parentFlags, parentLength, setIndex, @exfat_entry[0]) = 0
    ProcedureReturn 0
  EndIf
  exfat_Put16(@exfat_entry[0], 4, attr)
  exfat_Put16(@exfat_entry[0], 12, exfat_time)
  exfat_Put16(@exfat_entry[0], 14, exfat_date)
  PokeA(@exfat_entry[0] + 21, exfat_time10ms)
  PokeA(@exfat_entry[0] + 23, exfat_utcOffset)
  If exfat_DirWrite(parentFirst, parentFlags, parentLength, setIndex, @exfat_entry[0]) = 0
    ProcedureReturn 0
  EndIf
  If exfat_DirRead(parentFirst, parentFlags, parentLength, setIndex + 1, @exfat_aux[0]) = 0
    ProcedureReturn 0
  EndIf
  PokeA(@exfat_aux[0] + 1, newFlags)
  exfat_Put64(@exfat_aux[0], 8, validLength)
  exfat_Put32(@exfat_aux[0], 20, newFirst)
  exfat_Put64(@exfat_aux[0], 24, dataLength)
  If exfat_DirWrite(parentFirst, parentFlags, parentLength, setIndex + 1, @exfat_aux[0]) = 0
    ProcedureReturn 0
  EndIf
  checksum = exfat_SetChecksum(parentFirst, parentFlags, parentLength, setIndex, secondary)
  If checksum < 0
    ProcedureReturn 0
  EndIf
  If exfat_DirRead(parentFirst, parentFlags, parentLength, setIndex, @exfat_entry[0]) = 0
    ProcedureReturn 0
  EndIf
  exfat_Put16(@exfat_entry[0], 2, checksum)
  ProcedureReturn exfat_DirWrite(parentFirst, parentFlags, parentLength, setIndex, @exfat_entry[0])
EndProcedure
...
Procedure.i ExFatWrite(*src, count.i)
  ...
  If exfat_EnsureClusters(exfat_fileFirst, exfat_fileFlags, oldLength, endPos) = 0
    ProcedureReturn -1
  EndIf
  exfat_fileFirst = exfat_growFirst
  exfat_fileFlags = exfat_growFlags
  If exfat_StreamWriteFixed(exfat_fileFirst, exfat_fileFlags, ((endPos + exfat_bytesPerCluster - 1) / exfat_bytesPerCluster) * exfat_bytesPerCluster, exfat_filePos, *src, count) = 0
    ProcedureReturn -1
  EndIf
...
Procedure.i ExFatTruncate(newLength.i)
  ...
  If newLength > exfat_fileLength
    If exfat_EnsureClusters(exfat_fileFirst, exfat_fileFlags, exfat_fileLength, newLength) = 0
      ProcedureReturn 0
    EndIf
    exfat_fileFirst = exfat_growFirst
    exfat_fileFlags = exfat_growFlags
    ; New clusters are zeroed by the allocator. Existing partial-cluster tail
    ; is also cleared so ValidDataLength can safely advance.
    If newLength > exfat_fileLength
      tailLength = newLength - exfat_fileLength
      exfat_Zero(@exfat_aux[0], exfat_bytesPerSector)
      While tailLength > 0
        cut = tailLength
        If cut > exfat_bytesPerSector
          cut = exfat_bytesPerSector
        EndIf
        If exfat_StreamWriteFixed(exfat_fileFirst, exfat_fileFlags, ((newLength + exfat_bytesPerCluster - 1) / exfat_bytesPerCluster) * exfat_bytesPerCluster, exfat_fileLength + (newLength - exfat_fileLength - tailLength), @exfat_aux[0], cut) = 0
          ProcedureReturn 0
        EndIf
        tailLength = tailLength - cut
      Wend
    EndIf
...
Procedure.i exfat_GrowParentDirectory()
  ...
  If exfat_EnsureClusters(exfat_parentFirst, exfat_parentFlags, oldAllocation, newAllocation) = 0
    ProcedureReturn 0
  EndIf
...
Procedure.i exfat_WriteNewSet(parentFirst.i, parentFlags.i, parentLength.i, setIndex.i, *name, nameLength.i, attr.i, first.i, streamFlags.i, validLength.i, dataLength.i, *primaryTemplate)
  ...
  If hash < 0
    ProcedureReturn 0
  EndIf
  ; Stream and name secondaries are written first. Without the primary they
  ; are not an entry set, so interrupted creation cannot expose a half-file.
  exfat_Zero(@exfat_aux[0], 32)
  PokeA(@exfat_aux[0], #EXFAT_ENTRY_STREAM)
  streamFlags = streamFlags | #EXFAT_STREAM_ALLOC_POSSIBLE
  PokeA(@exfat_aux[0] + 1, streamFlags)
  PokeA(@exfat_aux[0] + 3, nameLength)
  exfat_Put16(@exfat_aux[0], 4, hash)
  exfat_Put64(@exfat_aux[0], 8, validLength)
  exfat_Put32(@exfat_aux[0], 20, first)
  exfat_Put64(@exfat_aux[0], 24, dataLength)
  If exfat_DirWrite(parentFirst, parentFlags, parentLength, setIndex + 1, @exfat_aux[0]) = 0
    ProcedureReturn 0
  EndIf
  ...
  While e < nameEntries
    exfat_Zero(@exfat_aux[0], 32)
    PokeA(@exfat_aux[0], #EXFAT_ENTRY_NAME)
    j = 0
    While j < 15
      If n < nameLength
        exfat_Put16(@exfat_aux[0], 2 + j * 2, PeekW(*name + n * 2) & $FFFF)
        n = n + 1
      EndIf
      j = j + 1
    Wend
    If exfat_DirWrite(parentFirst, parentFlags, parentLength, setIndex + 2 + e, @exfat_aux[0]) = 0
      ProcedureReturn 0
    EndIf
    e = e + 1
  Wend
  If *primaryTemplate <> 0
    exfat_Copy(@exfat_entry[0], *primaryTemplate, 32)
  Else
    exfat_Zero(@exfat_entry[0], 32)
  EndIf
  PokeA(@exfat_entry[0], #EXFAT_ENTRY_FILE)
  PokeA(@exfat_entry[0] + 1, secondary)
  exfat_Put16(@exfat_entry[0], 4, attr)
  If *primaryTemplate = 0
    exfat_Put16(@exfat_entry[0], 8, exfat_time)
    exfat_Put16(@exfat_entry[0], 10, exfat_date)
    exfat_Put16(@exfat_entry[0], 12, exfat_time)
    exfat_Put16(@exfat_entry[0], 14, exfat_date)
    exfat_Put16(@exfat_entry[0], 16, exfat_time)
    exfat_Put16(@exfat_entry[0], 18, exfat_date)
    PokeA(@exfat_entry[0] + 20, exfat_time10ms)
    PokeA(@exfat_entry[0] + 21, exfat_time10ms)
    PokeA(@exfat_entry[0] + 22, exfat_utcOffset)
    PokeA(@exfat_entry[0] + 23, exfat_utcOffset)
    PokeA(@exfat_entry[0] + 24, exfat_utcOffset)
  EndIf
  checksum = exfat_PrimaryChecksum(@exfat_entry[0], parentFirst, parentFlags, parentLength, setIndex, secondary)
  If checksum < 0
    ProcedureReturn 0
  EndIf
  exfat_Put16(@exfat_entry[0], 2, checksum)
...
Procedure.i ExFatUnmount()
  ...
  EndIf
  If exfat_mounted <> 0 And exfat_dirty <> 0
    If ExFatFlush() = 0
      ProcedureReturn 0
    EndIf
  EndIf
