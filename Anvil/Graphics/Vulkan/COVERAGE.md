...
| File | What it owns |
|---|---|
| `vk_core_1_0.pbi` | Registry-derived vocabulary: constants and 64 naturally aligned AArch64 C-layout structures with the registry's own member names, order and widths. `VkPhysicalDeviceFeatures` is complete even though every member currently reports false. |
...
| `vk_memory.pbi` | `VkDeviceMemory` and `VkImage`: the device heap, first-fit suballocation with real reuse, memory requirements, binding rules, image usages including `COLOR_ATTACHMENT`, layout and queue-family ownership. |
...
|---|---|
| Generated vocabulary | A pinned core-1.0 generator exists. The checked-in slice contains the exact result, structure-type, command-lifecycle, image, memory, queue-family, access, stage, aspect and fence values this implementation uses, plus 64 scalar, nested, fixed-array, structure-array and pointer-bearing structures. It is not the complete 1.0 vocabulary. `VkPhysicalDeviceFeatures` carries all 55 core feature members; every member is currently false. `VkClearColorValue` is a union and is deliberately **not** declared, because PureMetal has no union and one arm of it posing as the whole type is the silent wrong answer this project refuses. |
| Semantic implementation | Opaque typed and generation-tagged handles for nine object types; instance, physical device, device and one queue that always advertises transfer and advertises graphics exactly when its backend owns the draw capability; device memory and linear `B8G8R8A8_UNORM` images with exact requirements, binding rules, usage and per-image layout; framebuffer colour attachments must carry `COLOR_ATTACHMENT`, while transfer source/destination bits authorize only transfer work; image memory barriers with layout tracking checked at record time and again at submit time; whole-image colour clears; one-at-a-time submission with resource retention; fences with the full two-state contract and a doubly bounded finite host wait, plus an explicit refusal for unsatisfied `UINT64_MAX` rather than a false timeout; one-sample `pSampleMask` bit zero carried into the backend so a zero mask suppresses primitive emission without suppressing the render-pass clear; and triangle-list draws where zero vertices consume no draw slot and a trailing incomplete triangle is discarded by primitive assembly. Every refusal is a real code and a whole sentence naming it and the next thing to check. |
