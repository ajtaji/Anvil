...
; ----------------------------------------------------------------------
;  CORRECTED 2026-09-15. THIS USED TO SAY THE CONSTRAINT WAS A COMPILER
;  FACT. It no longer is, on this backend, and the three consequences
;  below are unchanged - so the REASON had to be rewritten before someone
;  read the old reason, checked it, found it false, and concluded the
;  rules it justified were false too.
;
;  What it said: "EVERY PROCEDURE PARAMETER IN THIS LANGUAGE IS A STATIC
;  GLOBAL (IRCore_A64.pbi:2315-2349). Locals are static .bss slots. NO
;  PROCEDURE IN THIS LANGUAGE IS RE-ENTRANT, and that is not a Pi 4 fact -
;  it is a compiler fact."
;
;  That was true when it was written and it is not true now. On the 64-bit
;  Arm backend a procedure's parameters AND its locals live in that
;  invocation's own frame, addressed from x29 - RaspberryPi4/A64FramePlan.pbi
;  plans one slot per automatic symbol and carries an IsParameter flag for
;  each, and IRCore_A64.pbi refuses any lowering that is not a negative
;  displacement from the frame base. SP is per-core. So the same generated
;  procedure CAN be entered again while it is running, and CAN run on four
;  cores at once, which is exactly what ABI 1.3's parallel group offers a
;  payload for its own code.
;
;  THE REAL REASON IS ANVIL'S OWN STATE, AND IT DID NOT CHANGE. This
;  monitor's drivers and services keep their state in Globals with one
;  writer and no lock: the console's cursor, the network's buffers, the
;  filesystem's handles, gSvcDetail, gSvcClip. Those are shared no matter
;  how a procedure's parameters are passed. So every rule below stands,
;  for a reason that is about THIS PROGRAM rather than about the language
;  it is written in.
;
;  (The older reason still holds on the 8-bit backends, which is why
;  RaspberryPi4/Lib/sched.pi4 is cooperative rather than pre-emptive.)
;
...
;      would re-enter Anvil's own code on top of a call the main line is
;      already inside, and Anvil's globals have one writer.
;      Interrupts are masked at entry and the payload has no reason to
;      unmask them.
;    * A SLOT IS CORE 0 ONLY, AT EVERY VERSION OF THIS ABI. A payload may
;      use the other three cores for PURE COMPUTATION - and from 1.3 it
;      may run its OWN generated procedures there, not only naked
;      assembly - but no slot may be called from any core but the one
;      that was entered. The thing that changed at 1.3 is what the
;      payload may put on a secondary, never who may call into Anvil.
;
...
;    184   Parallel execution         8         3   (new at 1.3)
;    192   (next group starts here)
;
...
; NEW AT 1.3, AND THE FIRST GROUP ADDED SINCE THE TABLE WAS DRAWN. The
; core group was the obvious home for "run this on four cores" and it is
; FULL - 16 of 16 since 1.1 filled its last two with the module seam
; pair - so the versioning rule above applies literally: a group that
; fills its block gets a new block at the end. entry_count rises 184 to
; 192 and not one existing index moves.
#SVC_BASE_PAR     = 184
...
Procedure.i SvcCapGet(cap.i)
  ...
  If cap = #SVCCAP_PWM Or cap = #SVCCAP_THERMAL
    ...
    ProcedureReturn 0
  EndIf
  ; NEW AT 1.3, AND ANSWERED THE OLD WAY. A module cannot fill this one:
  ; running a payload procedure on a secondary core needs the reserved
  ; stacks, the spin-table contract and the resident job owner, all three
  ; of which are in the monitor before any module is loaded. So it is a
  ; compile-time #CAP_* like the first fourteen, not a seam question.
  If cap = #SVCCAP_PARALLEL : ProcedureReturn #CAP_PARALLEL : EndIf
  ProcedureReturn 0
EndProcedure
...
; Is a payload-supplied key one of the two guarded names?
;
; THE GUARD MUST BE EXACTLY AS LAX AS THE STORE IT GUARDS - same fold,
; same answer - AND IT GETS THAT BY ASKING THE STORE, not by holding an
; opinion of its own. SettingsKeySame() is the store's own definition of
; "these two spellings are the same key"; the whole question here is
; whether the write this payload is asking for would land on the setting
; the reboot-loop guard depends on, and only the store can say.
;
; IT USED TO COMPARE BYTE FOR BYTE, and a comment here asserted that the
; settings store was case-sensitive. It is not and never was: names are
; case-insensitive by design and the store folds every one to lower case
; before it is written or looked up (see THE KEY in settings.pbi). So the
; comparison recognised FEWER names than the store treats as one, which
; is the dangerous direction for a guard to be wrong in: `BOOT.FAILS` did
; not match, was accepted, and was then folded onto the very setting the
; guard exists to protect. The hole was one capital letter wide and it
; defeated the reboot-loop stop outright. There is no special case here
; for either name, because a special case would be a third opinion about
; key identity - the fix is that there is now exactly one.
...
; ======================================================================
;  GROUP 15 - PARALLEL EXECUTION, base 184. NEW AT ABI 1.3.
; ----------------------------------------------------------------------
;  THE PAYLOAD HANDS OVER ONE PROCEDURE AND N ARGUMENT BLOCKS. Anvil runs
;  that procedure on cores 0, 1, 2 and 3 - the primary takes a block too -
;  waits for all of them inside a bounded deadline, and returns. The
;  partitions are the payload's, and it makes them DISJOINT OUTPUT
;  RANGES, so no result is written by two cores and no result's
;  arithmetic order changes. A kernel that is bit-identical to its scalar
;  reference on one core stays bit-identical on four, by construction
;  rather than by measurement.
;
;  THIS IS THE ONE SLOT MOST LIKELY TO BE READ AS RELAXING THE CORE-0
;  RULE, AND IT DOES NOT. Cores 1..3 run PAYLOAD code. They may not call
;  a service slot, print, touch a device, pet the deadman, unmask
;  interrupts, or reach Anvil's state at all - Anvil's own globals are
;  single-writer and its drivers have one set of them and no lock. What
;  changed at 1.3 is only that the payload's own code may now run there:
;  on this backend a procedure's parameters and locals live in its frame,
;  so four invocations on four stacks do not collide. Anvil's do not
;  follow them across.
;
;  THE ENTRY IS CALLED AS entry(argsBase + i * argsStride) AND NOTHING
;  ELSE IS DEFINED. Its return value is kept and readable; zero means it
;  is content. It must return.
;
;  WHAT A REFUSAL COSTS. A partition that does not answer inside the
;  bound latches the service failed and every later call refuses; there
;  is no warm rejoin for a core in that state, because a core that
;  stopped answering cannot be proven safe to re-use. Recovery is a
;  reset. That is the same rule the resident acceptance owner already
;  follows and it is deliberately strict.
;
;  THE WORKERS DO NOT OUTLIVE THE PAYLOAD THAT STARTED THEM, and a
;  payload author should count on nothing surviving its own return. A
;  worker walks whatever page tables were live when it was started, and
;  a payload that turns its own MMU on builds those tables in its own
;  memory - so Anvil sends every secondary back to the firmware spin
;  state at the payload boundary, holding nothing, and the next
;  payload's first SvcParallelRun starts them again in whatever regime
;  is live then. The visible consequence is the one that matters: a
;  four-core payload no longer costs a restart, and SvcParallelCores()
;  answers the same 4 to the payload after it as to the payload before
;  it.
;
;  THE COROLLARY, AND IT IS NOT AN IMPLEMENTATION DETAIL: the number
;  SvcParallelCores() returns is true when it is asked and is not a
;  constant for the life of a payload. A payload that asks before
;  enabling its own MMU and caches is asking in a context where the
;  service cannot start a core, and gets 1. Ask again once the payload's
;  own regime is up.
; ======================================================================
CompilerIf #CAP_PARALLEL = 1

; Slot 184. HOW MANY PARTITIONS THIS MONITOR WILL RUN AT ONCE.
;
; NEVER AN ERROR, and 1 is a real answer rather than a failure: it means
; "this Anvil will run your work on the core you are already on". A
; monitor older than 1.3 gives the same answer by not having the slot, so
; a payload has ONE fallback path - its existing single-core code - and
; not two.
Procedure.i SvcParallelCores()
  If SvcRequireCap(#CAP_PARALLEL, "parallel", "this monitor cannot run a payload procedure on more than one core") = 0
    ProcedureReturn 1
  EndIf
  ProcedureReturn CoreParallelCores()
EndProcedure

; Slot 185. RUN entry ON count CORES AND JOIN.
Procedure.i SvcParallelRun(entry.i, argsBase.i, argsStride.i, count.i)
  Define r.i
  If SvcRequireCap(#CAP_PARALLEL, "parallel", "this monitor cannot run a payload procedure on more than one core") = 0
    ProcedureReturn #SVC_ENOCAP
  EndIf
  r = CoreParallelRun(entry, argsBase, argsStride, count)
  If r <> 0
    gSvcDetail = 0
    ProcedureReturn #SVC_OK
  EndIf
  ; One core-level cause, one service code, one sentence. The sentence
  ; keeps the number and names the first thing to look at, and the core it
  ; happened on is in SvcErrDetail rather than lost.
  Select CoreParallelErr()
    Case #CORE_PAR_ERR_ARG
      gSvcDetail = "count must be 1 to 4, entry must be a non-zero word-aligned address, and argsStride must not be negative"
      ProcedureReturn #SVC_EARG
    Case #CORE_PAR_ERR_CONTEXT
      gSvcDetail = "a parallel run may only be started from the core the payload was entered on, at EL3, with interrupts masked"
      ProcedureReturn #SVC_EPERM
    Case #CORE_PAR_ERR_BUSY
      gSvcDetail = "the secondary cores could not be started because something else already holds them"
      ProcedureReturn #SVC_EBUSY
    Case #CORE_PAR_ERR_STATE
      gSvcDetail = "a secondary core has an unfinished job outstanding or was permanently parked, and neither is recoverable without a reset"
      ProcedureReturn #SVC_ESTATE
    Case #CORE_PAR_ERR_TIMEOUT
      gSvcDetail = "a secondary core did not finish its partition inside the five-second bound; a payload entry that faults on a secondary parks it silently, because a secondary has no exception vectors of its own"
      ProcedureReturn #SVC_ETIMEOUT
    Case #CORE_PAR_ERR_FPCR
      gSvcDetail = "a secondary core is not rounding the way the primary is, and bit-identical results need every core to share one floating-point control register"
      ProcedureReturn #SVC_EPERM
    Case #CORE_PAR_ERR_GUARD
      gSvcDetail = "a partition ran off the bottom of its secondary stack and overwrote the guard below it, so this job's outputs cannot be trusted"
      ProcedureReturn #SVC_EPERM
    Case #CORE_PAR_ERR_WORKER
      gSvcDetail = "the payload's own partition procedure reported failure; the value it returned is in SvcParallelStatus and no Anvil state is at fault"
      ProcedureReturn #SVC_EIO
  EndSelect
  gSvcDetail = "the parallel run did not complete and gave no reason, which is itself a defect worth reporting"
  ProcedureReturn #SVC_EIO
EndProcedure

; Slot 186. WHAT PARTITION index DID. The entry's own return value on a
; run that completed, and the failing partition's index is what
; SvcErrDetail's sentence is about on one that did not. Valid only
; immediately after the call, like SvcErrDetail.
Procedure.i SvcParallelStatus(index.i)
  If SvcRequireCap(#CAP_PARALLEL, "parallel", "this monitor cannot run a payload procedure on more than one core") = 0
    ProcedureReturn 0
  EndIf
  ProcedureReturn CoreParallelStatus(index)
EndProcedure

CompilerElse

; A board that declares #CAP_PARALLEL = 0 has no secondary-core owner to
; link against, so this is what compiles there. SvcParallelCores still
; answers a NUMBER rather than an error, because "one" is the truth and a
; payload that divides its work by it is correct on this board too.
Procedure.i SvcParallelCores()
  ProcedureReturn 1
EndProcedure
...
;  THE SLOT INSTALLER.
;
;  BuildServiceTable() fills the header and all #SVC_SLOT_COUNT pointers
;  (192 at 1.3, 184 before it). Called by
...
Procedure BuildServiceTable()
  ...
  ; Group 15, new at 1.3. Both arms of the CompilerIf define all three, so
  ; these lines are not themselves gated: on a board without the
  ; capability they point at the arm that answers one core and
  ; #SVC_ENOCAP, which is a better answer than @SvcUnimplemented's
  ; #SVC_ENOSYS - "this board has one core for you" rather than "this
  ; monitor has never heard of the question".
  gSvcTab[#SVC_HDR_WORDS + #SVC_BASE_PAR + 0] = @SvcParallelCores
  gSvcTab[#SVC_HDR_WORDS + #SVC_BASE_PAR + 1] = @SvcParallelRun
  gSvcTab[#SVC_HDR_WORDS + #SVC_BASE_PAR + 2] = @SvcParallelStatus
