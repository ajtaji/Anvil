...
;              takes its block reader/writer by pointer (FsSetRangeReader
;              / FsSetRangeWriter over MscReadBlocks / SdReadBlocks). These
...
;              fat.pi4 (Anvil/Storage/hwfile.pbi), the Q over UEFI
...
;                 HwFileRemove(*path)    remove a file, or an empty
;                                        directory. 1 or 0.
;                 HwFileRename(*old, *new)
;                                        rename or move an entry within
;                                        one volume. 1 or 0.
;                 HwDirCreate(*path)     create a directory. 1 or 0.
;                 HwDirRemove(*path)     remove an EMPTY directory, and
;                                        refuse a file. 1 or 0.
;                                        All three are write-gated exactly
;                                        like HwFileWriteAll, and a board
;                                        whose medium cannot do one answers
;                                        0 with #HW_FILE_UNSUPPORTED. On the
;                                        Pi 4 a path may begin N: to name
;                                        partition N (Anvil/Storage/
;                                        filesystem.pbi); no prefix is
;                                        partition 1.
...
;                                        in sentences, FOR EVERY
;                                        INTERFACE THIS BOARD HAS - not
;                                        for the one traffic happens to
;                                        be leaving by. A pin is a
;                                        routing choice; it is not
;                                        evidence about what an
;                                        interface holds or has counted,
;                                        and this member may not read
;                                        it. An interface with no such
;                                        counters gets a sentence saying
;                                        so, named, beside the one that
;                                        has them. Not the IP layer's
...
;
;    THE CONSOLE BYTES  (NO CAPABILITY)  THE MONITOR'S OWN TEXT, IN AND
;              OUT. Twenty-five names every board supplies and no shared
;              file defines - and until 2026-09-18 they were a real,
;              working, three-board seam that was written down nowhere.
;              Roughly 1,400 calls in Anvil/ resolve to them.
;
;              THIS IS NOT HwCon*. HwCon* below is a PIXEL SURFACE FOR A
;              PAYLOAD. This is the terminal the operator types at, and
;              it moves bytes, not rectangles.
;
;              THE TRANSPORT:
;                 UartInit(baud)         bring the console up from a baud
;                                        alone; 0 if this board cannot,
;                                        because it needs a clock it has
;                                        to be told
;                 UartInitClk(baud, hz)  the same with the clock supplied
;                 UartRunning()          1 once it is up
;                 UartWrite(c)           one byte. BOUNDED - a console
;                                        that spins on a FIFO that will
;                                        never drain is indistinguishable
;                                        from a crashed board
;                 UartWriteStr(*s)       a NUL-terminated string
;                 UartWriteNl()          end the line, CR and LF both
;                 UartDrain()            block, bounded, until the last
;                                        STOP BIT has left the wire - not
;                                        until the FIFO is empty, which is
;                                        one character earlier and is the
;                                        difference between a clean last
;                                        line and a truncated one before a
;                                        reset
;                 UartDropped()          bytes UartWrite gave up on. The
;                                        one number that separates "the
;                                        program printed nothing" from
;                                        "the port ate it"
;                 UartReadReady()        1 if a byte is waiting; never
;                                        blocks
;                 UartRead()             one byte, BLOCKING FOR EVER, on
;                                        purpose: it is the line editor's
;                                        read and waiting for a human is
;                                        the whole job
;                 UartReadWait(spins)    one byte or -1. The right call
;                                        inside a transfer, where a dead
;                                        wire must leave a live prompt
;
;              THE MESSAGE BRACKET, which is why this is a seam and not a
;              convenience. Output that is FOR the operator but is not the
;              answer to the command they typed must never reach the
;              command line. The decision is made at the ONE BYTE every
;              console shares, not sentence by sentence in thirty callers:
;                 UartMessageSetDrain(*fn)   who empties the ring. The
;                                        drain MUST NOT print - it runs
;                                        inside UartWrite - and is
;                                        re-entry guarded anyway, because
;                                        "must not" is not a mechanism
;                 UartMessageBegin() / UartMessageEnd()    the bracket. It
;                                        NESTS: a service calling another
;                                        service is one context, not two.
;                                        Closing the outermost hands what
;                                        was written to the drain
;                 UartMessageAbandon()   leave every bracket at once. The
;                                        fatal path only - A FAULT IS NOT
;                                        A MESSAGE, and one taken inside a
;                                        background service must still be
;                                        printed
;                 UartMessageDepth() / UartMessageActive()
;                 UartMessageGet()       one diverted byte, -1 when empty
;                 UartMessageLost() / UartMessageBytes()
;
;              NOBODY REGISTERED, NOTHING DIVERTED. With no drain the
;              bracket is counted and ignored and every byte goes where it
;              always went, so a diagnostic that has no message log does
;              not lose its output into a ring nobody reads.
;
;              THREE OF THE NAMES ARE NOT OURS TO CHOOSE. Print and PrintN
;              are COMPILER KEYWORDS on these targets. The router picks a
;              formatter from the argument's TYPE and calls one of three
;              procedures BY NAME, LOWERCASED - str_print_at, printdec,
;              printnl - and it does not choose the transport. Whichever
;              file defines those three owns where every Print in the
;              program goes.
;                 str_print_at(p)        walk a NUL-terminated string at a
;                                        raw address
;                 PrintDec(v)            a signed 64-bit integer. THE MOST
;                                        NEGATIVE VALUE HAS NO POSITIVE
;                                        COUNTERPART, so take the digits
;                                        from the negative side and print
;                                        the sign first rather than
;                                        negating
;                 PrintNl()              end the line
;
;              RENAMING ONE OF THE THREE DOES NOT PRODUCE A LINK ERROR. It
;              produces a front-end message about a missing string-output
;              file, which is a confusing thing to be told about a file
;              you are looking at. A SECOND COPY of them in another file
;              is a duplicate-procedure error, and that error is the RIGHT
;              outcome - a program has one Print destination at a time -
;              but it points at the wrong file.
;
;              Implemented today by RaspberryPi4/Lib/uart.pi4,
;              RaspberryPi3/Lib/pl011_console.pi3 and, over the firmware's
;              text protocols with no UART at all,
;              ArduinoQ/Board/qcon_q.unoq. THAT LAST ONE IS THE PROOF THAT
;              THIS IS A SEAM AND NOT A DRIVER: the same names, the same
;              contracts, and not one register in common.
;
;              THE AUX TAP IS PART OF THIS and is what makes everything
;              the monitor prints reach a network peer:
;                 UartAuxMirror(on) / UartAuxSetFlush(*fn) / UartAuxGet()
;                 UartAuxBulk(on) / UartAuxOn() / UartAuxFillHigh()
;              A board with no network console supplies them as no-ops.
;
;    THE TIME  (NO CAPABILITY)  A MONOTONIC COUNT, AND WHAT IT COUNTS IN.
;              Every board has a clock, so there is nothing that could be
;              absent and no flag. Declared 2026-09-18 after twenty-odd
;              private copies of the same generic-timer read had begun to
;              diverge.
;
;                 Ticks()                the raw monotonic count. NEVER
;                                        GOES BACKWARDS and never negative
;                                        - a reader that cannot get a
;                                        coherent value answers 0, because
;                                        a clock that is monotonic almost
;                                        always is not a monotonic clock
;                 TickHz()               how many of those are a second
;                 TimerInit()            1 when the counter is usable. A
;                                        board with nothing to prepare
;                                        answers 1 and says so in a
;                                        comment rather than leaving an
;                                        empty procedure
;                 Micros()               microseconds since the counter's
;                                        origin, 0 IF THE CLOCK IS
;                                        STOPPED. A stopped clock is
;                                        reported as a stopped clock, not
;                                        as a plausible small number
;                 millis()               milliseconds, same rule
;                 delayMicroseconds(us)  wait. 1 if the wait completed, 0
;                                        if it gave up because the counter
;                                        was not advancing. A ZERO OR
;                                        NEGATIVE ARGUMENT RETURNS 0
;                 delay(ms)              1 for a zero or negative argument
;                                        and otherwise delayMicroseconds
;                                        of a thousand times it
;                 HwMicros()              the same microseconds, under the
;                                        name Anvil/Hal/abi.pbi asks a
;                                        board for on behalf of a payload
;
;              delay(0) ANSWERS 1 AND delayMicroseconds(0) ANSWERS 0. That
;              reads like a defect and it is a CONTRACT: both names are
;              called across the fleet and the pair behaves this way on
;              every part that has them. Fix it in every implementation or
;              in none.
;
;              THE DIVIDE GOES ON THE TICKS, NOT ON THE PRODUCT. ticks *
;              1000000 overflows a signed 64-bit value in about fifteen
;              minutes on a 19.2 MHz counter - long enough for a bench
;              session to look fine and a vehicle not to.
;
;              THE SOURCE IS THE BOARD'S BUSINESS. Where an AArch64
;              generic timer exists, CNTPCT_EL0 and CNTFRQ_EL0 are the
;              expected source and the frequency is READ rather than
;              baked, because it is 54 MHz on one part in this tree and
;              19.2 MHz on another. Where a part has a plain 1 MHz
;              counter, as the BCM2837 does, the conversions vanish and
;              TickHz() is a constant that happens to be true. A DRIVER
;              IS FREE TO READ THOSE REGISTERS ITSELF rather than owe a
;              timer file anything - RaspberryPi4/Lib/emmc.pi4 does,
;              deliberately, and says so - but it may not invent a second
;              set of these names.
;
;    THE CONSOLE BYTES  (NO CAPABILITY)  THE MONITOR'S OWN TEXT, IN AND
;              OUT. Twenty-five names every board supplies and no shared
...
;
;    HwMemory* (NO CAPABILITY - see below)  HOW MUCH MEMORY THIS BOARD
;              HAS. One number, in BYTES, and where it came from:
;                 HwMemoryBytes()        the TOTAL DRAM fitted to this
;                                        board, in bytes, or -1 when the
;                                        board honestly cannot know.
;                                        NOT the free space, not what a
;                                        payload may have, and not what
;                                        the monitor has mapped: the
;                                        memory the machine is built
;                                        with. Asked once and remembered
;                                        - a board does not grow memory
;                                        between two commands, and the
;                                        first caller is usually a guard
;                                        standing in front of a command
;                                        somebody typed.
;                 HwMemoryWhere()        the ADDRESS of a short lowercase
;                                        fragment saying where that
;                                        number came from ("the board
;                                        revision code the firmware
;                                        reports"), printed with
;                                        UartWriteStr exactly as
;                                        HwAddrReason() is. When the
;                                        answer is -1 this says WHY, in
;                                        the same shape, so the reason is
;                                        never missing. NEVER 0.
;
;              -1 IS AN ANSWER AND IT IS NOT A FAILURE. A board that
;              cannot ask its firmware, or that reads a revision code in
;              a scheme it does not decode, has to say so: the whole
;              point of the pair is that "four gigabytes" and "nobody
;              here knows" are different sentences, and a board that
;              guessed would be printing the second one in the words of
;              the first. 0 is not used for it - 0 is a number of bytes,
;              and a caller that forgot to test would read "no memory at
;              all" as a fact rather than as a gap.
;
;              WHY IT IS BYTES AND NOT MEGABYTES. Every caller of this
;              is comparing it against an ADDRESS - the address-safety
;              seam asks whether a range the operator typed is above the
;              memory this board has - and a unit that has to be
;              multiplied at each call site is a unit that will one day
;              be multiplied at all but one of them.
;
;              IT HAS NO #CAP_ FLAG, for the same reason HwAddr* has
;              none: every board has memory, so there is nothing that
;              could be absent. What differs is what the board KNOWS
;              about it, and that is an answer, not a capability.
;
;              Pi 4: RaspberryPi4/Board/hw_mem.pi4, which decodes the
;              MEMSIZE field of the firmware's board revision code. An
;              old-style revision code answers -1.
;              UNO Q: ArduinoQ/Board/hw_mem_q.unoq, which answers -1
;              today and says why - the firmware is UEFI, the total is
;              in the UEFI memory map, and this image does not walk it.
;
...
;                 HwUsbReadBlocks(lba,n,*b) n 512-byte blocks in as few
;                                        device commands as the driver
;                                        allows, into *b at any address;
;                                        1 or 0
...
;                 HwUsbHostLinkSpeed()/LinkWidth()  the controller's own
;                                        bus link: PCIe speed code (1 = 2.5,
;                                        2 = 5.0 GT/s) and lane count, -1 if
;                                        the board has no such link
;                 HwUsbHostLinkSsc()     1 spread-spectrum clocking on, 0 off,
;                                        -1 not applicable
;                 HwUsbHostFirmware()    the controller firmware version, -1
;                                        if the board cannot say
;              and the RAW LINK REGISTERS behind that one-sentence summary,
;              which `usb link` prints. A flag saying "spread-spectrum
;              clocking on" cannot be audited: it reads the same whether
;              the driver turned the feature on, found it on, or only
;              believes it. So the board publishes the words themselves,
;              and the core prints rows it is handed without learning what
;              any of them mean:
;                 HwUsbLinkRegCount()    registers this board can show, 0
;                                        if its host sits on no such link -
;                                        which is a fact, not a failure
;                 HwUsbLinkRegName(r)    the register's name (a pointer)
;                 HwUsbLinkRegValue(r)   its raw word, or -1 if nothing
;                                        answered - NEVER 0 for that case
;                 HwUsbLinkRegNote(r)    a sentence (a pointer) saying what
;                                        the drivers that run this part in
;                                        service leave in that register, so
;                                        the reader compares rather than
;                                        admires
;                 HwUsbLinkFieldCount(r) named fields inside it
;                 HwUsbLinkFieldName(r,f)  the field's name and bit range
;                                        (a pointer)
;                 HwUsbLinkFieldValue(r,f) its value, taken out of the same
;                                        word the row printed, -1 when the
;                                        register could not be read
;              and THE WAIT LEDGER, which `usb timing` prints. The USB
;              stack keeps one row per place it waits - not per event - with
;              how many times it waited there, how long that cost in total,
;              the worst single visit, and when the first and last visit
;              happened relative to the first wait of the boot. It exists
;              because the question "the configuration half of the boot
;              takes thirteen seconds, where?" had no answer on the board:
;              the boot trail's ring holds 64 entries and a boot writes 161,
;              so the record of the part worth reading is overwritten by the
;              part that follows it. A fixed set of rows cannot wrap.
;              The core prints rows it is handed and learns nothing about
;              any controller:
;                 HwUsbWaitCount()       rows this board publishes, 0 if it
;                                        keeps no ledger - a fact, not a
;                                        failure
;                 HwUsbWaitName(i)       the site's name (a pointer)
;                 HwUsbWaitVisits(i)     how many times it was waited at
;                 HwUsbWaitTotalUs(i)    microseconds spent there in total
;                 HwUsbWaitWorstUs(i)    the worst single visit
;                 HwUsbWaitFirstUs(i)    when it was first reached, and
;                 HwUsbWaitLastUs(i)     when it was last left, both in
;                                        microseconds since the ledger's
;                                        own origin
;                 HwUsbWaitBytes()       bytes the bulk row moved, so the
;                                        board can state its own throughput
;                                        without a host stopwatch in it
;                 HwUsbWaitLedgerReset() zero every row, to measure ONE
;                                        operation instead of a whole boot
;              and WHAT THE STICK ADVERTISED, which is not the same as what
;              it is being driven as. A device can offer a faster transport
;              than the one in use, and until it is printed nobody at the
;              prompt can tell a slow stick from a fast one being driven
;              slowly:
;                 HwUsbStorageUas()      1 when the device advertises USB
;                                        Attached SCSI beside Bulk-Only
;                 HwUsbStorageConfigLen()  wTotalLength of its configuration
;                                        descriptor, as the device said
;                 HwUsbStorageProtocol() the bInterfaceProtocol actually in
;                                        use, -1 when none was taken
;                 HwUsbBurstOut() / HwUsbBurstIn()  bMaxBurst as actually
;                                        configured on each bulk endpoint,
;                                        -1 when there is none
;              and WHICH DEVICE the listing is about. `usb devices` decodes
;              descriptors, but none of that decoding is a seam: it is USB,
;              it is identical on every bus, and it lives once in
;              Anvil/Bus/usb_core.pbi with the store it fills. What the
;              BOARD still owns is choosing the device and naming its own
;              drivers:
;                 HwUsbDescLoad(i)       select device i and have the USB
;                                        core fetch it; 1, or 0 with the
;                                        core's store cleared - never half
;                 HwUsbDescLoaded()      which index is loaded, -1 for none
;                 HwUsbDevRootPort(i)    the root port it came in on
;                 HwUsbDevParent(i)      the slot of the hub above it, 0 on
;                                        a root port
;                 HwUsbDevIfaceDriver(i, n)  which driver holds interface n
;                                        (a pointer): the BOARD names its
;                                        own drivers, the core never does
;                 HwUsbBindCore()        hand the shared USB core this
;                                        board's two control-transfer
;                                        procedures, before anything
;                                        enumerates
;                 HwUsbPortLinkState(p)  PORTSC link state (0 = U0), -1 none
;                 HwUsbPortPowerTimeouts(p) PORTPMSC: U1 timeout low byte, U2
;                                        next byte, -1 none
...
;              and the enumeration walk's witness, which `usb tree` prints
;              when nothing was addressed or a port stopped short (a board
;              with no such walk answers zero rows and nothing prints):
;                 HwUsbWalkCount()       rows the walk recorded
;                 HwUsbWalkPort(i) / WalkIsHub(i)   where row i stopped
;                 HwUsbWalkPortsc(i)     the raw PORTSC word at that point
;                 HwUsbWalkStep(i)       how far the port got (board-defined)
;                 HwUsbWalkStepText(s)   that step as a sentence (pointer)
;                 HwUsbWalkReached(i)    1 if the port was addressed
;                 HwUsbWalkErr(i) / WalkComp(i) / WalkSlot(i)
;                                        the driver's error code, the last
;                                        completion code, the slot if any
;                 HwUsbWalkErrText(c)    the driver's sentence for code c
;                 HwUsbWalkUsbSts() / WalkUsbCmd()
;                                        USBSTS and USBCMD after the walk
; ======================================================================
...
;
;  ABSENT means the board KNOWS there is nothing at this address: not a
;  gated block that might come up, not a corner nobody has read, but a
;  span of the address map with no memory and no device behind it. It
;  was added on 2026-09-17, when a board first learned how much memory
;  it has, and it exists because neither of the other two would have
;  told the truth. UNCLOCKED says "the processor will stop on this and
;  the board needs its power removing", which on a part that reads an
;  empty address as a value is a frightening lie. UNKNOWN says "THIS
;  REFUSAL IS IGNORANCE, NOT KNOWLEDGE" in those words, and this refusal
;  is exactly knowledge - the board asked its firmware how much memory
;  is fitted and the address is above it. Three answers could not carry
;  a fourth meaning without one of them becoming false, and a seam whose
;  vocabulary is nearly right is how a monitor starts lying by tone.
;
;  AN ABSENT ANSWER IS STILL ONLY A REFUSAL. `force` overrides it like
;  any other, because a debugger that cannot be pointed at an address
;  its board says is empty is a debugger that cannot prove the board
;  wrong.
;
...
#HW_ADDR_ABSENT    = 3   ; the board knows there is nothing here at all

; ----------------------------------------------------------------------
;  THE ANSWER A BOARD GIVES WHEN IT DOES NOT KNOW ITS OWN MEMORY SIZE.
;
;  -1 rather than 0: see the HwMemory* contract above. It is named here,
;  beside the vocabulary the same seam feeds, so that a board backend and
;  a core caller cannot each pick their own sentinel.
; ----------------------------------------------------------------------
#HW_MEM_UNKNOWN    = -1  ; HwMemoryBytes() has no honest number to give
...
#HW_FILE_BADNAME   = 4   ; the name is not one this board's medium can
                         ; express (on the Pi 4, a FAT partition takes a
                         ; root-directory 8.3 short name; exFAT a path)
...
#HW_FILE_UNSUPPORTED = 8 ; this board's medium cannot do that operation
                         ; at all (a directory change over a service
                         ; that only opens files by name)
#HW_FILE_UNNAMED   = 9   ; THE FILESYSTEM SAID NO AND NAMED NOTHING. A
                         ; backend maps its own layer's code into the list
                         ; above on a failure; if that code comes back as
                         ; "no error", the two disagree and this is what is
                         ; reported. It is never the medium's answer - it
                         ; is a defect in the layer that refused, and it is
                         ; given its own code so that it cannot be printed
                         ; as "nothing went wrong". Found 2026-09-17, when
                         ; a refused mkdir on build 174 said exactly that.
...
Procedure.i RequireCap(cap.i, *name, *reason)
  ...
  UartWriteStr("!! the ")
  UartWriteStr(*name)
  UartWriteStr(" command is not available on this board, because")
  UartWriteNl()
  UartWriteStr("   ")
  UartWriteStr(*reason)
  UartWriteStr(".")
  UartWriteNl()
  UartWriteStr("   Nothing was done. Anvil is one core built for many boards, so a")
  UartWriteNl()
  UartWriteStr("   command that needs hardware this board does not have refuses in a")
  UartWriteNl()
  UartWriteStr("   whole sentence rather than pretending or failing quietly. This is")
  UartWriteNl()
  UartWriteStr("   not a fault. Type info to see what this board is, and help for the")
  UartWriteNl()
  UartWriteStr("   commands that do work here.")
  UartWriteNl()
  ProcedureReturn 0
EndProcedure
