...
; ----------------------------------------------------------------------
;  16 IS NEW AT ABI 1.3 AND IT IS NOT A HARDWARE QUESTION.
;
;  #SVCCAP_PARALLEL asks whether this monitor will run one payload
;  procedure on more than one core at a time. Every id above it is
;  answered from a board's compile-time #CAP_*, and so is this one - but
;  what the board is declaring is that it reserves a stack for each
;  secondary core and carries the resident job owner that uses them, not
;  that it has a peripheral.
;
;  A PAYLOAD SHOULD NOT NEED THIS ID. SvcParallelCores() answers the same
;  question with a number it can divide work by, and a monitor too old to
;  have the group answers by not having the slot. The id exists so the
;  capability is nameable in the one vocabulary that spans both, the way
;  every other group is nameable.
; ----------------------------------------------------------------------
#SVCCAP_PARALLEL = 16
#SVCCAP_MAX      = 16     ; anything above this answers 0, not an error
