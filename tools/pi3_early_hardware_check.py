...
def run(a64,data,mode,el=2):
    ...
    out=[base.u64(cpu,base.OUT+n*8) for n in range(8)]
    ...
    # Pi3UartReadReady answers the receive-FIFO-empty flag and takes
    # nothing: it is asked BEFORE the read above, and that read still
    # returns its byte, which is what proves the non-consumption.
    assert out[7]==(0 if mode=='rxempty' else 1),('readready',mode,out)
