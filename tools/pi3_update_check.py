...
def volume():
    ...
    struct.pack_into('<II',mbr,454,2048,66581);mbr[510:]=b'\x55\xaa';disk[0]=bytes(mbr)
    v=bytearray(512);v[:3]=b'\xeb\x58\x90';v[3:11]=b'ANVIL   '
    struct.pack_into('<H',v,11,512);v[13]=1;struct.pack_into('<H',v,14,32);v[16]=2
    v[21]=0xf8;struct.pack_into('<I',v,28,2048);struct.pack_into('<I',v,32,66581)
    ...
    for off in range(0,len(old),512):disk[DATA+FIRST[0]-2+off//512]=old[off:off+512]
    disk[DATA+FIRST[2]-2]=record(0,1,old)
    return disk,old
    ...
    with tempfile.TemporaryDirectory(prefix='pi3-update-') as tmp:
        ...
        class Rig:
            def __init__(self,disk,cut=None,tear=False):
                ...
                self.reads=0;self.read_fail_at=None;self.read_lbas=[];self.sha_calls=0
            ...
            def call(self,name,*argv):
                ...
                hooks={base.LOAD+sym[n]:n for n in ['pi3sdreadblock','pi3sdwriteblock','sha256begin','sha256update','sha256end','sha256of']}
                for step in range(5000000):
                    ...
                    if hook:
                        ...
                        if hook=='pi3sdreadblock':
                            ...
                            if self.reads==self.read_fail_at:result=0
        ...
        # confirmed fallback. Read #4 is TRIED readback; #5 is second verify.
        for failed in (1,4,5):
            disk,old=volume();new=container(bytes([0xa5])*1076)
            for off in range(0,len(new),512):disk[DATA+FIRST[1]-2+off//512]=new[off:off+512].ljust(512,b'\0')
            disk[DATA+FIRST[3]-2]=record(1,2,new,1)
            rig=Rig(disk);rig.mount();rig.read_fail_at=rig.reads+failed
