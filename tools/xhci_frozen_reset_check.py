...
def run(data,symbols,mode):
 ...
 # THE STEP BUDGET IS A GUARD AGAINST AN INFINITE LOOP, NOT A MEASUREMENT,
 # and it had to grow when the SETUP reset took Linux's ten-second budget
 # (XHCI_RESET_LONG_USEC) instead of U-Boot's 250 ms. The modelled counter
 # advances one millisecond per read, so a stuck reset now spins ten
 # thousand times round a loop that was spinning two hundred and fifty.
...
def main():
 ...
 for blob in (data,):
  ...
  # AND THE SETUP RESET WAITS THE LONG BUDGET, not the short one. Ten
  # seconds at 54 MHz is 540,000,000 ticks; the 250 ms figure would stop at
  # 13,500,000, so this separates the two constants rather than accepting
  # either. A reset declared dead forty times too early is the defect.
  _,state=run(blob,symbols,'reset_stuck');assert state['ticks']>=540000000,('short reset budget',state['ticks'])
 try:
