...
Anvil listens on UDP port 5555 as soon as it has an address, on WHICHEVER
interface holds it - the Ethernet cable or the Wi-Fi radio, wired
preferred when both. Whoever sends it a datagram becomes the peer: from
then on everything the monitor prints comes back as UDP, and every line
...
the address of the interface on THIS machine that heard it. That last
part is the point on a bench with more than one path to the board: an
answer on 192.168.137.x is the board on the Ethernet cable, an answer on
192.168.1.x is the same board over the access point, and knowing which
is which is the difference between a working session and half an hour.
...
The board prints its address and which link the console is on in its own
boot log, and `net` on the board says the same thing at any time.
"""
import socket
import struct
import sys
