...
#
# The reader already decodes with errors="replace" (tools/anvil.py), so a
# bad byte on the wire becomes U+FFFD and nothing throws. WRITING it is
# the half that threw: on a console whose code page cannot represent
# U+FFFD, print() raises UnicodeEncodeError. On 2026-09-07 that killed
# this tool between the save and the reset - the image was already on the
# stick and correct, and the only thing on the screen was a traceback.
#
# A tool whose job is to relay a device's output must not be able to die
# on the output. Both streams are reconfigured to replace what they
# cannot encode. Guarded because .reconfigure arrived in 3.7 and a
# redirected stream may not have it; portable to both platforms, which is
# house rule 14.
# ----------------------------------------------------------------------
