...
def main():
    ...
    def run(case,command=None,extension=True):
        ...
        # The background network bring-up claims the controller during boot, so
        # by the time anyone can type, these are what the command actually meets.
        if case=='taken':put('p3usb_host_state',(1<<64)-1);put('p3usb_native_power',1)
        ...
        if case in ('context','unconfirmed','repeat','taken','quarantine-command') or (command is not None and (command!='usbdiag' or not extension)):
        ...
        if case=='taken':
            assert 'already owned' in text,text
            assert 'GSNPSID=' in text,'the live controller was not reported: '+text
            assert c.load(sym['global_p3diag_used'],8)==0,'a refusal spent the one-shot'
            assert dma==0,'a refusal touched the DMA enable'
        if case=='deadline':assert calls.count('pi3usbenumerationstep')<151
        return text
    for case in ('ok','context','unconfirmed','repeat','init','reset','enum','deadline','owned','taken'):run(case)
    ...
    assert 'already owned' in run('taken','usbdiag')
    print('PASS 16 emitted candidate orchestration/command cases; MMIO and enumeration mocked at documented boundaries')
