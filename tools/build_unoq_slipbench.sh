#!/bin/bash
# ======================================================================
#  build_unoq_slipbench.sh - build the Arduino UNO Q SLIP BENCH RUN and
#                            wrap it as an AArch64 UEFI application.
#
#  Run from the repo root:  bash tools/build_unoq_slipbench.sh
#
...
#  ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqslipbench.unoq before deploying it.
#
...
usage() {
  echo "usage: bash tools/build_unoq_slipbench.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqslipbench.unoq"
IMG="$OUTDIR/anvilqslipbench.img"
EFI="$OUTDIR/anvilqslipbench.efi"
