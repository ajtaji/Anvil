...
def manifest_hash(root):
 ...
 return h.hexdigest()
def freeze_build_inputs(compiler):
 ...
 try:
  ...
  if not runtime_keywords.is_file():raise RuntimeError(f'IR V3D42 runtime keywords missing: {runtime_keywords}')
  shutil.copy2(runtime_keywords,snapshot/'keywords.def')
...
def main():
 ...
 compiler_hash=sha256_file(compiler);snapshot,snapshot_hash=freeze_build_inputs(compiler)
