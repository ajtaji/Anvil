...
READLINE_OUTSIDE = {
    ...
    "InputErrText", "AnvilPromptAdmitUart", "UartReadReady", "UartRead", "InputFeedByte",
    "KeyboardChar", "NetConsoleGetc", "HwPwmKind", "AnvilSchedulerIdleIfSafe",
    ...
    for depth, code in bracket_walk(lines[:loop_end + 1], "ReadLine"):
        for name in calls_in(code):
            if name in READLINE_SERVICES:
                ...
                found.add(name)
    scheduler = body_of(parse, "Procedure AnvilPromptServiceOne()")
    for depth, code in bracket_walk(scheduler, "AnvilPromptServiceOne"):
        for name in calls_in(code):
            if name in READLINE_SERVICES:
                if depth == 0:
                    raise GateFailure(f"AnvilPromptServiceOne: {name}() runs outside the message context")
                found.add(name)
    missing = [n for n in READLINE_SERVICES if n not in found]
    if missing:
        raise GateFailure(f"prompt scheduler services not found in the loop/dispatcher: {', '.join(missing)}")
