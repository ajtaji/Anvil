#!/usr/bin/env python3
"""pmfshot_wifi.py - take a screenshot of the Pi 4 over Wi-Fi.

The wireless twin of pmfshot.py. Speaks Anvil's `pr` command - the
...
Stop-and-wait, exactly the discipline the OTA update uses, so a busy
screen (58k+ runs) survives the air instead of losing one datagram in
the middle and wasting the whole capture. A CRC32 over the entire run
