...
transcript that can be trusted.
    ...
    python tools/pi4_simd_silicon.py --payload <image> \
...
ordinary `--entry-returns` images that go into the low payload window
and give control back - at the address the board names, which since
2026-09-08 is above whatever the monitor turned out to be rather than
the constant 0x400000 this line used to carry.  So this tool never issues
`save` and never resets, and the resident monitor the next run meets is
the one it left.
...
# WHERE THE PAYLOAD GOES - ASKED, NOT CARRIED. --addr defaulted to
# 0x400000 here until 2026-09-08. The monitor has no size cap since then,
# so its extent is the size of its own image and a payload sent to a
# constant can land on the program that is receiving it - which does not
# crash the board, it leaves it answering and unable to obey. See WHERE
# THE BOARD WANTS A FILE PUT in tools/anvil.py for the one reader of the
# board's answer in this tree.
...
def main() -> int:
    ...
    # --addr IS AN OVERRIDE, NOT A DEFAULT. Left alone the board
    # says where the payload goes, and it is the only thing in the
    # arrangement that knows how big the monitor turned out to be.
    ...
    try:
        ...
        # WHERE IT GOES, ASKED BEFORE THE DEADMAN IS ARMED AND BEFORE
        # ANY BYTE MOVES: a board that will not say where a payload may
        # go is a run that must not start.
        if args.addr is None:
            out = b.ask("map", "stage a file at", cap=15, idle=4)
            rec = read_map(out)
            if rec is None:
                print(stage_help("the payload"))
                return 1
            addr = rec.stage
            print("staging at %08X, which is where this board said to "
                  "put it" % addr)
        else:
            addr = int(args.addr, 16)
        tr.append("  staging at %08X%s" % (addr, " (--addr)"
                                          if args.addr else " (the board)"))
