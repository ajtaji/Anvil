...
LOWERER = ROOT / "Anvil" / "Graphics" / "Vulkan" / "vk_ir_v3d42.pi4"
...
def execute(a64, image: pathlib.Path):
    ...
    progress_slot = load(OUT + REPORT_WORDS * 8, 8)
    progress_value = load(OUT + progress_slot * 8, 8) if progress_slot < REPORT_WORDS else 0
    raise SystemExit(
...
def grade(cpu, rc) -> Grader:
    ...
    g.need("[odd viewport] V3D X scaling is exactly 1152 fine units",
           slot(637) & 0xFFFFFFFF, f32_bits(9 * 128))
    g.need("[odd viewport] V3D Y scaling is exactly 1408 fine units",
           slot(638) & 0xFFFFFFFF, f32_bits(11 * 128))
    ...
    # --- the refusals ---
    g.need("a pipeline with a non-source-over blend equation is refused",
           slot(42), ERR_UNSUPPORTED)
    g.need("the exact source-over pipeline retains its normalized blend mode",
           slot(656), 1)
    g.need("the source-over pipeline's closed draw reaches the backend",
           slot(657), 1)
    g.need("the disabled pipeline retains disabled blending", slot(658), 0)
    g.need("the disabled pipeline's closed draw resets Neon's blend state",
           slot(659), 0)
    ...
    g.need_bytes("[odd viewport] coordinate uniforms carry exact odd half extents",
                 blob(cpu, dead_base + OFF_UNIF_CS, (10 + 4) * 4),
                 expected_vertex_uniforms(10, 9 * 128, 11 * 128))
    g.need_bytes("[odd viewport] vertex uniforms carry exact odd half extents",
    ...
    g.need("stale-tail command buffer ends", slot(680), 0)
    g.need("stale tail refuses the complete draw list", slot(681), ERR_STATE)
    g.need("stale tail refuses before submit counter mutation", slot(682), 0)
    g.need("stale tail refuses before completion counter mutation", slot(683), 0)
    g.need("stale tail refuses before backend invocation", slot(684), 0)
    g.need("stale tail executes no partial draw", slot(685), 0)
    g.need("stale tail leaves no outstanding flight", slot(686), 0)
    g.need("stale tail leaves its fence unsignalled", slot(687), 1)
    g.need("stale tail retains no list dependency", slot(688), 0)
    g.need("stale tail publishes no closed flight list", slot(689), 0)
    g.need("stale tail commits no flight ledger", slot(690), 0)
    ...
    g.need("held draw retains its exact render pass", slot(663), 1)
    g.need("held draw refuses pipeline destruction", slot(664), ERR_STATE)
    g.need("held draw refuses render-pass destruction", slot(665), ERR_STATE)
    g.need("held draw refuses descriptor-pool destruction", slot(666), ERR_STATE)
    g.need("completion releases pipeline/set/framebuffer/render-pass ledger", slot(667), 0)
    g.need("three-draw command buffer ends", slot(668), 0)
    g.need("three draws are linked in the command buffer", slot(669), 3)
    g.need("closed three-draw list submits", slot(670), 0)
    g.need("one backend transaction consumes all three draws", slot(671), 3)
    g.need("closed list remains outstanding while backend is held", slot(672), 1)
    g.need("backend receives one three-record list", slot(673), 3)
    g.need("per-draw vertex ranges survive later state", slot(674), 1)
    g.need("per-draw binding bases survive later binds", slot(675), 1)
    g.need("per-draw pipelines and binding counts survive later binds", slot(676), 1)
    g.need("aliases retain additively across the complete list", slot(677), 1)
    g.need("held three-draw list completes", slot(678), 0)
    g.need("three-draw completion releases the exact captured ledger", slot(679), 0)
    return g
...
MUTANTS = (
    ...
    ("the V3D clipper scaling loses its required 1/256-pixel units",
     ...
     "Procedure.i AnvilVkV3dViewportHalfWidthBits(pipe.i)\n  ProcedureReturn avkqF32FromInt(AnvilVkV3dViewportHalfWidthFine(pipe) / 256)\nEndProcedure\n\nProcedure.i AnvilVkV3dViewportHalfHeightBits(pipe.i)\n  ProcedureReturn avkqF32FromInt(AnvilVkV3dViewportHalfHeightFine(pipe) / 256)\nEndProcedure\n"),
...
LOWERER_MUTANTS = (
    ("the uniform stream's viewport scale is half what it should be",
     ...
     "  halfW = *t\\viewportWidth * 64 : halfH = *t\\viewportHeight * 64\n"),
)
...
DRAW_LIST_COMMAND_MUTANTS = (
    ("submission skips whole-list preflight before acquiring flight state",
     "  If avkCbDrawCount[c] > 0\n    job = avkDrawListPreflight(c)\n    If job <> #ANVIL_VK_OK : ProcedureReturn job : EndIf\n  EndIf\n",
     "  If avkCbDrawCount[c] > 99\n    job = avkDrawListPreflight(c)\n    If job <> #ANVIL_VK_OK : ProcedureReturn job : EndIf\n  EndIf\n"),
    ("submission commits no captured draw-list retain ledger",
     "  avkFlightRetain(c)\n  avkDrawListRetain(c)\n  avkCopyBufferRetain(c)\n",
     "  avkFlightRetain(c)\n  avkCopyBufferRetain(c)\n"),
)
COMMAND_MUTANTS += DRAW_LIST_COMMAND_MUTANTS
...
PIPELINE_MUTANTS = (
    ...
    ("the accepted source-over equation is normalized as disabled",
     "  PokeI(*outMode, #ANVIL_VK_BLEND_SRC_OVER)\n",
     "  PokeI(*outMode, #ANVIL_VK_BLEND_DISABLED)\n"),
    ("the closed draw drops its pipeline's blend mode",
     "  avkDrawRecord\\blendMode = avkPipeBlendMode[p]\n",
     "  avkDrawRecord\\blendMode = #ANVIL_VK_BLEND_DISABLED\n"),
...
DRAW_LIST_PIPELINE_MUTANTS = (
    ("vkCmdDraw silently drops every draw after the first",
     "  If avkRecordedDrawAppend(c, firstVertex, vertexCount, vx, vy, vw, vh, sx, sy, sw, sh, 0, 0, 0) < 1\n",
     "  If avkCbDrawCount[c] > 0\n    ProcedureReturn\n  EndIf\n  If avkRecordedDrawAppend(c, firstVertex, vertexCount, vx, vy, vw, vh, sx, sy, sw, sh, 0, 0, 0) < 1\n"),
    ("whole-list preflight validates only the first recorded draw",
     "  slot = avkCbDrawHead[c]\n  draw = 0\n  While draw < count\n",
     "  slot = avkCbDrawHead[c]\n  draw = 0\n  While draw < 1\n"),
    ("closed draw records use the command buffer's later pipeline bind",
     "    avkFlightDraw[draw]\\pipeline = p\n",
     "    avkFlightDraw[draw]\\pipeline = avkPipeSlot(avkCbPipe[c])\n"),
    ("closed draw records use the command buffer's later vertex offset",
     "      avkFlightBinding[idx]\\base = avkHeapBase + avkMemOffset[bindingMem] + avkBufMemOffset[b] + avkRecordedDraw[slot]\\vertexOffset[k]\n",
     "      avkFlightBinding[idx]\\base = avkHeapBase + avkMemOffset[bindingMem] + avkBufMemOffset[b] + avkCbVtxOffset[(c * #ANVIL_VK_MAX_BINDINGS) + k]\n"),
    ("closed backend transaction publishes one draw regardless of list length",
     "  avkFlightDrawList\\drawCount = count\n",
     "  avkFlightDrawList\\drawCount = 1\n"),
    ("aliased pipelines retain only once across the draw list",
     "    s = avkFlightPipe[draw] : avkPipeInFlight[s] = avkPipeInFlight[s] + 1\n",
     "    s = avkFlightPipe[draw] : avkPipeInFlight[s] = 1\n"),
    ("aliased framebuffers retain only once across the draw list",
     "    s = avkFlightFb[draw] : avkFbInFlight[s] = avkFbInFlight[s] + 1\n",
     "    s = avkFlightFb[draw] : avkFbInFlight[s] = 1\n"),
    ("aliased vertex buffers retain only once across the draw list",
     "        avkBufInFlight[s] = avkBufInFlight[s] + 1\n        avkMemInFlight[avkFlightVertexMem[idx]] = avkMemInFlight[avkFlightVertexMem[idx]] + 1\n",
     "        avkBufInFlight[s] = 1\n        avkMemInFlight[avkFlightVertexMem[idx]] = 1\n"),
    ("draw completion releases the command buffer's later pipeline instead of the captured ledger",
     "    s = avkFlightPipe[draw] : If avkPipeInFlight[s] > 0 : avkPipeInFlight[s] = avkPipeInFlight[s] - 1 : EndIf\n",
     "    s = avkPipeSlot(avkCbPipe[avkFlightCb]) : If avkPipeInFlight[s] > 0 : avkPipeInFlight[s] = avkPipeInFlight[s] - 1 : EndIf\n"),
    ("a submitted pipeline may be destroyed while its draw is held",
     "  If avkPipeInFlight[s] <> 0\n    avkFault(#ANVIL_VK_ERR_STATE, \"vkDestroyPipeline was called while a submitted command buffer still uses this pipeline",
     "  If avkPipeInFlight[s] < 0\n    avkFault(#ANVIL_VK_ERR_STATE, \"vkDestroyPipeline was called while a submitted command buffer still uses this pipeline"),
    ("a submitted render pass may be destroyed while its draw is held",
     "  If avkRpInFlight[s] <> 0\n    avkFault(#ANVIL_VK_ERR_STATE, \"vkDestroyRenderPass was called while one or more submitted draws still use this render pass",
     "  If avkRpInFlight[s] < 0\n    avkFault(#ANVIL_VK_ERR_STATE, \"vkDestroyRenderPass was called while one or more submitted draws still use this render pass"),
    ("a submitted framebuffer may be destroyed while its draw is held",
     "  If avkFbInFlight[s] <> 0\n    avkFault(#ANVIL_VK_ERR_STATE, \"vkDestroyFramebuffer was called while a submitted command buffer still uses this framebuffer",
     "  If avkFbInFlight[s] < 0\n    avkFault(#ANVIL_VK_ERR_STATE, \"vkDestroyFramebuffer was called while a submitted command buffer still uses this framebuffer"),
)
PIPELINE_MUTANTS += DRAW_LIST_PIPELINE_MUTANTS
...
TEST_BACKEND_MUTANTS = (
    ("the test backend observes every draw as blend-disabled",
     "  avkTbLastBlendMode = *d\\blendMode\n",
     "  avkTbLastBlendMode = #ANVIL_VK_BLEND_DISABLED\n"),
...
def main() -> int:
    ...
    if args.mutate:
        anchor_errors = []
        for path, mutants in ((EMITTER, MUTANTS), (LOWERER, LOWERER_MUTANTS),
                              (PIPELINE, PIPELINE_MUTANTS),
    ...
    if args.mutate_name:
        ...
        total = (len(MUTANTS) + len(LOWERER_MUTANTS) + len(PIPELINE_MUTANTS) + len(COMMAND_MUTANTS)
