...
def read_memory(con: board_run.NetConsole, addr: int, length: int) -> bytes:
    """`readback <addr> <len>`, verified, as bytes.

    THE LENGTH AND THE CRC-32 ARE BOTH CHECKED before a word is judged -
    tools/anvil_readback.py refuses a stream that came back short or
    changed, asks once more, and refuses in a sentence if the second read
    is no better. Nothing is judged from a partial read.
    """
    try:
        data, _stats = read_range(con, addr, length, seconds=30.0,
                                  progress=None)
    except ReadbackError as error:
        raise SystemExit(
            "the verdict block could not be read back: %s Nothing was judged."
            % error)
    return data
