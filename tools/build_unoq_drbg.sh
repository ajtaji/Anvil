#!/bin/bash
# ======================================================================
#  build_unoq_drbg.sh - build the Arduino UNO Q HMAC-DRBG PROOF (the
#                       known-answer test, plus a real seed taken from
#                       the firmware's EFI_RNG_PROTOCOL) and wrap it as
#                       an AArch64 UEFI application.
#
#  Run from the repo root:  bash tools/build_unoq_drbg.sh
#
#  Same contract as build_unoq_efiprobe.sh: -t unoq (the shared A64
#  backend), linked into the Q's confirmed-free low-bank DRAM,
#  --entry-returns so x0/x1 (ImageHandle/SystemTable) survive into
#  Main().
#
#  LIKE THE PROTOCOL WALK AND UNLIKE THE GPU PROBE, THIS CANNOT HANG THE
#  BOARD. It writes no register and reads no peripheral; every address it
#  dereferences was handed to it by the firmware. See the header of
#  ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqdrbg.unoq.
# ======================================================================
set -e
...
usage() {
  echo "usage: bash tools/build_unoq_drbg.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqdrbg.unoq"
IMG="$OUTDIR/anvilqdrbg.img"
EFI="$OUTDIR/anvilqdrbg.efi"

LOAD=0x70000000
STACK=0x68000000

echo ">> compiling the UNO Q HMAC-DRBG proof (shared A64 backend, -t unoq)"
