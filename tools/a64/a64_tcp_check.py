...
(OP_RESET, OP_SETMAC, OP_SETIP, OP_ARPSET, OP_TXMAX, OP_USE,
 ...
 OP_RECV_RELEASE, OP_UNLISTEN) = range(1, 36)
...
#H_CLOCK  = $06F00000
#H_CAPMAX = 16384
; Every connection in this harness is on the wired interface.
...
; One frame, once, on the wired interface; then #LINK_RX_NONE.
Procedure.i LinkPumpAllNet(ms.i)
  Define n.i
  If stubRxLen <= 0
    ProcedureReturn #LINK_RX_NONE
  EndIf
  n = NetInput(#H_KIND, stubRxPtr, stubRxLen)
  stubRxLen = 0
  stubLastIn = n
  ProcedureReturn n
EndProcedure
...
RADIO_FRAME = 1136          # the radio's largest 802.3 frame
...
def build_script() -> tuple[Script, dict, dict]:
    ...
    a("txmax", OP_TXMAX, "LinkTxMaxOn() becomes 1136 - the radio's number",
      a=RADIO_FRAME)
    a("connect2", OP_CONNECT, "TcpConnect(peer, 7000) over the radio",
      a=PEER_IP, b=7000)
