...
--------------------------------------------------
  ...
  H. A STOP-AND-WAIT TRANSFER'S BLOCKING WAIT IS SPENT ON THE WIRE ITS
     ANSWERS ARRIVE ON, AND EVERY OTHER WIRE IS STILL LISTENED TO.
     LinkPumpAllNet is FAIR: it polls everything at zero and then blocks
     its whole slice on a rotating cursor, which is right for a listener
     with no expectation and exactly wrong for one that has just sent
     something.  On a board holding the cable AND a Wi-Fi lease - both
     usable, so the blocking branch is live - the cursor put the 200 ms
     wait on the RADIO on the turn after every TFTP block while the
     acknowledgement lay unread in the wired ring: about 2.5 KB/s, and a
     1,812,000-byte put that ran past the ten-minute ceiling instead of
     finishing (2026-09-16).  LinkPumpAllNetPrefer takes the caller's
     routed interface and grades four things here: the reply is collected
     in ONE turn, not a slice later; no deadline is spent on the other
     interface; an ARP request arriving on that other interface during
     the transfer is still answered and its sender learned; and the fair
     pump's rotation cursor is neither read nor moved, so tcp.pi4 and the
     idle prompt are unchanged.  The first negative control below is not
     an invented mistake - it is the code the board was running.
...
-------------------------------------------------------------
  ...
  * TIMING, AS A DURATION.  Nothing here blocks: a fixture with a frame
    ready returns it at once, and one with nothing returns at once too.
    A driver that ignored its deadline would not be caught here.  WHICH
    WIRE A DEADLINE WAS SPENT ON *is* graded - the fixture counts the
    waiting calls and the milliseconds asked for, per interface, which is
    what case H below is about - but how long a slice really takes is the
    board's and is measured on it.
...
(OP_RESET, OP_HAS, OP_TXMAX_SET, OP_SELECT, OP_USEKIND, OP_FORGET,
 ...
 OP_WIRE, OP_WIREMODE, OP_NETADD, OP_PUMPPREF, OP_PUMPALL, OP_WAITS,
 OP_WAITCLR, OP_CURSOR) = range(1, 33)
...
;
; EACH FRAME IS ON AN INTERFACE AND ARRIVES AT A MOMENT, because a pump
; that chooses WHERE to wait cannot be graded by a queue that hands its
; next frame to whoever asks. fxQKind is the interface the frame is
; actually on, or 0 for "whichever kind asks first" - which is what every
; case written before 2026-09-16 wants and gets by leaving the argument
; out. fxQArm is the number of HwLinkRecv calls that must have happened
; before the frame is on the wire at all, so a reply can be made to land
; AFTER a pump has already begun its turn: that is the whole of the TFTP
; rhythm, where the host answers about a millisecond after the block
; went out and the zero-wait sweep has already run and found nothing.
Global Dim fxQ.a[#FX_QUEUE_MAX * #FX_FRAME_MAX]
Global Dim fxQLen.i[#FX_QUEUE_MAX]
Global Dim fxQKind.i[#FX_QUEUE_MAX]
Global Dim fxQArm.i[#FX_QUEUE_MAX]
Global Dim fxQTaken.i[#FX_QUEUE_MAX]
Global fxQTail.i
Global fxRecvTotal.i

; WHERE THE WAITING WENT, PER INTERFACE. The gate's own header says this
; fixture does not block, and it still does not: it RECORDS the deadline
; it was handed instead of sleeping through it, because what is being
; graded is which wire a slice was spent on and not how long a run takes.
; fxWaitMs only counts deadlines that came back EMPTY - a wait that was
; answered cost the answer's round trip on a real link, not its ceiling.
#FX_KINDS = 8
Global Dim fxRecvCalls.i[#FX_KINDS]
Global Dim fxWaitCalls.i[#FX_KINDS]
Global Dim fxWaitMs.i[#FX_KINDS]
...
Procedure.i HwLinkRecv(kind.i, buf.i, max.i, ms.i)
  ...
  ; COUNTED BEFORE ANYTHING ELSE, including a kind this fixture board has
  ; not got. A slice spent waiting on an interface that cannot answer is
  ; exactly the defect this accounting exists to catch, and a counter
  ; that skipped it would hide the worst case.
  fxRecvTotal = fxRecvTotal + 1
  If kind >= 0 And kind < #FX_KINDS
    fxRecvCalls[kind] = fxRecvCalls[kind] + 1
    If ms > 0
      fxWaitCalls[kind] = fxWaitCalls[kind] + 1
    EndIf
  EndIf
  ...
  ; The first frame that is on THIS interface and has arrived by now.
  ; Untagged frames belong to whichever kind asks, which is what the
  ; one-interface cases rely on.
  slot = -1
  i = 0
  While i < fxQTail
    If fxQTaken[i] = 0
      If fxQKind[i] = 0 Or fxQKind[i] = kind
        If fxQArm[i] <= fxRecvTotal
          slot = i
          Break
        EndIf
      EndIf
  ...
  If slot < 0
    If ms > 0 And kind >= 0 And kind < #FX_KINDS
      fxWaitMs[kind] = fxWaitMs[kind] + ms
    EndIf
    ProcedureReturn 0
  EndIf
...
Procedure fxReset()
  ...
  fxQTail = 0
  fxRecvTotal = 0
  For i = 0 To #FX_QUEUE_MAX - 1
    fxQKind[i] = 0
    fxQArm[i] = 0
    fxQTaken[i] = 0
    fxQLen[i] = 0
  Next
  For i = 0 To #FX_KINDS - 1
    fxRecvCalls[i] = 0
    fxWaitCalls[i] = 0
    fxWaitMs[i] = 0
  Next
...
Procedure.i Main()
  ...
  Repeat
    ...
    ElseIf op = 7
      ...
      e2 = LinkTxMax()
    ElseIf op = 8
      ; queue one frame for the fixture link. a is the interface it is
      ; ON (0 = whichever kind asks first) and b is how many receive
      ; calls must pass before it is on the wire (0 = it already is).
      If fxQTail < #FX_QUEUE_MAX
        ...
        fxQLen[fxQTail] = ln
        fxQKind[fxQTail] = a
        ; b IS RELATIVE TO NOW. An absolute call number would make a case
        ; depend on how many receive calls every earlier case made.
        fxQArm[fxQTail] = fxRecvTotal + b
        fxQTaken[fxQTail] = 0
        fxQTail = fxQTail + 1
        r = 1
      EndIf
    ...
    ElseIf op = 26
      ...
      r = 1
    ElseIf op = 27
      ; A SECOND INTERFACE, WITHOUT WIPING THE FIRST. op 16 calls
      ; NetInit() so that a case starts from a known stack; a board with
      ; a cable AND a lease cannot be built out of two of those.
      NetSetMac(d, blob)
      r = NetSetIPv4(d, a, b, c)
      e1 = NetError()
      e2 = NetIfUsableCount()
    ElseIf op = 28
      r = LinkPumpAllNetPrefer(a, b)
      e1 = LinkRxLen()
      e2 = LinkRxFrames()
    ElseIf op = 29
      r = LinkPumpAllNet(a)
    ...
    ElseIf op = 31
      ...
      r = 1
    ElseIf op = 32
      ; THE FAIRNESS CURSOR, PLACED AND THEN READ BACK. Where it stands
      ; decides which wire LinkPumpAllNet blocks on, so a case about
      ; waiting cannot be left to wherever an earlier case left it. The
      ; same op reads it, because "the preferring pump does not move it"
      ; is a fact about this variable and nothing else can see it.
      If a > 0
        gLinkPumpNext = a
      EndIf
      r = gLinkPumpNext
...
# The RADIO's own segment, so a board can hold a cable AND a lease at
# once the way the Pi 4 does - 169.254/192.168.137 on the wire and a
# 192.168.1 lease on the radio. Two interfaces is the situation the
# preferring pump exists for and it cannot be built out of one subnet.
WIFI_IP = ip4(192, 168, 9, 20)
WIFI_GW = ip4(192, 168, 9, 1)
WIFI_PEER_IP = ip4(192, 168, 9, 9)
WIFI_PEER_MAC = bytes([0x02, 0x77, 0x66, 0x55, 0x44, 0x03])
...
def build_script() -> tuple[Script, dict]:
    ...
    # =================================================================
    #  H. A STOP-AND-WAIT TRANSFER'S BLOCKING WAIT BELONGS ON THE WIRE
    #     ITS ANSWERS ARRIVE ON - LinkPumpAllNetPrefer, 2026-09-16
    # =================================================================
    #  THE SITUATION IS THE BOARD'S OWN, on the day this was measured: a
    #  cable with an address and a Wi-Fi lease, both usable at once. That
    #  makes LinkPumpAllNet's blocking branch live, and the branch spends
    #  a whole #ETH_SLICE_MS on whichever wire the fairness cursor names.
    #
    #  WHY THE REPLY IS ARMED FOR THE THIRD RECEIVE CALL. That is the
    #  TFTP rhythm, in this fixture's units: the block goes out, the pump
    #  begins its turn and sweeps both interfaces at zero wait
    #  (calls 1 and 2) before the host has answered, and the
    #  acknowledgement lands about a millisecond later - while the pump
    #  is in its blocking call (call 3). A reply that was already queued
    #  would be taken by the sweep and would prove nothing about waiting.
    #
    #  THE CURSOR IS PLACED ON THE RADIO ON PURPOSE. On the board it gets
    #  there by itself and stays: a sweep that takes a frame off the
    #  cable sets the cursor to WIRED+1, and the idle slice that follows
    #  advances it past the unusable kinds and back, so the fair pump's
    #  wait lands on the radio after EVERY block rather than half of
    #  them. Placing it here makes that deterministic instead of lucky,
    #  the same way RaspberryPi4/Tests/net_xfer_pump_ownership_emitted_gate.pi4
    #  does for the theft it grades.
    a("reset_tp1", OP_RESET)
    a("cfg_tp1", OP_NETCFG, OUR_IP, MASK, GW_IP, HW_LINK_WIRED, blob=OUR_MAC)
    a("add_tp1", OP_NETADD, WIFI_IP, MASK, WIFI_GW, HW_LINK_WIFI,
      blob=WIFI_MAC)
    a("bind_tp1", OP_BIND, UDP_PORT, HW_LINK_WIRED)
    a("sel_tp1", OP_SELECT, 1, 1, 1, 1)
    a("cursor_tp1", OP_CURSOR, HW_LINK_WIFI)
    a("waitclr_tp1", OP_WAITCLR)
    a("feed_tp1", OP_FEED, HW_LINK_WIRED, 3, blob=udp_to_us(4000, UDP_PORT))
    a("pump_tp1", OP_PUMPPREF, HW_LINK_WIRED, 200)
    a("waits_tp1_wired", OP_WAITS, HW_LINK_WIRED)
    a("waits_tp1_wifi", OP_WAITS, HW_LINK_WIFI)
    a("cursor_tp1_after", OP_CURSOR, 0)

    # THE OTHER WIRE IS STILL LISTENED TO. A transfer owns the WAITING,
    # not the board's ears: an ARP request arriving on the radio while a
    # transfer is running on the cable must still be answered, or a board
    # goes unreachable on one interface for the length of every transfer
    # - which is the defect NetIfClaimRx was written to end.
    a("reset_tp2", OP_RESET)
    a("cfg_tp2", OP_NETCFG, OUR_IP, MASK, GW_IP, HW_LINK_WIRED, blob=OUR_MAC)
    a("add_tp2", OP_NETADD, WIFI_IP, MASK, WIFI_GW, HW_LINK_WIFI,
      blob=WIFI_MAC)
    a("sel_tp2", OP_SELECT, 1, 1, 1, 1)
    a("cursor_tp2", OP_CURSOR, HW_LINK_WIRED)
    a("arp_tp2_before", OP_ARPCOUNT, HW_LINK_WIFI)
    a("waitclr_tp2", OP_WAITCLR)
    a("feed_tp2", OP_FEED, HW_LINK_WIFI, 0,
      blob=eth(BCAST, WIFI_PEER_MAC, ET_ARP,
               arp(1, WIFI_PEER_MAC, WIFI_PEER_IP, b"\x00" * 6, WIFI_IP)))
    a("pump_tp2", OP_PUMPPREF, HW_LINK_WIRED, 200)
    a("arp_tp2_after", OP_ARPCOUNT, HW_LINK_WIFI)
    a("waits_tp2_wired", OP_WAITS, HW_LINK_WIRED)
    a("waits_tp2_wifi", OP_WAITS, HW_LINK_WIFI)
    a("cursor_tp2_after", OP_CURSOR, 0)

    # THE FAIRNESS CURSOR IS NOT THIS PROCEDURE'S TO MOVE, and the proof
    # is behavioural as well as direct: after a preferring pump has run,
    # the FAIR pump must still block exactly where its own cursor said,
    # because tcp.pi4 and the idle prompt ride that one and a transfer
    # must not reach into their state.
    a("reset_tp3", OP_RESET)
    a("cfg_tp3", OP_NETCFG, OUR_IP, MASK, GW_IP, HW_LINK_WIRED, blob=OUR_MAC)
    a("add_tp3", OP_NETADD, WIFI_IP, MASK, WIFI_GW, HW_LINK_WIFI,
      blob=WIFI_MAC)
    a("sel_tp3", OP_SELECT, 1, 1, 1, 1)
    a("cursor_tp3", OP_CURSOR, HW_LINK_WIFI)
    a("waitclr_tp3", OP_WAITCLR)
    a("pump_tp3_idle", OP_PUMPPREF, HW_LINK_WIRED, 200)
    a("waits_tp3_wired", OP_WAITS, HW_LINK_WIRED)
    a("waits_tp3_wifi", OP_WAITS, HW_LINK_WIFI)
    a("cursor_tp3_after", OP_CURSOR, 0)
    a("waitclr_tp3b", OP_WAITCLR)
    a("pump_tp3_fair", OP_PUMPALL, 200)
    a("waits_tp3b_wired", OP_WAITS, HW_LINK_WIRED)
    a("waits_tp3b_wifi", OP_WAITS, HW_LINK_WIFI)
    a("cursor_tp3b_after", OP_CURSOR, 0)

    # A CALLER WITH NOTHING TO PREFER GETS THE FAIR PUMP, not a slice
    # spent on nothing. #HW_LINK_NONE is what XferKind() answers before a
    # transfer has routed.
    a("reset_tp4", OP_RESET)
    a("cfg_tp4", OP_NETCFG, OUR_IP, MASK, GW_IP, HW_LINK_WIRED, blob=OUR_MAC)
    a("add_tp4", OP_NETADD, WIFI_IP, MASK, WIFI_GW, HW_LINK_WIFI,
      blob=WIFI_MAC)
    a("sel_tp4", OP_SELECT, 1, 1, 1, 1)
    a("cursor_tp4", OP_CURSOR, HW_LINK_WIFI)
    a("waitclr_tp4", OP_WAITCLR)
    a("pump_tp4_none", OP_PUMPPREF, HW_LINK_NONE, 200)
    a("waits_tp4_wired", OP_WAITS, HW_LINK_WIRED)
    a("waits_tp4_wifi", OP_WAITS, HW_LINK_WIFI)
...
def check(res: list[Result], at: dict, fails: list[str]) -> None:
    ...
    # ---- H. the slice is spent on the wire the answers arrive on ------
    # The situation has to EXIST before anything about it can be graded.
    # With one usable interface both pumps collapse to the same call and
    # the case would pass while proving nothing.
    expect(R("add_tp1").r == 1,
           "the radio's address was refused (NetError %d), so the "
           "two-interface board this case is about was never built"
           % R("add_tp1").e1, fails)
    expect(R("add_tp1").e2 == 2,
           "%d interface(s) are usable and this case needs 2 - a cable "
           "with an address AND a radio with a lease, which is what the "
           "board held when this was measured. With one, every pump in "
           "this file behaves identically" % R("add_tp1").e2, fails)

    expect(R("pump_tp1").r == NET_IN_UDP,
           "the transfer's reply landed on the CABLE a moment after the "
           "pump began its turn, and the pump answered %d instead of "
           "#NET_IN_UDP (%d). It spent the slice somewhere the answer "
           "could not arrive. THIS IS THE DEFECT MEASURED ON 2026-09-16: "
           "one whole #ETH_SLICE_MS per 512-byte block, about 2.5 KB/s, "
           "and a 1.8 MB put that ran past the ten-minute ceiling"
           % (R("pump_tp1").r, NET_IN_UDP), fails)
    expect(R("waits_tp1_wifi").e1 == 0,
           "the pump made %d blocking receive call(s) on the RADIO while "
           "a transfer routed over the cable was waiting for its "
           "acknowledgement. A stop-and-wait transfer's blocking wait "
           "belongs on the wire its answers arrive on"
           % R("waits_tp1_wifi").e1, fails)
    expect(R("waits_tp1_wifi").r == 0,
           "%d ms of deadline was spent waiting on the RADIO for an "
           "answer that was always going to arrive on the cable. Every "
           "millisecond of that is a block that does not go out"
           % R("waits_tp1_wifi").r, fails)
    expect(R("waits_tp1_wired").e1 == 1,
           "the pump made %d blocking receive call(s) on the CABLE and it "
           "must make exactly one: the sweep found nothing anywhere, so "
           "the slice goes on the interface this transfer is routed over"
           % R("waits_tp1_wired").e1, fails)
    expect(R("waits_tp1_wired").r == 0,
           "the wait on the cable burned %d ms of its deadline. A "
           "deadline is a ceiling, not a cost - the receive call returns "
           "the moment the frame lands, which is what makes a block cost "
           "a host round trip instead of a slice"
           % R("waits_tp1_wired").r, fails)
    expect(R("waits_tp1_wired").e2 >= 2,
           "the pump made %d receive call(s) on the cable; it must poll "
           "it once at zero wait and then wait on it"
           % R("waits_tp1_wired").e2, fails)
    expect(R("cursor_tp1_after").r == HW_LINK_WIFI,
           "the fairness cursor moved to %d across a preferring pump and "
           "it was placed at %d. That cursor belongs to LinkPumpAllNet, "
           "which tcp.pi4 and the idle prompt ride; a transfer that moved "
           "it would change where THEY wait"
           % (R("cursor_tp1_after").r, HW_LINK_WIFI), fails)

    # ---- H. and the other wire is still listened to -------------------
    expect(R("arp_tp2_after").r - R("arp_tp2_before").r == 1,
           "the radio's ARP cache went from %d to %d while a transfer was "
           "running on the cable; an ARP request that arrived on the "
           "radio had to be answered and its sender learned. Learning the "
           "sender is a side effect only NetInput has, so an unchanged "
           "count is proof the frame never reached the IP layer - and a "
           "board that does not answer ARP cannot be spoken to at all"
           % (R("arp_tp2_before").r, R("arp_tp2_after").r), fails)
    expect(R("pump_tp2").r != LINK_RX_NONE,
           "the pump answered #LINK_RX_NONE with a frame waiting on the "
           "radio. The zero-wait sweep covers EVERY usable interface; "
           "only the waiting is owned by one", fails)
    expect(R("waits_tp2_wired").e1 == 0 and R("waits_tp2_wifi").e1 == 0,
           "the pump blocked (%d call(s) on the cable, %d on the radio) "
           "although a frame was already assembled. Anything already "
           "waiting anywhere is taken before any deadline is spent"
           % (R("waits_tp2_wired").e1, R("waits_tp2_wifi").e1), fails)
    expect(R("cursor_tp2_after").r == HW_LINK_WIRED,
           "the fairness cursor moved to %d when the preferring pump took "
           "a frame off the radio; it was placed at %d and is not this "
           "procedure's to move"
           % (R("cursor_tp2_after").r, HW_LINK_WIRED), fails)

    # ---- H. an idle slice, and the fair pump left alone ---------------
    expect(R("pump_tp3_idle").r == LINK_RX_NONE,
           "an idle preferring pump answered %d and must answer "
           "#LINK_RX_NONE" % R("pump_tp3_idle").r, fails)
    expect(R("waits_tp3_wired").r == 200,
           "an idle slice spent %d ms on the cable and the caller asked "
           "for 200. An idle slice still goes on the preferred wire: that "
           "is where the next answer will arrive"
           % R("waits_tp3_wired").r, fails)
    expect(R("waits_tp3_wifi").r == 0,
           "an idle slice spent %d ms on the radio although the caller "
           "named the cable" % R("waits_tp3_wifi").r, fails)
    expect(R("cursor_tp3_after").r == HW_LINK_WIFI,
           "the fairness cursor reads %d after an IDLE preferring pump "
           "and was placed at %d" % (R("cursor_tp3_after").r, HW_LINK_WIFI),
           fails)
    expect(R("waits_tp3b_wifi").r == 200 and R("waits_tp3b_wired").r == 0,
           "the FAIR pump, run straight after a preferring one, spent %d "
           "ms on the radio and %d on the cable; its own cursor said the "
           "radio. LinkPumpAllNet's callers must be unchanged by the "
           "existence of the preferring pump"
           % (R("waits_tp3b_wifi").r, R("waits_tp3b_wired").r), fails)
    expect(R("cursor_tp3b_after").r == HW_LINK_WIFI + 1,
           "the fair pump left the cursor at %d after an idle slice; it "
           "rotates even across an idle one, so from %d it must reach %d. "
           "The fair pump still rotates; the preferring one never does"
           % (R("cursor_tp3b_after").r, HW_LINK_WIFI, HW_LINK_WIFI + 1),
           fails)

    # ---- H. nothing to prefer falls back to the fair pump -------------
    expect(R("waits_tp4_wifi").r == 200 and R("waits_tp4_wired").r == 0,
           "with #HW_LINK_NONE preferred the slice went %d ms to the "
           "radio and %d ms to the cable; the fair pump's cursor said the "
           "radio, so that is what a caller with nothing to prefer must "
           "get. A transfer that has not routed yet has no wire to wait "
           "on, and a slice spent on nothing at all is a transfer that "
           "makes no progress"
           % (R("waits_tp4_wifi").r, R("waits_tp4_wired").r), fails)
...
MUTATIONS = [
    ...
    # ---- the preferring pump. The first of these is not an invented
    # ---- mistake: it is the code the board was running on 2026-09-16.
    ("THE SHIPPED DEFECT - the transfer's pump goes back to the fair "
     "rotation, so the slice parks on the radio after every block",
     "  If prefer < 1 Or prefer >= #NET_IF_KINDS\n"
     "    ProcedureReturn LinkPumpAllNet(ms)\n  EndIf\n"
     "  If NetIfUsable(prefer) = 0\n"
     "    ProcedureReturn LinkPumpAllNet(ms)\n  EndIf",
     "  ProcedureReturn LinkPumpAllNet(ms)"),
    ("the preferring sweep asks only the preferred interface, so the "
     "other wire goes deaf for the length of a transfer",
     "    kind = 1 + ((prefer - 1 + off) % (#NET_IF_KINDS - 1))",
     "    kind = prefer"),
    ("the slice goes on the fairness cursor even though the caller "
     "named the wire its answers arrive on",
     "    ProcedureReturn LinkPumpKind(prefer, ms)",
     "    ProcedureReturn LinkPumpKind(gLinkPumpNext, ms)"),
    ("the preferring pump moves the fairness cursor, so a transfer "
     "changes where tcp.pi4 and the idle prompt wait",
     "      n = LinkPumpKind(kind, 0)\n      If n <> #LINK_RX_NONE\n"
     "        ProcedureReturn n\n      EndIf",
     "      n = LinkPumpKind(kind, 0)\n      If n <> #LINK_RX_NONE\n"
     "        gLinkPumpNext = kind + 1\n        ProcedureReturn n\n"
     "      EndIf"),
    ("the preferring pump never waits at all, so it spins instead of "
     "sleeping and a reply in flight is never collected",
     "  If ms > 0\n    ProcedureReturn LinkPumpKind(prefer, ms)\n  EndIf\n"
...
def main() -> int:
   ...
   * HOW LONG A SLICE TAKES. The fixture answers at once, so a driver
     that ignored its receive deadline would not be caught here. WHICH
     WIRE a deadline was spent on IS graded - that is case H, the
     transfer pump - but the seconds on the clock are the board's.
""")
    if args.mutate:
