...
set, the group bases and the table itself).
  ...
  G. THE GUARDED SETTINGS - the two names slots 65 and 67 refuse, tried in
     every spelling of their case, against the REAL settings store.
     Setting names are case-insensitive: the store folds one to lower case
     before it writes or reads it, so a guard that compares byte for byte
     is TIGHTER than the store it guards and the spelling it fails to
     recognise lands on the guarded setting anyway.  Each spelling must be
     refused with #SVC_EPERM AND leave the stored value untouched - a
     refusal that wrote anyway would pass on the return code alone - while
     names that only resemble the guarded ones stay writable.  It builds a
     small fixture out of abi.pbi's own procedure text rather than the
     monitor, because part D's monitor build MOVES THE SHARED BUILD NUMBER
     and this question does not need an image.

--only <part>
=============
One part, for when the whole gate cannot be run - notably when the shared
build number must not move, which part D always moves.  It prints what it
did and does NOT claim a green gate.
...
directory; the tree is never written) and requires each defect to be CAUGHT:
  ...
  guard-exact            the guarded-name comparison put back the way it
                         was before 2026-09-16 - byte for byte, no fold.
                         G red.
  store-no-fold          the store stops folding, so the guard and the
                         store agree again and every refusal is still
                         made - while two spellings become two settings.
                         G red, which is what stops G from proving only
                         that two copies of one mistake match.
...
# ---------------------------------------------------------------------
ABI_MAJOR = 1
# 3 on Anvil main: 1.1 filled core slots 14/15 (SvcSeamFill, SvcSeamGet),
# 1.2 filled console slot 13 (SvcScreenCapture), and 1.3 opened group 15 -
# parallel execution - at base 184.  THE FIRST THREE OF THOSE MOVED NO SLOT
# AND THE FOURTH MOVED NONE EITHER: the core group was full at 16 of 16, so
# the new block was APPENDED and entry_count rose rather than anything being
# renumbered.  That is still a minor bump - abi_version.pbi.
ABI_MINOR = 3
SLOT_COUNT = 192
HDR_BYTES = 16
...
INSTALLED = {
    ...
    # ABI 1.3: the four-core payload service.  Both arms of the CompilerIf
    # define all three, so these are installed on every board - a board
    # without the capability answers one core and #SVC_ENOCAP rather than
    # leaving the slots pointing at SvcUnimplemented.
    184: "SvcParallelCores", 185: "SvcParallelRun", 186: "SvcParallelStatus",
}
...
SVCCAPS = {
    ...
    "#SVCCAP_THERMAL": 15, "#SVCCAP_PARALLEL": 16, "#SVCCAP_MAX": 16,
}
...
def check_static() -> list[str]:
    ...
    names = {"core": "#SVC_BASE_CORE", "clock": "#SVC_BASE_CLOCK",
             ...
             "crypto": "#SVC_BASE_CRYPTO", "par": "#SVC_BASE_PAR"}
...
# ---------------------------------------------------------------------
#  G. THE GUARDED SETTINGS, FOLDED THE WAY THE STORE FOLDS
#
#  Slots 65 and 67 refuse the two settings the reboot-loop stop is built
#  on: a payload that could write or delete the failure counter could
#  switch the stop off, and the stop is what turns "the new firmware
#  hangs" from a board that needs a laptop into a board still answering.
#
#  SETTING NAMES ARE CASE-INSENSITIVE.  The store folds every name to
#  lower case before it writes one and before it looks one up, so
#  "BOOT.FAILS" and "boot.fails" are ONE setting holding ONE value.  A
#  guard that compares exactly is therefore TIGHTER than the store it
#  guards, which is the dangerous direction: the spelling it fails to
#  recognise is accepted and then folded onto the guarded setting anyway.
#  That was real - the refusal was one capital letter wide - and this
#  part is what makes it impossible to reintroduce quietly.
#
#  WHAT IS ASSERTED, per spelling: the service REFUSES with #SVC_EPERM,
#  the stored value is UNCHANGED afterwards, and no second key appeared.
#  The refusal alone is not enough evidence: a guard could refuse and
#  write, and the value is the only thing that says it did not.
#
#  AND THE OTHER DIRECTION, because a fold applied too widely is its own
#  defect: names that merely LOOK like the guarded ones - boot.fail,
#  boot.failsx, boot.file, xboot.fails - must still be accepted, and an
#  ordinary key must still be settable and deletable in any case.
#
#  THIS PART BUILDS A FIXTURE, NOT THE MONITOR, and that is deliberate
#  twice over.  The procedures under test are lifted VERBATIM out of
#  abi.pbi's own text, so --mutate reaches them, and they are compiled
#  against the REAL settings store rather than a double - the whole
#  question is whether the guard agrees with the store, and a stubbed
#  store could only ever agree with itself.  Building the monitor would
#  also move the shared build number (part D does, and says so), and
#  answering one question about a comparison is not a reason to supersede
#  an image somebody else has already built and not yet flashed.
# ---------------------------------------------------------------------
SETTINGS = ROOT / "Anvil" / "Core" / "settings.pbi"
GUARD_LOAD = 0x200000
GUARD_STEPS = 200_000_000

# The procedures lifted out of abi.pbi.  SvcKeyMatches is OPTIONAL: the
# fixed guard has no such procedure, and the mutant that puts an exact
# comparison back brings one with it.
GUARD_BODIES = ("SvcKeyFails", "SvcKeyMaxFails", "SvcKeyIsGuarded",
                "SvcSettingGet", "SvcSettingSet", "SvcSettingDel")
GUARD_BODIES_OPTIONAL = ("SvcKeyMatches",)

# A plain numeric #SVC_* constant, taken from abi.pbi rather than
# restated, because the fixture must refuse with the SAME number the
# board would.  Expressions are skipped on purpose: a gate that could
# evaluate one could evaluate a wrong one into the right answer.
SVC_CONST_RE = re.compile(r'^(#SVC_[A-Za-z0-9_]+)\s*=\s*(-?\$?[0-9A-Fa-f]+)\s*(?:;.*)?$')

# The two guarded names, spelled every way a payload might spell them.
# The canonical lower-case spelling is first in each list on purpose: the
# exact spelling must still be refused, and a fold that somehow stopped
# refusing it would otherwise look like a pass.
GUARDED_SPELLINGS = {
    "boot.fails": ["boot.fails", "BOOT.FAILS", "Boot.Fails", "bOoT.fAiLs",
                   "Boot.fails", "boot.FAILS"],
    "boot.maxfails": ["boot.maxfails", "BOOT.MAXFAILS", "Boot.MaxFails",
                      "boot.MAXFAILS", "BoOt.MaXfAiLs", "Boot.maxfails"],
}

# Names that are NOT the guarded ones and must stay writable.  A guard
# built on "does the name contain boot.fails" would eat boot.failsx and
# xboot.fails; one built on a prefix would eat boot.file.
NEAR_MISSES = ["boot.fail", "boot.failsx", "boot.file", "xboot.fails",
               "boot.maxfail", "BOOT.FILE"]

# The seed values, as the ASCII of their first byte - which is what the
# fixture reports, through the payload's own read service.
SEED_FAILS, SEED_MAX, SEED_MODE = ord("7"), ord("3"), ord("i")
ABSENT = -1


def guard_procedure(body: str, name: str, required: bool = True) -> str:
    starts = ("Procedure %s(" % name, "Procedure.i %s(" % name)
    lines = body.splitlines()
    first = next((i for i, l in enumerate(lines) if l.startswith(starts)), None)
    if first is None:
        if required:
            raise SystemExit("the guarded-key part cannot find %s in abi.pbi, so "
                             "there is nothing to test" % name)
        return ""
    last = next((i for i in range(first + 1, len(lines)) if lines[i] == "EndProcedure"), None)
    if last is None:
        raise SystemExit("%s in abi.pbi has no EndProcedure" % name)
    return "\n".join(lines[first:last + 1])


def guard_source(settings_include: str) -> tuple[str, list[tuple[str, int]]]:
    """The fixture, and what every check it makes must answer.

    The two are built side by side so that a case cannot be added without
    an expectation, and the program reports its OWN count of checks, so a
    case that silently stopped running is a length mismatch and not a
    pass."""
    abi = text(ABI)
    consts = [l for l in abi.splitlines() if SVC_CONST_RE.match(l.strip())]
    if not any(l.strip().startswith("#SVC_EPERM") for l in consts):
        raise SystemExit("abi.pbi no longer states #SVC_EPERM as a plain number; "
                         "the guarded-key part will not invent one")
    bodies = [guard_procedure(abi, n) for n in GUARD_BODIES]
    bodies += [guard_procedure(abi, n, required=False) for n in GUARD_BODIES_OPTIONAL]

    expect: list[tuple[str, int]] = []
    prog: list[str] = []

    def case(line: str, label: str, want: int) -> None:
        prog.append("  Chk(%s)" % line)
        expect.append((label, want))

    prog.append("Procedure Main()")
    prog.append("  SettingsReset()")
    prog.append('  SettingsSet("boot.fails", "7")')
    prog.append('  SettingsSet("boot.maxfails", "3")')
    prog.append('  SettingsSet("app.mode", "idle")')
    case("SettingsCount()", "the store was seeded with three keys", 3)
    case('Val1("boot.fails")', "the failure counter reads back", SEED_FAILS)
    case('Val1("boot.maxfails")', "and so does the limit", SEED_MAX)

    seed = {"boot.fails": SEED_FAILS, "boot.maxfails": SEED_MAX}
    for canonical, spellings in GUARDED_SPELLINGS.items():
        other = "boot.maxfails" if canonical == "boot.fails" else "boot.fails"
        for spelling in spellings:
            tag = "%s spelled %s" % (canonical, spelling)
            case('SvcSettingSet("%s", "0")' % spelling,
                 "setting " + tag + " is refused", SVC_ERRS["#SVC_EPERM"])
            case('Val1("%s")' % canonical,
                 "and " + canonical + " still holds its value", seed[canonical])
            case('Val1("%s")' % other,
                 "and " + other + " was not touched either", seed[other])
            case("SettingsCount()",
                 "and no key was added by " + spelling, 3)
            case('SvcSettingDel("%s")' % spelling,
                 "deleting " + tag + " is refused", SVC_ERRS["#SVC_EPERM"])
            case('Val1("%s")' % canonical,
                 "and " + canonical + " is still there", seed[canonical])
            case("SettingsCount()",
                 "and no key was removed by " + spelling, 3)

    # An ordinary key, in a case nobody typed, still works - and lands on
    # the SAME key, which is the store's half of the same fold.
    case('SvcSettingSet("App.Mode", "run")',
         "an unguarded key is accepted in mixed case", SVC_ERRS["#SVC_OK"])
    case('Val1("app.mode")', "and the write landed on the folded name", ord("r"))
    case('Val1("APP.MODE")', "and reads back through any spelling", ord("r"))
    case("SettingsCount()", "without creating a second key", 3)
    case('SvcSettingDel("APP.MODE")',
         "and an unguarded key deletes in mixed case", SVC_ERRS["#SVC_OK"])
    case('Val1("app.mode")', "and is gone", ABSENT)
    case("SettingsCount()", "leaving two keys", 2)

    # THE COUNT IS TRACKED BY FOLDED NAME, NOT BY HOW MANY NAMES WERE
    # TYPED, and that is the point of the last two entries: "boot.file"
    # and "BOOT.FILE" are one key, so the second write must NOT grow the
    # store.  An expectation that simply counted the writes reported this
    # as a failure the first time it ran, which is the store telling the
    # truth about itself.
    seen = {"boot.fails", "boot.maxfails"}
    for name in NEAR_MISSES:
        folded = name.lower()
        fresh = folded not in seen
        seen.add(folded)
        case('SvcSettingSet("%s", "1")' % name,
             "%s is not a guarded name and is accepted" % name,
             SVC_ERRS["#SVC_OK"])
        case('Val1("boot.fails")',
             "and writing %s did not disturb the counter" % name, SEED_FAILS)
        case('Val1("boot.maxfails")',
             "or the limit", SEED_MAX)
        case("SettingsCount()",
             ("and %s is a key of its own" % name) if fresh
             else ("and %s landed on the key already there" % name),
             len(seen))

    # THE LAST CHECK IS THE PROGRAM'S OWN COUNT.  If a case above stopped
    # running, the arrays are short and this disagrees - so a truncated
    # run cannot look like a clean one.
    prog.append("  Chk(nres)")
    expect.append(("the program counted its own checks", len(expect)))
    prog.append("EndProcedure")
    prog.append("")
    prog.append("Main()")

    src = "\n".join([
        "; GENERATED by tools/a64/a64_abi_check.py - not a tracked file and",
        "; never written into the tree.  The procedures below are abi.pbi's",
        "; own text, compiled against the real settings store.",
        ...
        "EndProcedure",
        "",
        "; The value as the payload itself would read it - through slot 64,",
        "; not by reaching into the store - so a refusal that wrote anyway",
        "; is visible on the same surface it was made on.",
        "Procedure.i Val1(*key)",
        "  Define n.i",
        "  n = SvcSettingGet(*key, @vbuf[0], #VBUF)",
        "  If n <= 0",
        "    ProcedureReturn -1",
        "  EndIf",
        "  ProcedureReturn PeekA(@vbuf[0])",
        "EndProcedure",
        "",
        "\n".join(prog),
        "",
...
def check_guarded_keys() -> list[str]:
    problems: list[str] = []
    # A DIRECTORY PER DISTINCT SET OF SOURCES, so two mutants cannot share
    # one build - which would let the second be judged on the first's image.
    key = abs(hash(tuple(sorted((str(p), t) for p, t in OVERRIDE.items())))) % (10 ** 12)
    work = CTX["work"] / ("guard_%d" % key)
    work.mkdir(parents=True, exist_ok=True)
    settings_src = materialise(SETTINGS, work / "src")
    include = (settings_src.resolve().as_posix()
               if settings_src != SETTINGS
               else SETTINGS.relative_to(ROOT).as_posix())
    src, expect = guard_source(include)
    fixture = work / "guardedkey_fixture.pi4"
    fixture.write_text(src, encoding="utf-8")
    out = work / "guardedkey.img"
    syms = compile_to(fixture, out, ["--load-addr", hex(GUARD_LOAD)])

    for needed in ("global_res", "global_nres", "_a64_end_trap"):
        if needed not in syms:
            fail(problems, "the fixture's symbol map has no %s, so nothing can be "
                           "read back from it" % needed)
            return problems

    cpu = A64(pc=GUARD_LOAD)
    for i, b in enumerate(out.read_bytes()):
        cpu.memory[GUARD_LOAD + i] = b
    trap = GUARD_LOAD + syms["_a64_end_trap"]
    for _ in range(GUARD_STEPS):
        if cpu.pc == trap:
            break
        cpu.step()
    else:
        fail(problems, "the guarded-key fixture never reached its end trap")
        return problems

    n = cpu.load(syms["global_nres"], 8)
    base = syms["global_res"]
    got = []
    for i in range(min(n, len(expect))):
        v = cpu.load(base + i * 8, 8)
        got.append(v - (1 << 64) if v >= (1 << 63) else v)
    if n != len(expect):
        fail(problems, "the fixture made %d checks and this file expects %d; a case "
                       "that stopped running is not a pass" % (n, len(expect)))
    for i, value in enumerate(got):
        label, want = expect[i]
        if value != want:
            fail(problems, "check %d (%s): got %d, expected %d" % (i, label, value, want))
    if not problems:
        print("     %d checks over %d spellings of the two guarded keys, "
              "%d near misses"
              % (len(expect),
                 sum(len(v) for v in GUARDED_SPELLINGS.values()),
                 len(NEAR_MISSES)))
    return problems
...
MUTANTS = {
    ...
    # THE DEFECT ITSELF, PUT BACK.  This is the guard exactly as it stood
    # before 2026-09-16: its own byte-for-byte comparison instead of the
    # store's answer.  It refuses the lower-case spelling and admits every
    # other one, which is the hole this part exists to keep shut.
    "guard-exact": (
        "Procedure.i SvcKeyIsGuarded(*key)\n"
        "  If SettingsKeySame(*key, SvcKeyFails()) <> 0\n"
        "    ProcedureReturn 1\n"
        "  EndIf\n"
        "  If SettingsKeySame(*key, SvcKeyMaxFails()) <> 0\n"
        "    ProcedureReturn 1\n"
        "  EndIf\n"
        "  ProcedureReturn 0\n"
        "EndProcedure",
    ...
    # THE OTHER END OF THE SAME ROPE.  If the store stopped folding, the
    # guard would agree with it again and every refusal above would still
    # be made - while two spellings became two settings and the counter
    # could be shadowed by a second key.  The part must notice that too,
    # or it is only testing that two copies of one mistake match.
    "store-no-fold": (
...
def mutate(only: str | None = None) -> tuple[int, int]:
    parts = {"static": check_static, "dynamic": check_dynamic,
             "version": check_version_gate, "guarded": check_guarded_keys}
    failures = 0
    ran = 0
    for name, (find, repl, which, path) in MUTANTS.items():
        if only and which != only:
            continue
        ran += 1
...
def main() -> int:
    ...
    ap.add_argument("--only", choices=("static", "hygiene", "container", "dynamic",
                                       "version", "reentrancy", "guarded"),
                    help="run ONE part (and, with --mutate, only the mutants that "
                         "part owns). The full gate is the default and is what a "
                         "green is claimed from; this exists because 'dynamic' "
                         "builds the whole monitor and therefore MOVES THE SHARED "
    ...
    with tempfile.TemporaryDirectory(prefix="anvil-abicheck-") as td:
        ...
        if args.mutate:
            ...
            if not ran:
                ...
                return 1
            if bad:
                print(f"RESULT: RED ({bad} of {ran} mutants not caught)")
                return 1
            print(f"RESULT: GREEN - all {ran} damaged ABIs were caught")
            return 0
        ...
        parts = (("A  static - the constants agree", check_static, "static"),
                 ...
                 ("D  dynamic - the built table is correct", check_dynamic, "dynamic"),
                 ("E  version gate - the payload refuses an older table",
                  check_version_gate, "version"),
                 ("F  re-entrancy - an interrupted slot keeps its own answer",
                  check_reentrancy_witness, "reentrancy"),
                 ("G  guarded keys - the guard folds the way the store folds",
                  check_guarded_keys, "guarded"))
        if args.only:
            parts = tuple(p for p in parts if p[2] == args.only)
        for title, fn, _name in parts:
            print("=" * 70)
            print(title)
            print("=" * 70)
            probs = fn()
    ...
    if total:
        ...
        return 1
    if args.only:
        print(f"RESULT: GREEN - part {args.only} only. THE OTHER PARTS DID NOT RUN, "
              "so this is not a green gate.")
        return 0
    ...
    print("  The two guarded settings are refused through the setting-set and")
    print("  setting-delete slots in every case-spelling of their names, with the")
    print("  stored value unchanged after each refusal, while names that merely")
    print("  resemble them are still accepted.")
    return 0
