...
Sections, in order:
  ...
  address    the address-safety seam on both boards, executed on both:
             the Q's table, and the Pi 4's top-of-memory refusal on each
             size a Pi 4 is sold with
  ...
  sendceiling  #CYW43_TXBUF through LinkTxMaxOn to TcpTxCap, executed
  netcon     where the network console lives and when it arms
  static     a stored address takes the lease's path
  netstats   `net stats` dispatches, and speaks for every interface
...
def put64(cpu: A64, addr: int, value: int) -> None:
    """Write one 64-bit word at an ABSOLUTE address.

    BSS symbols in the compiler's .sym are already absolute (read_sym says
    so), so nothing is added to them here. Used to plant a fact the board
    would have got from its firmware, so that the production decision can
    then be executed rather than argued about.
    """
    for i in range(8):
...
def section_layout(ctx: Ctx) -> None:
    ...
    ctx.case(e["MON_DATA_HI"] - e["MON_DATA_LO"] + 1 ==
             const(map_text, "MON_DATA_BYTES"),
             "the protected BSS region's endpoints do not match its declared size")
    ctx.case(ctx.call(cpu, "hwmonregionlo", 1) == e["MON_DATA_LO"],
             "HwMonRegionLo(1) does not protect the relocated monitor BSS")
    ctx.case(ctx.call(cpu, "hwmonregionhi", 1) == e["MON_DATA_HI"],
             "HwMonRegionHi(1) does not protect the full BSS reservation")
    bss_match = re.search(r"^\s*BssAddress\s+\$([0-9A-Fa-f]+)",
                          SOURCE.read_text(encoding="utf-8", errors="replace"),
                          re.M)
    ctx.case(bss_match is not None and int(bss_match.group(1), 16) == lo,
             "board.pi4 BssAddress does not match the emitted BSS start at "
             "$%08X" % lo)
    ctx.case(e["MON_DATA_HI"] < e["PAY1_LO"],
             "the monitor BSS reservation reaches outside low DRAM bank 0")
    eth_text = read("RaspberryPi4/Board/eth_globals.pi4")
    eth_lo = const(eth_text, "ETH_RX_BASE")
    eth_hi = eth_lo + const(eth_text, "ETH_RX_BYTES") - 1
    reserved = (
        ("low payload window", e["PAY0_LO"], e["PAY0_HI"]),
        ("payload stack corridor", 0x07F00000, 0x080FFFFF),
        ("Ethernet receive ring", eth_lo, eth_hi),
        ("DSI framebuffer", "MON_FB_LO", "MON_FB_HI"),
        ("driver module region", "MOD_REGION_LO", "MOD_REGION_HI"),
        ("V3D arena", "V3DCON_ARENA", None),
        ("screenshot capture", "MON_SHOT_LO", "MON_SHOT_HI"),
        ("Vulkan heap", "MON_VK_HEAP_LO", "MON_VK_HEAP_HI"),
        ("high payload window", e["PAY1_LO"], e["PAY1_HI"]),
    )
    v3d_text = read("RaspberryPi4/Board/v3d_console.pi4")
    for label, lo_name, hi_name in reserved:
        if isinstance(lo_name, int):
            other_lo, other_hi = lo_name, hi_name
        elif label == "V3D arena":
            other_lo = const(v3d_text, lo_name)
            other_hi = other_lo + const(v3d_text, "V3DCON_ARENA_BYTES") - 1
        else:
            other_lo = const(map_text, lo_name)
            other_hi = const(map_text, hi_name)
        ctx.case(not (lo <= other_hi and hi - 1 >= other_lo),
                 "monitor BSS $%08X..$%08X overlaps the %s reservation "
                 "$%08X..$%08X" % (lo, hi - 1, label, other_lo, other_hi))
...
HW_ADDR_UNKNOWN = 2
HW_ADDR_ABSENT = 3
HW_MEM_UNKNOWN = (1 << 64) - 1          # -1 as the emitted code returns it
...
def section_address(ctx: Ctx) -> None:
    ...
    for name in ("HwAddrCheck", "HwAddrReason", "HwAddrBlockLo",
        ...
        for rel in ("RaspberryPi4/Board/hw_addr.pi4",
            ...
            ctx.case(("Procedure.i " + name) in body
                     ...
                     "seam" % (rel, name))
    # The memory-size seam the refusal below is built on. Both halves on
    # both boards: a board that cannot say how much memory it has still
    # has to say SO, and has to say why.
    for name in ("HwMemoryBytes", "HwMemoryWhere"):
        for rel in ("RaspberryPi4/Board/hw_mem.pi4",
                    "ArduinoQ/Board/hw_mem_q.unoq"):
            body = read(rel)
            ctx.case(("Procedure.i " + name) in body,
                     "%s does not define %s - a board must supply the whole "
                     "seam" % (rel, name))
    ctx.case(not re.search(r"\$[0-9A-Fa-f]{4,}",
             ...
             "chip")
    # THE VOCABULARY ITSELF. The numbers below are executed against the
    # emitted code, so a value renumbered in hal.pi4 would turn this whole
    # section into a comparison of two wrong things that agree.
    hal_text = read("Anvil/Hal/hal.pbi")
    for name, want in (("HW_ADDR_SAFE", HW_ADDR_SAFE),
                       ("HW_ADDR_UNCLOCKED", HW_ADDR_UNCLOCKED),
                       ("HW_ADDR_UNKNOWN", HW_ADDR_UNKNOWN),
                       ("HW_ADDR_ABSENT", HW_ADDR_ABSENT)):
        ctx.case(const(hal_text, name) == want,
                 "#%s is %d in Anvil/Hal/hal.pbi and this gate executes "
                 "against %d" % (name, const(hal_text, name), want))
    # The core must act on the fourth answer. A board that returns it into
    # a core that has never heard of it prints the ignorance paragraph over
    # a refusal that is knowledge.
    ctx.case("#HW_ADDR_ABSENT" in read("Anvil/Core/addrsafe.pbi"),
             "Anvil/Core/addrsafe.pbi does not mention #HW_ADDR_ABSENT, so a "
             "board that knows there is nothing at an address is answered "
             "with the paragraph that says the refusal is ignorance")

    # ---- 2. the Pi 4: everything that exists answers SAFE ---------------
    #
    # THE SIZE IS PLANTED, NOT ASKED FOR. HwMemoryBytes() asks the VideoCore
    # firmware once and remembers the answer in two globals
    # (RaspberryPi4/Board/hw_mem.pi4); there is no firmware here, so the
    # gate writes the remembered pair and the production check runs on it
    # exactly as it would on a board of that size. That is the same trick
    # the rest of this file uses for a measured fact - plant the input,
    # execute the real decision - and it is what lets one desk run cover
    # the 1, 2, 4 and 8 GB boards this one image is sold on.
    cpu = ctx.cpu()

    def size_is(bytes_: int) -> None:
        put64(cpu, ctx.sym["global_gpimemasked"], 1)
        put64(cpu, ctx.sym["global_gpimembytes"], bytes_)

    # First, with the size UNKNOWN, which is what a board whose mailbox is
    # down answers: nothing may be refused for being past a top nobody
    # knows. These four addresses answered SAFE before this board could
    # size itself and must still.
    size_is(HW_MEM_UNKNOWN)
    for addr, what in ((0x04AC0004, "the address that hung the other board"),
        ...
        ctx.case(got == HW_ADDR_SAFE,
                 "the Pi 4 answered %d for $%X (%s) with its memory size "
                 "unknown, and a board that cannot say where memory ends "
                 "refuses nothing for being past it" % (got, addr, what))

    # Then the two refusals, on each size a Pi 4 is sold with. They are
    # different refusals and the difference is the whole point. Below the
    # peripheral aperture the tables map everything, so an address past the
    # memory reads a plausible value and is refused as ABSENT with the size
    # named. Above four gigabytes the tables map nothing but the PCIe window,
    # so an address there is a translation fault that STOPS the monitor - it
    # is refused as UNCLOCKED, whose prose says exactly that, and the size is
    # never consulted. Build 183 proved the second one the hard way, on
    # $103FFFFC0, an address this section previously required to be SAFE.
    # (size, address, expected, a word the reason must carry, why)
    for size, addr, expect, word, why in (
            (0x40000000, 0x3FFFFFFC, HW_ADDR_SAFE, None,
             "the last word of a 1 GB board's memory"),
            (0x40000000, 0x80000000, HW_ADDR_ABSENT, "1 GB",
             "a 1 GB board asked about the address a 2 GB board's memory "
             "reaches"),
            (0x40000000, 0xFE201000, HW_ADDR_SAFE, None,
             "the serial port on a 1 GB board - an aperture, not memory"),
            (0x40000000, 0x100000000, HW_ADDR_UNCLOCKED, "unmapped",
             "above four gigabytes this monitor's page tables map nothing, "
             "whatever is fitted - build 183 stopped on such an address"),
            (0x80000000, 0x7FFFFFFC, HW_ADDR_SAFE, None,
             "the last word of a 2 GB board's memory"),
            (0x80000000, 0x80000000, HW_ADDR_ABSENT, "2 GB",
             "one word past a 2 GB board's memory"),
            (0x100000000, 0xFBFFFFFC, HW_ADDR_SAFE, None,
             "a 4 GB board's memory reaches the aperture"),
            (0x100000000, 0x103FFFFFC, HW_ADDR_UNCLOCKED, "unmapped",
             "the 64 MB the aperture displaced is real memory on a 4 GB "
             "board AND this monitor does not map it: $103FFFFC0 is the "
             "address build 183 took a level-1 translation fault on"),
            (0x100000000, 0x104000000, HW_ADDR_UNCLOCKED, "unmapped",
             "one word past the top of a 4 GB board, and unmapped either way"),
            (0x100000000, 0x600000000, HW_ADDR_SAFE, None,
             "the PCIe outbound window, which this monitor maps"),
            (0x200000000, 0x203FFFFFC, HW_ADDR_UNCLOCKED, "unmapped",
             "the last word of an 8 GB board - four gigabytes of its memory "
             "are up here and none of it is reachable from this monitor"),
            (0x200000000, 0x204000000, HW_ADDR_UNCLOCKED, "unmapped",
             "one word past the top of an 8 GB board"),
            (HW_MEM_UNKNOWN, 0x100000000, HW_ADDR_UNCLOCKED, "unmapped",
             "a board that cannot say its size is still not allowed to walk "
             "into the fault: the unmapped refusal never asks the size"),
            (0x100000000, 0x7FFFFFFFC, HW_ADDR_SAFE, None,
             "the last word of the PCIe outbound window, which IS mapped"),
            (0x100000000, 0x800000000, HW_ADDR_UNCLOCKED, "page tables",
             "one word above the PCIe window, where the tables stop again")):
        size_is(size)
        got = ctx.call(cpu, "hwaddrcheck", addr, addr + 3, 0)
        if not ctx.case(got == expect,
                        "with %d bytes fitted the Pi 4 answered %d for $%X, "
                        "wanted %d - %s" % (size, got, addr, expect, why)):
            continue
        if expect == HW_ADDR_SAFE:
            continue
        text = read_cstring(cpu, ctx.call(cpu, "hwaddrreason"))
        ctx.case(word in text,
                 "the Pi 4 refused $%X on a %d-byte board without naming the "
                 "size in the reason. It said: %r" % (addr, size, text[:120]))
        blo = ctx.call(cpu, "hwaddrblocklo")
        bhi = ctx.call(cpu, "hwaddrblockhi")
        ctx.case(blo <= addr <= bhi,
                 "the Pi 4 refused $%X and reported the empty space as "
                 "$%X..$%X, which does not contain it" % (addr, blo, bhi))
    # Leave the image as this section found it, so a later section that
    # calls into the board does not inherit a planted memory size.
    put64(cpu, ctx.sym["global_gpimemasked"], 0)
    put64(cpu, ctx.sym["global_gpimembytes"], 0)
...
# =====================================================================
#  THE SEND CEILING, EXECUTED - #CYW43_TXBUF REACHES TcpTxCap
# =====================================================================
#  "There's no reason to send 1k packets when you can send much larger
#  packets" - the ruling of 2026-09-07. The fix is one constant in the
#  radio driver, and the reason it needs cases HERE rather than a comment
#  is that the constant is four procedures away from the number that
#  decides how big a TCP segment goes out:
#
#     #CYW43_TXBUF -> Cyw43DataMax -> HwLinkTxMax -> LinkTxMaxOn
#                                                 -> TcpTxCap
#
#  Every one of those is pure - no device, no MMIO, no radio - so the whole
#  chain is EXECUTED in the real built image, which is the only thing that
#  proves the constant arrived at the far end. Until 2026-09-07 this chain
#  ended at 1082 and the bench read it back as `send MSS 1082`; the buffer
#  holds a full 1514-octet frame plus both headers now and it ends at 1460.
#
#  THE CLAMP IN THE MIDDLE IS PART OF THE PROPERTY. Cyw43DataMax answers
#  1520 - the buffer minus the twelve octets of SDPCM header and the four of
#  BDC - and RaspberryPi4/Lib/link.pi4 clamps that to #LINK_ETH_MAX, because
#  a board answering larger would invite tcp.pi4 to build a segment no
#  classic Ethernet peer will accept. So 1520 and 1514 are each asserted in
#  their own right: deleting link.pi4's clamp turns exactly the LinkTxMax
#  cases over the radio red, and TcpTxCap still answers 1460, because
#  tcp.pi4 clamps a SECOND time at #TCP_MSS_RX. Two independent ceilings,
#  and neither is reached through the other - which is why LinkTxMax is not
#  left implied by TcpTxCap.
#
#  #CYW43_TXBUF back to 1152 turns the radio cases red and reproduces the
#  pre-fix numbers exactly, TcpTxCap 1082 included. The wired cases stay
#  green through it, because the wired MAC's ceiling is not the radio's
#  buffer.
#
#  WHICH LINK TcpTxCap ASKS ABOUT. It reads tcp_kind - the interface the
#  connection belongs to - through LinkTxMaxOn(tcp_kind), not the selected
#  link, so tcp_kind is what is set for its cases; gLinkKind is set only for
#  LinkTxMax, the wrapper that does read the selection. Both globals are put
#  back afterwards so no later section inherits them.
def section_sendceiling(ctx: Ctx) -> None:
    sym = ctx.sym
    WIRED, WIFI = ctx.hw_wired, ctx.hw_wifi
    LINK_ETH_MAX = 1514
    need = ("cyw43datamax", "hwlinktxmax", "linktxmaxon", "linktxmax",
            "tcptxcap", "global_glinkkind", "global_tcp_kind")
    missing = [n for n in need if n not in sym]
    if not ctx.case(not missing, "the send-ceiling chain cannot be executed: "
                    "%s not in the Pi 4 image's .sym" % ", ".join(missing)):
        return
    cpu = ctx.cpu()
    call = ctx.call
    got = call(cpu, "cyw43datamax")
    ctx.case(got == 1520,
             "Cyw43DataMax() answered %d, wanted 1520. That is #CYW43_TXBUF "
             "minus the twelve octets of SDPCM header and the four of BDC, so "
             "the buffer no longer holds a full %d-octet frame plus both "
             "headers" % (got, LINK_ETH_MAX))
    got = call(cpu, "hwlinktxmax", WIFI)
    ctx.case(got == 1520,
             "HwLinkTxMax(WIFI) answered %d, wanted 1520 - the board seam is "
             "meant to ASK the driver rather than write the number down, so "
             "this disagreeing with Cyw43DataMax means it was written down"
             % got)
    labels = ((WIFI, "the radio"), (WIRED, "the wired MAC"))
    for kind, label in labels:
        got = call(cpu, "linktxmaxon", kind)
        ctx.case(got == LINK_ETH_MAX,
                 "LinkTxMaxOn(%s) answered %d, wanted %d - the 1514 ceiling "
                 "is applied in RaspberryPi4/Lib/link.pi4 and not left to "
                 "each backend" % (label, got, LINK_ETH_MAX))
    kind_addr = sym["global_glinkkind"]
    tcp_addr = sym["global_tcp_kind"]
    saved_kind = peek64(cpu, kind_addr)
    saved_tcp = peek64(cpu, tcp_addr)
    try:
        for kind, label in labels:
            poke64(cpu, kind_addr, kind)
            got = call(cpu, "linktxmax")
            ctx.case(got == LINK_ETH_MAX,
                     "LinkTxMax() with %s selected answered %d, wanted %d - "
                     "the 1514 ceiling is applied in RaspberryPi4/Lib/link.pi4 "
                     "and not left to each backend"
                     % (label, got, LINK_ETH_MAX))
        poke64(cpu, kind_addr, saved_kind)
        want = LINK_ETH_MAX - 14 - 20 - 20
        for kind, label in labels:
            poke64(cpu, tcp_addr, kind)
            got = call(cpu, "tcptxcap")
            ctx.case(got == want,
                     "TcpTxCap() for a connection on %s answered %d, wanted "
                     "%d. THIS IS THE NUMBER THE RULING WAS ABOUT: it was 1082 "
                     "while the radio's transmit buffer held 1136 octets of "
                     "frame, and a segment that small costs one SDPCM header, "
                     "one CMD53 command phase, one PIO drain and one air "
                     "access for 74 per cent of an MTU" % (label, got, want))
    finally:
        poke64(cpu, kind_addr, saved_kind)
        poke64(cpu, tcp_addr, saved_tcp)
...
def section_static(ctx: Ctx) -> None:
    ...
    # THE WIRED STORE'S NUMBERS GO ON THE WIRED ROW, WHATEVER IS SELECTED.
    # gEthIp/gEthMask/gEthGw come from EthHasIpConfig(), which reads the
    # wired settings keys and falls back to the wired link-local address;
    # every one of those is the cable's. HwLinkStaticUp brings the CABLE up
    # and then answers the kind that won the selection, which under
    # `net link wifi` is the radio - so a binder that passes that kind
    # writes the cable's address over the radio's lease row. A pin chooses
    # which door traffic leaves by; it is not evidence about what any
    # interface holds. The kind must be written out literally.
    #
    # The negative control is run inline: the same predicate is applied to
    # the pre-fix line, and if it does not complain about that, the check is
    # not checking anything and this gate says so instead of passing.
    def pin_binder_faults(text: str) -> list:
        """(binders found, kinds that are not #HW_LINK_WIRED) for every
        NetAddressBound that binds the wired store's numbers."""
        kinds = [m.group(1).strip() for m in re.finditer(
            r"NetAddressBound\s*\(([^,]+),\s*gEthIp\b", text)]
        return [len(kinds), [k for k in kinds if k != "#HW_LINK_WIRED"]]

    found, bad = pin_binder_faults(ncmd_code)
    ctx.case(found >= 1,
             "Anvil/Core/net_cmd.pbi has no NetAddressBound(<kind>, gEthIp, "
             "...) at all, so the pin-binder check below has nothing to read "
             "and would pass vacuously")
    ctx.case(not bad,
             "Anvil/Core/net_cmd.pbi binds the wired settings store's address "
             "to a kind taken from the current selection (%s) instead of to "
             "#HW_LINK_WIRED. gEthIp/gEthMask/gEthGw come from "
             "EthHasIpConfig(), which reads the wired keys and falls back to "
             "the wired link-local address - so binding them to whatever is "
             "selected puts the cable's address on the radio's row, and "
             "`net link wifi` then announces one address on both interfaces "
             "and stops answering ARP for the one the board is reachable at"
             % ", ".join(sorted(set(bad))))
    prefix_shape = ("  k = HwLinkStaticUp()\n"
                    "  If k = #HW_LINK_NONE\n"
                    "    ProcedureReturn #HW_LINK_NONE\n"
                    "  EndIf\n"
                    "  NetAddressBound(k, gEthIp, gEthMask, gEthGw, "
                    "#NET_ADDR_SAVED)\n")
    ctx.case(pin_binder_faults(prefix_shape)[1] == ["k"],
             "NEGATIVE CONTROL: the pin-binder check does not complain about "
             "the exact pre-fix NetStaticApply line, NetAddressBound(k, gEthIp, "
             "...), so it would pass whether or not the defect were present")
...
# =====================================================================
#  `net stats` - WHAT EACH NETWORK CONTROLLER COUNTED
# =====================================================================
#  help.pbi documented `net stats` and `net counters` while CmdNet had no
#  arm for either, so both got the unknown-word refusal. The dispatch is
#  read here; HwLinkSayCounters is EXECUTED, with the radio selected, on a
#  controller that has not been started and on one that has. It used to
#  refuse whenever LinkKind() was not the wired link - a routing choice
#  standing in for evidence about what a controller counted - so a board
#  with a cable in and the radio pinned would not print the one number the
#  command exists for. The MIB registers read zero out of flat memory, which
#  is what a healthy link reads, so "pause frames sent: 0" is the line
#  required; the counters themselves are graded by a64_genet_check.
def section_netstats(ctx: Ctx) -> None:
    sym = ctx.sym
    ncmd_code = code_of(read("Anvil/Core/net_cmd.pbi"))
    eth_code = code_of(read("RaspberryPi4/Board/eth.pi4"))
    setter = proc_body(ncmd_code, r"Procedure CmdNet\(\)") or ""
    arm = re.search(r'If WordIs\("stats"\) <> 0 Or WordIs\("counters"\) <> 0'
                    r'\s*\n\s*HwLinkSayCounters\(\)\s*\n\s*ProcedureReturn',
                    setter)
    ctx.case(bool(arm),
             "CmdNet has no `net stats` / `net counters` arm that calls "
             "HwLinkSayCounters(), so the command help.pbi documents answers "
             "with the unknown-word refusal")
    say = proc_body(eth_code, r"Procedure HwLinkSayCounters\(\)") or ""
    ctx.case(bool(say) and "LinkKind()" not in say and "LinkPin()" not in say,
             "HwLinkSayCounters reads the link selection. Which interface "
             "traffic leaves by is not evidence about what a controller has "
             "counted, and it must not decide whether the counters print")
    ctx.case(say.count("eth_SayRadioCounters()") == 2,
             "HwLinkSayCounters does not speak for the radio on both of its "
             "paths (eth_SayRadioCounters appears %d times, want 2)"
             % say.count("eth_SayRadioCounters()"))
    need = ("hwlinksaycounters", "global_genet_started", "global_glinkkind")
    missing = [n for n in need if n not in sym]
    if not ctx.case(not missing, "%s not in the Pi 4 image's .sym, so `net "
                    "stats` cannot be executed (an unreferenced procedure is "
                    "not linked: is the dispatch there?)" % ", ".join(missing)):
        return
    for started in (0, 1):
        cpu = ctx.cpu()
        poke64(cpu, sym["global_glinkkind"], ctx.hw_wifi)
        poke64(cpu, sym["global_genet_started"], started)
        try:
            text = pi4_console(ctx, cpu, "hwlinksaycounters")
        except SystemExit as error:
            ctx.case(False, "HwLinkSayCounters with the controller %s did not "
                     "return: %s" % (("stopped", "started")[started], error))
            continue
        low = " ".join(text.split()).lower()
        ctx.case("the wired ethernet" in low and "the wi-fi radio" in low
                 and "net link" not in low,
                 "`net stats` with the radio selected and the Ethernet "
                 "controller %s does not name both interfaces, or tells the "
                 "reader to change the selection:\n%s"
                 % (("stopped", "started")[started], text))
        if started:
            ctx.case("pause frames this board sent: 0." in low,
                     "`net stats` on a started controller does not print "
                     "'Pause frames this board SENT: 0.' with the radio "
                     "selected - the line that must print whatever it says:\n"
                     "%s" % text)
        else:
            ctx.case("has not been started" in low,
                     "`net stats` on a controller that was never started does "
                     "not say so:\n%s" % text)
...
def section_interfaces(ctx: Ctx) -> None:
    ...
    # MEASURED OVER THE CABLE 2026-09-07 on build 13, and it is the fault
    # that let a board be talked to and not FOUND: a unicast to the
    # link-local address came back, while a discovery broadcast to
    # 169.254.255.255 found nothing, because the reply was sourced from the
    # address the LAST UNICAST had named and the host discarded it as
    # unsolicited with nothing logged at either end.
    #
    # The console's reply source is NetRxTo(), which net.pbi's receive path
    # sets - so that is what is graded, not only NetSrcFor. EACH CASE STARTS
    # WITH NetRxTo() STANDING AT THE OTHER ADDRESS, staged the only way the
    # board can reach that state: by RECEIVING a unicast to it first, through
    # the real parser. A case that started from a clean slate would pass with
    # the broadcast arm taken back out.
    def udp_frame(c, dst_ip, src_ip):
        """A 42-octet UDP datagram to port 5555, in scratch memory. The
        Ethernet destination is the broadcast address for both the unicast
        and the broadcast case, so the ONE thing differing between them is
        the IPv4 destination - the field the rule under test is about."""
        f = SCRATCH + 0x600
        for i in range(80):
            c.memory[f + i] = 0
        for i in range(6):
            c.memory[f + i] = 0xFF
            c.memory[f + 6 + i] = PEER_MAC[i]
        c.memory[f + 12], c.memory[f + 13] = 0x08, 0x00
        h = f + 14
        c.memory[h + 0] = 0x45
        c.memory[h + 3] = 0x1C
        c.memory[h + 8] = 64
        c.memory[h + 9] = 17
        for i in range(4):
            c.memory[h + 12 + i] = (src_ip >> (8 * (3 - i))) & 0xFF
            c.memory[h + 16 + i] = (dst_ip >> (8 * (3 - i))) & 0xFF
        # The header checksum, computed here rather than taken from the
        # library under test.
        ck = 0
        for i in range(0, 20, 2):
            ck += (c.memory.get(h + i, 0) << 8) | c.memory.get(h + i + 1, 0)
            ck = (ck & 0xFFFF) + (ck >> 16)
        ck = (~ck) & 0xFFFF
        c.memory[h + 10], c.memory[h + 11] = ck >> 8, ck & 0xFF
        u = h + 20
        c.memory[u + 1] = 0x99
        c.memory[u + 2], c.memory[u + 3] = 0x15, 0xB3          # 5555
        c.memory[u + 5] = 8
        return f

    for asker, want, stale in ((quad(169, 254, 183, 67), LL, SELF137),
                               (quad(192, 168, 137, 5), SELF137, LL)):
        c2 = two_link_cpu()
        ...
        rc = call(c2, "netinput", WIRED, udp_frame(c2, stale, asker), 42)
        got = call(c2, "netrxto")
        ctx.case(rc == IN_UDP and got == stale,
                 "a unicast from %08X to %08X did not leave the board holding "
                 "%08X as the address it was spoken to at (NetInput %d, "
                 "NetRxTo %08X), so the broadcast case below is not being "
                 "asked the question it exists to ask"
                 % (asker, stale, stale, rc, got))
        rc = call(c2, "netinput", WIRED, udp_frame(c2, 0xFFFFFFFF, asker), 42)
        if ctx.case(rc == IN_UDP, "a UDP broadcast from %08X to port 5555 on "
            ...
            got = call(c2, "netrxto")
            ctx.case(got == want,
                     "after a unicast to %08X and then a broadcast from %08X, "
                     "NetRxTo() - the console's reply source - still answers "
                     "%08X and must answer %08X. Leaving the last unicast's "
                     "answer standing is how a board with two addresses can "
                     "be talked to and cannot be FOUND"
                     % (stale, asker, got, want))
            got = call(c2, "netrxkind")
            ctx.case(got == WIRED,
                     "NetRxKind() answers %d after a broadcast on the wired "
                     "interface. The console builds its reply for that "
                     "interface, and one built for the other link leaves by a "
                     "wire the asker is not on" % got)
    ...
    def arp_request(c, tpa, spa, sha, kind=WIRED):
        """An ARP request fed to NetInput on `kind`; returns its verdict."""
        f = SCRATCH + 0x500
        for i in range(64):
            c.memory[f + i] = 0
        for i in range(6):
            c.memory[f + i] = 0xFF
            c.memory[f + 6 + i] = sha[i]
        c.memory[f + 12], c.memory[f + 13] = 0x08, 0x06
        a = f + 14
        c.memory[a + 1] = 1
        c.memory[a + 2], c.memory[a + 3] = 0x08, 0x00
        c.memory[a + 4], c.memory[a + 5] = 6, 4
        c.memory[a + 7] = 1
        for i in range(6):
            c.memory[a + 8 + i] = sha[i]
        for i in range(4):
            c.memory[a + 14 + i] = (spa >> (8 * (3 - i))) & 0xFF
            c.memory[a + 24 + i] = (tpa >> (8 * (3 - i))) & 0xFF
        return call(c, "netinput", kind, f, 42)
    ...
    for tpa, spa, why in (
        ...
        rc = arp_request(c3, tpa, spa, PEER_MAC)
    ...
    # ---- K5b. an RFC 3927 conflict belongs to one interface -------------
    # [RFC 3927 s2.5: a host may defend "an interface where that host is
    # currently using the same address".] A board holding 169.254.x.y on a
    # cable while its radio is joined to a house network receives ARP on
    # both, and a frame off the RADIO's segment says nothing whatever about
    # who owns an address on the CABLE - acting on it would surrender an
    # address nobody contested, on the one link the directly attached host
    # is on. The SAME frame is fed in on each interface and the two answers
    # must differ; the watched mode is CLAIMED, and the conflict flag is set
    # before the mode is looked at, so the defence that follows does not
    # decide the verdict.
    net_text = read("Anvil/Network/net.pbi")
    LL_CLAIMED = const(net_text, "NET_LL_CLAIMED")
    STRANGER_MAC = [0x02, 0x99, 0x88, 0x77, 0x66, 0x55]
    for kind, want, why in (
            (WIFI, 0, "a frame off the RADIO's segment claiming the CABLE's "
             "link-local address"),
            (WIRED, 1, "a frame on the CABLE itself claiming the address the "
             "cable is defending")):
        c3 = two_link_cpu()
        call(c3, "netifset", WIRED, LL, LLMASK, 0, ADDR_LL)
        call(c3, "netifset", WIFI, HOMEIP, HOMEMASK, HOMEGW, ADDR_LEASE)
        call(c3, "netllwatch", WIRED, LL, LL_CLAIMED)
        # A stranger announcing the address the cable holds: the sender
        # protocol address IS the watched address, the "somebody is using
        # it" limb of the rule.
        arp_request(c3, LL, LL, STRANGER_MAC, kind=kind)
        got = call(c3, "netllconflict")
        ctx.case(got == want,
                 "NetLlConflict() is %d and should be %d after %s. RFC 3927 "
                 "defends an INTERFACE, not a board: a conflict seen on a "
                 "link that does not hold the address is somebody else's "
                 "segment, and acting on it gives away an address nobody "
                 "contested" % (got, want, why))
    ...
    # ---- K6b. the broadcast builder will not forge a source address -----
    # A frame to 255.255.255.255 is on no subnet, so nothing about the
    # destination can say which of an interface's addresses it leaves with,
    # and the two callers want different answers on the same wire: a DHCP
    # CLIENT with no address yet must source 0.0.0.0 [RFC 2131 s4.1], while
    # this board's DHCP SERVER must source the address it hands leases out
    # on - the interface's ALIAS, 192.168.137.1, not its link-local primary.
    # So the source is named in the call, and naming it is exactly where a
    # layer becomes a forgery engine unless it refuses an address the
    # interface does not hold. #NET_E_ARG is the refusal; it is read out of
    # net_err (8 octets in the .sym.meta) rather than through an accessor
    # this image may not keep.
    m_arg = re.search(r"(?m)^\s*#NET_E_ARG\s*=\s*(-?\d+)", net_text)
    NET_E_ARG = int(m_arg.group(1)) if m_arg else None
    if ctx.case(NET_E_ARG is not None and "netudpbuildbcast" in sym
                and "global_net_err" in sym,
                "#NET_E_ARG is not declared in Anvil/Network/net.pbi, or "
                "NetUdpBuildBcast or net_err is not in the Pi 4 image's .sym, "
                "so the forged-source refusal cannot be executed"):
        c = serving_cpu()
        for i, bb in enumerate(b"BOOTP"):
            c.memory[SCRATCH + 0xA00 + i] = bb
        for src, want_ok, why in (
                (0, True, "0.0.0.0 is the source a DHCP CLIENT must use "
                 "before it has an address [RFC 2131 s4.1]"),
                (SELF137, True, "192.168.137.1 is this interface's alias and "
                 "the address the server hands leases out on"),
                (LL, True, "169.254.170.250 is this interface's primary "
                 "address"),
                (HOMEIP, False, "192.168.1.12 is the RADIO's address and not "
                 "this interface's"),
                (quad(192, 168, 137, 7), False, "192.168.137.7 is on the "
                 "served subnet and is an address this board HANDS OUT, so "
                 "sourcing from it impersonates a client of its own"),
                (quad(10, 0, 0, 5), False, "10.0.0.5 is on no subnet this "
                 "interface has")):
            n = call(c, "netudpbuildbcast", WIRED, src, 68, 67,
                     SCRATCH + 0xA00, 5)
            err = signed(peek64(c, sym["global_net_err"]))
            if want_ok:
                if ctx.case(n != 0,
                            "NetUdpBuildBcast refused %08X as a source address "
                            "and it must build: %s. net_err is %d"
                            % (src, why, err)):
                    out = call(c, "netoutbuf")
                    got = be32(c, out + 14 + 12)
                    ctx.case(got == src,
                             "NetUdpBuildBcast was given %08X as the source "
                             "address and put %08X in the IPv4 header. A DHCP "
                             "reply whose source does not match the server "
                             "identifier inside it is one every client throws "
                             "away" % (src, got))
            else:
                ctx.case(n == 0,
                         "NetUdpBuildBcast answered %d - it BUILT a frame "
                         "sourced from %08X. It must refuse: %s. An interface "
                         "that can be told to source a broadcast from an "
                         "address it does not hold is one any caller can make "
                         "forge a frame" % (n, src, why))
                ctx.case(err == NET_E_ARG,
                         "NetUdpBuildBcast refused %08X with net_err = %d and "
                         "must refuse with #NET_E_ARG (%d) - %s. A refusal the "
                         "caller cannot tell from a short buffer or a bad port "
                         "is one it cannot report" % (src, err, NET_E_ARG, why))
    ...
    # ---- K7b. the role is taken on one link and answered on that link ----
    # MEASURED ON SILICON 2026-09-08, and the source read alone did not find
    # it. `net` reported "9 offers and 0 acknowledgements" on the wired
    # Ethernet in the same reply as "there is nothing plugged into the
    # Ethernet socket". DhcpdStart refuses a radio (K6), but that guard is
    # on TAKING the role; there was none on the traffic. The console pump
    # walks EVERY addressed interface, so a board serving the cable answered
    # DISCOVERs heard on the RADIO with offers of 192.168.137.x into a house
    # network. Those clients took their router's lease and ignored ours, so
    # no REQUEST ever came back: the offer count was real, and the
    # conclusion drawn from it - "our OFFER is malformed" - was not.
    #
    # The radio is given its house lease first, as on that bench. Without
    # it the radio has no hardware address in the IP layer and nothing could
    # be staged on it whatever DhcpdInput decided, so the first two cases
    # would pass with the guard removed. With it, an answer on the radio is
    # still refused a second time by NetUdpBuildBcast (the radio does not
    # hold 192.168.137.1), which is why the wrong-link COUNT is the case
    # that goes red when only the DHCP server's own guard is removed.
    c = serving_cpu()
    call(c, "netifset", WIFI, HOMEIP, HOMEMASK, HOMEGW, ADDR_LEASE)
    m, n = bootp(c, DISCOVER)
    ctx.case(call(c, "dhcpdinput", WIFI, m, n, 0) == 0,
             "a DHCPDISCOVER heard on the RADIO was answered by a server that "
             "took its role on the CABLE. That is a board handing out "
             "192.168.137.x leases into somebody's home network, which is the "
             "one thing Anvil/Core/dhcpd.pbi's header forbids outright")
    ctx.case(call(c, "dhcpdoffers") == 0,
             "a DISCOVER on a link this server does not serve was counted as "
             "an OFFER. That count is the only evidence anybody has when a "
             "DHCP exchange does not complete, and it must not include replies "
             "to somebody else's network")
    ctx.case(call(c, "dhcpdwronglink") == 1,
             "a DHCP message from a link this server does not serve was not "
             "counted as such. `net link` prints that count so the next reader "
             "sees 'somebody else's network overheard' instead of an offer "
             "tally nobody can explain")
    # AND THE SAME GUARD THE OTHER WAY. A client on the house network
    # broadcasts its REQUEST to port 67 naming the router it chose; that
    # reaches this board on the radio, and without the guard the "a request
    # naming another server ends the role" branch stands the CABLE's server
    # down because of a conversation on the RADIO.
    c = serving_cpu()
    m, n = bootp(c, REQUEST, req_ip=quad(192, 168, 1, 50))
    for i, b in enumerate((54, 4, 192, 168, 1, 2, 255)):
        c.memory[m + n - 1 + i] = b                  # option 54, the router
    call(c, "dhcpdinput", WIFI, m, n + 6, 0)
    ctx.case(call(c, "dhcpdon") == 1,
             "a REQUEST overheard on the RADIO ended the CABLE's server role. "
             "A real server on the other link is not a rival for this link's "
             "addresses, and standing down drops the one link that has no "
             "server on it at all")
    # A real server's own BOOTREPLY on the radio is not evidence about the
    # cable either.
    c = serving_cpu()
    m, n = bootp(c, OFFER, op=2)
    call(c, "dhcpdinput", WIFI, m, n, quad(192, 168, 1, 2))
    ctx.case(call(c, "dhcpdon") == 1,
             "an OFFER from the house router, heard on the RADIO, ended the "
             "CABLE's server role")
    ...
    ctx.case(bool(lp) and "HwLinkRecv(kind," in lp and "NetInput(kind," in lp,
             ...
             "is ignored")
    # AND NOTHING BUT `kind`. The two cases above ask that one call names
    # the interface; these ask that EVERY call in each body does. A second
    # NetInput beside the right one - a literal #HW_LINK_WIFI, the preferred
    # interface, the selected link - would compile, satisfy a substring
    # test, and parse a frame as an interface it did not arrive on.
    for body, where in ((pump, "netcon_PumpOne in Anvil/Core/netconsole.pbi"),
                        (lp, "LinkPumpKind in RaspberryPi4/Lib/link.pi4")):
        firsts = [a.strip() for a in re.findall(
            r"\bNetInput\s*\(\s*([^,)]*)", code_cut(body or ""))]
        ctx.case(bool(firsts) and all(a == "kind" for a in firsts),
                 "%s hands NetInput %s as the interface; every call there must "
                 "pass exactly `kind`, the interface the frame was read from"
                 % (where, ", ".join(repr(a) for a in sorted(set(firsts)))
                    or "nothing - it never calls NetInput"))
...
def run_sections() -> int:
    ...
    for section in (section_layout, section_parse, section_address,
                    ...
                    section_sendceiling,
                    section_netcon, section_static, section_netstats,
                    section_linklocal,
                    section_interfaces):
