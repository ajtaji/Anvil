...
def check_source(lib: pathlib.Path, fails: list) -> int:
    ...
    # Both spellings: FsSetRangeWriter is the filesystem dispatch that the
    # file seam arms, and it reaches FatSetRangeWriter underneath.
    n = code.count("FatSetRangeWriter") + code.count("FsSetRangeWriter")
    if n:
        fails.append(
...
MUTATIONS = [
    ...
    # THE FOLD MOVED AND THIS ANCHOR MOVED WITH IT, 2026-09-16. It used to
    # name the two lines inlined in set_MakeKey; the fold is now one
    # procedure, because the payload service's guard has to ask the store
    # what "the same key" means rather than keep a second opinion, and a
    # fold with two copies is exactly the thing that cannot be asked. An
    # anchor left behind would not have failed: the mutation simply would
    # not apply and the leg would report GREEN while breaking nothing,
    # which is how a sabotage control quietly stops being one.
