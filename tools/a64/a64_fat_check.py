...
fat32.pbi at it through the same FatSetBlockReader / FatSetBlockWriter
...
ex("long name code", E_NAME)
ex("path separator refused", 0)
ex("path separator code", E_NAME)
ex("two dots refused", 0)
ex("two dots code", E_NAME)
ex("four-character extension refused", 0)
ex("four-character extension code", E_NAME)
# 9. skipping
...
ex("the unaligned destination never took the direct path", 0)
...
ex("its entry is in the second root cluster", clus_lba(8))
...
ex("is-a-directory code", E_IS_DIR)
...
def check_structure(r: Run, fails: list) -> int:
    ...
    if two:
        ...
        want(r.chain(two["clus"]) == [10],
             "TWO.BIN's chain is %s, want [10]" % r.chain(two["clus"]))
    ...
    if three:
        want(three["lba"] == clus_lba(8),
             "THREE.BIN's entry is at LBA %d, want %d - the second root "
             "cluster" % (three["lba"], clus_lba(8)))
    ...
    want(r.chain(2) == [2, 8],
         "the root directory's chain is %s, want [2, 8]" % r.chain(2))
    seen_end = False
    junk = None
...
MUTATIONS = [
    ...
    ("a cluster is written to before it is claimed in the FAT",
