...
def check_source(vop_path: Path) -> None:
    ...
    telemetry = procedure(source, "rockvopcapturetelemetry")
    for token in (
        ...
        require(token in telemetry, f"VOP raw telemetry omits {token}")
    ...
    require("rockvopfield(#vop_win0_ctrl0" not in start,
            "start reads active CTRL0 and discards pending line-buffer fields")
...
def check_assembly(assembly_path: Path) -> None:
    ...
    require(re.search(r"\bbl\s+rockvopwrite\b", start) is not None and
            re.search(r"\bbl\s+rockvoplatchframe\b", start) is not None,
            "emitted start omits complete WIN0 write or frame latch")
    require(re.search(r"\bbl\s+rockvopfield\b", start) is None,
            "emitted start still reads active CTRL0 before pending CFG_DONE")
