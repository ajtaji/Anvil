...
def main() -> int:
    ...
    # ---- the console keepalive (forum 839) ---------------------------
    # The PC's firewall forgets a silent UDP flow in about two minutes, so a
    # long wait on the console must keep sending. What is checked: EMPTY
    # datagrams (the monitor reads those as no keystrokes) go out while the
    # console only waits, and none go out when the interval has not passed
    # - a keepalive that fired on every receive would flood the board.
    import importlib.util as _ilu
    _spec = _ilu.spec_from_file_location("pi4_upload_ka", HERE / "pi4_upload.py")
    _tool = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(_tool)
    # AND NEVER AT AN IDLE PROMPT (forum 888): the monitor hands the console
    # to whoever sends a datagram while nobody owns it and frees it only when
    # a command finishes, so an empty datagram between commands locks every
    # other host out. The fake board below answers nothing, so a command sent
    # here stays outstanding; a prompt from the board ends it.
    for label, interval, want_some, command, prompt_back in (
            ("sends while a command is outstanding", 0.15, True, "sleep 300", False),
            ("stays quiet inside the interval", 60.0, False, "sleep 300", False),
            ("stays quiet at an idle prompt", 0.15, False, "", False),
            ("stops once the prompt comes back", 0.15, False, "version", True)):
        cases += 1
        board = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        board.bind(("127.0.0.1", 0))
        board.settimeout(0.05)
        con = _tool.WifiConsole("127.0.0.1", board.getsockname()[1])
        con.keepalive_seconds = interval
        got: list[bytes] = []
        stop = threading.Event()

        def listen(sock=board, sink=got, done=stop):
            while not done.is_set():
                try:
                    sink.append(sock.recvfrom(4096)[0])
        ...
        if command:
            con.send(command)
        if prompt_back:
            board.sendto(b"version\r\npmf> ", ("127.0.0.1", con.s.getsockname()[1]))
        con.read_until(["never"], 1.0)
        stop.set()
        t.join(timeout=2)
        con.close()
        board.close()
        empties = sum(1 for d in got if d == b"")
        others = [d for d in got if d not in (b"", b"\r", (command + "\r").encode())]
        good = (empties >= 3) if want_some else (empties == 0)
        good = good and not others and empties == con.keepalives
        if not good:
            fails.append("the console keepalive %s: %d empty datagrams, %d "
                         "counted, other traffic %r" % (label, empties,
                                                         con.keepalives, others))
        print("  %d. the console keepalive %s  %s" % (cases, label, "OK" if good else "FAIL"))
