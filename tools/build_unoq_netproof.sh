#!/bin/bash
# ======================================================================
#  build_unoq_netproof.sh - build the Arduino UNO Q LINK SURVEY AND IP
#                           CONFORMANCE run and wrap it as an AArch64
#                           UEFI application.
#
#  Run from the repo root:  bash tools/build_unoq_netproof.sh
#
#  Same contract as build_unoq_efiprobe.sh: -t unoq (the shared A64
#  backend), linked into the Q's confirmed-free low-bank DRAM,
#  --entry-returns so x0/x1 (ImageHandle/SystemTable) survive into Main().
#
#  LIKE THE PROTOCOL WALK AND UNLIKE THE GPU PROBE, THIS CANNOT HANG THE
#  BOARD. It writes no register and reads no peripheral; the only system
#  registers it touches are CNTPCT_EL0 and CNTFRQ_EL0, which are
#  architectural on every ARMv8-A core. See the header of
#  ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqnetproof.unoq.
#
#  STAGE THE SCRIPT FIRST. The conformance half does nothing without
#  \EFI\anvil\net.in; build one with tools/qnetfile_script.py. With no
#  script staged the image still prints the link survey and says so.
# ======================================================================
set -e
...
usage() {
  echo "usage: bash tools/build_unoq_netproof.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqnetproof.unoq"
IMG="$OUTDIR/anvilqnetproof.img"
EFI="$OUTDIR/anvilqnetproof.efi"

LOAD=0x70000000
STACK=0x68000000

echo ">> compiling the UNO Q link survey and IP conformance run (-t unoq)"
