#!/usr/bin/env python3
"""anvil_wifi_update.py - replace Anvil with a new build, over Wi-Fi.

    python tools/anvil_wifi_update.py <board-ip> [image]

The serial twin, tools/anvil_update.py, sends the image down the FTDI
cable. This sends it over the air instead, so the cable is free for
something else and a board with no serial attached can still be
