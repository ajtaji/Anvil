#!/bin/bash
# ======================================================================
#  build_unoq_efiprobe.sh - build the Arduino UNO Q UEFI PROTOCOL
#                           DATABASE WALK and wrap it as an AArch64 UEFI
#                           application.
#
#  Run from the repo root:  bash tools/build_unoq_efiprobe.sh
#
#  Same contract as build_unoq_conprobe.sh and build_unoq_gpuprobe.sh:
#  -t unoq (the shared A64 backend), linked into the Q's confirmed-free
#  low-bank DRAM, --entry-returns so x0/x1 (ImageHandle/SystemTable)
#  survive into Main().
#
#  UNLIKE THE GPU PROBE, THIS ONE CANNOT HANG THE BOARD. It writes no
#  register and reads no peripheral; every address it dereferences was
#  handed to it by the firmware. See the header of
#  ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqefiprobe.unoq.
# ======================================================================
set -e
...
usage() {
  echo "usage: bash tools/build_unoq_efiprobe.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqefiprobe.unoq"
IMG="$OUTDIR/anvilqefiprobe.img"
EFI="$OUTDIR/anvilqefiprobe.efi"

LOAD=0x70000000
STACK=0x68000000

echo ">> compiling the UNO Q UEFI protocol walk (shared A64 backend, -t unoq)"
