...
def source_contract() -> None:
    ...
    for include in ("cru.pbi", "tcphy.pbi", "cdn_dp.pbi", "vop.pbi", "display.pbi", "anvil/graphics/edid.pbi"):
    ...
    for token in (
        ...
        "sharedblocks=extensioncount+1",
        "sharedblocks>#anvil_edid_max_blocks",
        "anviledidparse(@rock_cdn_edid[0],sharedblocks*#anvil_edid_block_bytes,sharedblocks,sharedtruncated)",
        "rockmodeselect(@rock_cdn_edid[0])",
    ):
    ...
    security_result = subprocess.run(
        [sys.executable, str(ROOT / "tools/rockpi4c_security_check.py")],
        cwd=ROOT, capture_output=True, text=True)
    require(security_result.returncode == 0,
            "DMA security contract failed: " + security_result.stdout + security_result.stderr)
    ...
    require(positions == sorted(positions),
            "selected VPLL slow/program/lock/normal order drifted")
    require("rockcrufield($cc,$0001" not in vpll,
            "VPLL rate change incorrectly powers the PLL down")
    ...
    selected_clock = cru.split("procedure.i rockcruvpllmodeforvop(vopid.i)", 1)[1].split(
        "endprocedure", 1
    )[0]
    selected_order = ["rockcrugate(10,12+vopid,0)",
    ...
    shared_parse = read_edid.find("anviledidparse(@rock_cdn_edid[0]")
    rock_caps = read_edid.find("rockedidcapscollect(@rock_cdn_edid[0],extensioncount+1)")
    rock_select = read_edid.find("rockmodeselect(@rock_cdn_edid[0])")
    require(0 <= rock_caps < shared_parse < rock_select,
            "common EDID parsing must consume fetched blocks without replacing Rock mode policy")
    require("sharedblocks=extensioncount+1" in read_edid and
            "sharedblocks=#anvil_edid_max_blocks" in read_edid and
            "sharedtruncated=1" in read_edid,
            "shared EDID view must be bounded and mark omitted extension blocks")
...
def compiler_contract(compiler: Path) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix="anvil-rockpi4c-display-") as temp_name:
        ...
        command = [
            ...
            "-t", "rockpi4c", "--load-addr", "0x00041000",
