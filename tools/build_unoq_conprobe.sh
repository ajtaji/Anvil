#!/bin/bash
# ======================================================================
#  build_unoq_conprobe.sh - build the Arduino UNO Q CONSOLE PROBE and
#                           wrap it as an AArch64 UEFI application.
#
#  Run from the repo root:  bash tools/build_unoq_conprobe.sh
#
#  Same contract as build_unoq_coreproof.sh: -t unoq (shared A64 backend),
#  linked into the Q's confirmed-free low-bank DRAM, position independent,
#  --entry-returns so x0/x1 (ImageHandle/SystemTable) survive into Main().
#  See ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqconprobe.unoq for what it asks
#  the firmware and why.
# ======================================================================
set -e
...
usage() {
  echo "usage: bash tools/build_unoq_conprobe.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqconprobe.unoq"
IMG="$OUTDIR/anvilqconprobe.img"
EFI="$OUTDIR/anvilqconprobe.efi"

LOAD=0x70000000
STACK=0x68000000

echo ">> compiling UNO Q console probe (shared A64 backend, -t unoq)"
