#!/bin/bash
# ======================================================================
#  build_unoq_probe.sh - build the Arduino UNO Q bring-up probe and wrap
#                        it as an AArch64 UEFI application.
#
#  Run from the repo root:  bash tools/build_unoq_probe.sh
#
#  This builds with -t unoq. It once built with -t pi4, as a stopgap from
#  before the unoq target had a code generator. The unoq target reuses the
#  same A64 backend as pi4, and the probe touches only architectural system
#  registers, DRAM and the runtime UEFI SystemTable - no BCM2711
#  peripheral - so its output is fully portable to the Q.
# ======================================================================
set -e
...
usage() {
  echo "usage: bash tools/build_unoq_probe.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/anvilqprobe.unoq"
IMG="$OUTDIR/anvilqprobe.img"
EFI="$OUTDIR/anvilqprobe.efi"
...
# has to pass A64SetLoadBase. Stack top is absolute and must be in mapped
# DRAM clear of the Qualcomm-reserved holes - 0x68000000 is inside the
# large free block confirmed on the board via /proc/iomem.
LOAD=0x70000000
STACK=0x68000000

echo ">> compiling probe (shared A64 backend, -t unoq)"
