...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='anvil-touch-visual-') as td:
  d=Path(td); exe=d/pmfc.name; shutil.copy2(pmfc,exe); src=d/'gate.pi4'; src.write_text(f,encoding='utf-8'); img=d/'gate.img'; env=os.environ.copy(); env['PMF_ROOT']=str(ROOT)
 ...
 print(f'touch_visual_emitted_check: PASS - ten IDs, motion/release, three exits and renderer restoration, {steps:,} instructions'); return 0
