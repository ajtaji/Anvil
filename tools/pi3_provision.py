...
No disk is selected, partitioned, formatted, or cleaned. By default, replaced
files are copied to a new backup directory before card writes; the caller may
explicitly opt out with --skip-backup.
"""
...
def install(card_root: Path, bundle: Path, backup_dir: Path | None,
            manifest_sha256: str, *, yes_replace_kernel8: bool,
    ...
    if skip_backup:
        if backup_dir is not None:
            raise ProvisionError("--skip-backup and --backup-dir cannot be combined")
        backup = None
    else:
        if backup_dir is None:
            raise ProvisionError("--backup-dir is required unless --skip-backup is explicit")
    ...
    validate_config(bundle / "config.txt")
    required_existing = ("bootcode.bin", "start.elf", "fixup.dat",
    ...
    summary = {
        ...
        "backupSkipped": bool(skip_backup),
        "manifestSha256": manifest_sha256.lower(),
        "installed": {name: digest for name, _, _, digest in destinations},
        "changedBackups": ([name for name, _ in changed] if backup is not None else []),
...
def main() -> int:
    ...
    parser.add_argument("--backup-dir", type=Path,
                        help="new directory outside the card for prior files (default) ")
    parser.add_argument("--skip-backup", action="store_true",
                        help="explicitly replace files without making prior-file copies")
