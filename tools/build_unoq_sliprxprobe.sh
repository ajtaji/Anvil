#!/bin/bash
# ======================================================================
#  build_unoq_sliprxprobe.sh - build the Arduino UNO Q SLIP RECEIVE WORD PROBE and
#                            wrap it as an AArch64 UEFI application.
#
#  Run from the repo root:  bash tools/build_unoq_sliprxprobe.sh
#
...
#  ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqsliprxprobe.unoq before deploying it.
#
...
usage() {
  echo "usage: bash tools/build_unoq_sliprxprobe.sh [--compiler <PureMetalForge.exe>]"
}
while [ $# -gt 0 ]; do
  case "$1" in
...
mkdir -p "$OUTDIR"
SRC="$ROOT/ArduinoQ/Examples/Diagnostics/NotBuilding/anvilqsliprxprobe.unoq"
IMG="$OUTDIR/anvilqsliprxprobe.img"
EFI="$OUTDIR/anvilqsliprxprobe.efi"
