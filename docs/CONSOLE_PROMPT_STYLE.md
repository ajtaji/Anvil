...
Nothing but the prompt, the typed input and a command's own output is ever
written after the prompt: background messages go to the message log
([CONSOLE_MESSAGES.md](CONSOLE_MESSAGES.md)), so the five bytes are never
followed by a notice a tool would have to skip.
