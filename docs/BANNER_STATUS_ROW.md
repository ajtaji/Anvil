...
3. the fan's duty, as a whole percent, when a pulse-width modulator is actually
   being driven;
4. the newest message nobody has read yet, with how many are unread, when there
   is one - `2 new messages: Wi-Fi: group key rekeyed.` Background services
   never print on the command line ([CONSOLE_MESSAGES.md](CONSOLE_MESSAGES.md));
   this is where the screen shows that one has spoken, and it goes as soon as
   `messages` has been run. It is last, so a narrow surface clips it first.
