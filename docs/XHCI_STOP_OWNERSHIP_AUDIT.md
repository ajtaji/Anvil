...
## Later change: HCE-forced reset and the halt witness (unproven)

`xh_Halt` now quiesces the way Linux `xhci_quiesce()` does (the three interrupt enables always come off, Run/Stop only when running), refuses an all-ones USBSTS read as `XHCI_ERR_GONE` (46) in both `xh_WaitReady` and the halt loop, and names a timed-out halt with USBSTS.HCE set as `XHCI_ERR_HCE` (47). Seven witness globals keep every register the halt path read (`XhciHaltSts0/Cmd0/CmdW/CmdBack/Sts1/Ms/Forced`).

This adds one path to the boundary above. On `XHCI_ERR_HCE`, `xh_Reset` sets `xh_haltForced = 1`, clears the error and issues HCRST **without HCHalted having been observed**, and `XhciInitAt` lets that one refusal fall through to `xh_Reset`. The justification is xHCI 1.2b section 4.24.1 (HCE means the xHC "shall cease all activity"; the software response is HCRST), read against section 5.4.1's rule that forbids HCRST on an *actively running* controller. It is derived from the specification only. **No controller with HCE set has been observed on silicon, and the forced reset has not run on a board.** A retained marker sequence of 2 followed by 4 (no 3) identifies it. `xh_FailStop` keeps the original initialization error if the cleanup reset is forced and then succeeds, so a failed initialization cannot report "ok".

Desk proof: `tools/xhci_initial_readiness_check.py` HCE and all-ones cases, with mutants for each.
