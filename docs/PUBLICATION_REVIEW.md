...
- **A working tree that is not a clean view of the Git selection.** The module
  ...
  `verify_export.py --for-publication` reports after the hold itself. The scratch
  at the repository root — `build/`, `_work/`, `.migration-local/`,
  `chatgptHandoff.md` — is gitignored and outside the tracked set, so it is not
  part of what would be published.
