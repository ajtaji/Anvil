...
The complete physical-device properties ABI is now present and checked, but
...
| `vkEnumerateInstanceLayerProperties` / `vkEnumerateInstanceExtensionProperties` / `vkEnumerateDeviceLayerProperties` / `vkEnumerateDeviceExtensionProperties` | Implemented. Both layer lists and both extension lists are empty. Named-layer extension queries return `VK_ERROR_LAYER_NOT_PRESENT`. |
...
| `vkGetPhysicalDeviceSparseImageFormatProperties` | Implemented. Sparse residency is not supported, so the query returns zero properties and leaves supplied property storage untouched. |
...
| `vkGetImageSparseMemoryRequirements` | Implemented. Every accepted image is non-sparse, so a live image returns zero requirements without writing the supplied array. Null count, stale handles and wrong-device ownership leave caller output untouched and record a fault. |
| `vkGetImageSubresourceLayout` | Implemented for the color aspect, mip zero and layer zero of a linear image. It reports the backend's image size and row pitch; array and depth pitch have no defined meaning for this 2D non-array image. |
...
| `vkFlushMappedMemoryRanges` / `vkInvalidateMappedMemoryRanges` | Implemented for valid mapped ranges of a `HOST_COHERENT` allocation. The complete input array is validated; coherent memory needs no extra host cache operation. Non-coherent types remain unsupported. |
...
| `vkAllocateCommandBuffers` / `vkFreeCommandBuffers` | Implemented for multiple command buffers per call. Allocation initializes every output to null, unwinds all partial allocations on failure, and frees arrays after validating every non-null handle and owner. |
...
| `vkCmdCopyBuffer` | Implements multiple disjoint `VkBufferCopy` regions per call and multiple ordered buffer-copy commands per submission on the coherent buffer heap. Recording validates an entire region array before appending it, captures both handles and offsets, and rejects overlapping source/destination or destination/destination regions within one call. Submission preflights every copy's live same-device bindings, usage, bounds and source/destination overlap before any write, retains both buffers and allocations, then copies the requested bytes in recording order. A mix with a draw, clear or image copy is refused before work begins; the pool bounds the number of recorded regions. |
| `vkCmdFillBuffer` | Implements a four-byte aligned transfer-destination fill with a repeated host-endian `uint32_t` and `VK_WHOLE_SIZE` rounding down to whole words. It records a generation-tagged buffer handle and value, revalidates the entire mixed copy/fill stream before any write, retains buffer and allocation during submission, and executes fills and copies in recording order. Draws, image clears and image copies remain separate submissions. |
| `vkCreateEvent` / `vkDestroyEvent` / `vkGetEventStatus` / `vkSetEvent` / `vkResetEvent` | Implemented for host-owned events on a graphics-capable device. New events start reset; repeated set/reset is idempotent. Typed generations and parent checks reject stale and wrong-device handles. GPU event commands remain unavailable. |
...
| `vkDeviceWaitIdle` / `vkQueueWaitIdle` | Implemented. Anvil exposes one queue per device; queue wait validates the generation-tagged queue handle and uses the existing bounded device-idle polling path. |
...
| `vkCreateGraphicsPipelines` / `vkDestroyPipeline` | Accepts multiple independent `VkGraphicsPipelineCreateInfo` entries per call, without a pipeline cache. It attempts every entry, leaves failed outputs null, and preserves successful handles when another entry fails. See the state table below for the supported pipeline state. |
...
|---|---|---|
| Registry and ABI | A PureBasic generator verifies the pinned registry hash and emits a selected core-1.0 slice. The checked-in vocabulary covers 79 naturally aligned AArch64 structures with registry member names, order and widths. The complete 55-member `VkPhysicalDeviceFeatures` record, `VkFormatProperties`, `VkImageFormatProperties`, `VkSamplerCreateInfo`, `VkSemaphoreCreateInfo`, `VkBufferCopy` and `VkPipelineDynamicStateCreateInfo`, and the complete nested `VkPhysicalDeviceProperties` / `VkPhysicalDeviceLimits` / `VkPhysicalDeviceSparseProperties` ABI are among them. The properties query behavior and its backend limits remain to be implemented. The foundation gate compares its registry-owned structures and executes exact size and `OffsetOf` fixtures; the independent complete ABI inventory covers all 79 structures and reports 123 of 281 core declaration categories present: 32 enums, 12 bitmasks and 79 structures. Another 156 categories remain missing and the two core unions remain deliberately unrepresentable. | The complete core-1.0 enum, bitmask, alias, handle, union, callback, command prototype, platform guard, optionality and valid-usage vocabulary is not generated. `VkClearColorValue` is a union and is deliberately not declared — PureMetal has no union, so the adapter reads the caller's sixteen bytes as the `float32` arm and says so. No other target ABI is declared equivalent merely because AArch64 layout passes. |
| Public API | The entry points in the table above validate `sType`, `pNext`, flags, counts, pointers, parents and host-synchronization rules before entering the object engine. Invalid usage that core Vulkan leaves undefined returns an Anvil code far outside `VkResult`'s range, with a whole sentence naming the code and the next thing to check; void commands record that sentence and, where the specification allows, invalidate the command buffer. Byte-exact, case-sensitive `vkGetInstanceProcAddr` and `vkGetDeviceProcAddr` lookup exposes 94 audited core command names in their registry dispatch domains. The independent inventory finds 95 declared-callable core commands and 42 missing across all 137 core-1.0 commands; `vkFreeDescriptorSets` is the one intentionally withheld from lookup because every descriptor pool refuses the enabling flag. | There is no platform loader integration, layer interface, extension negotiation or `VkAllocationCallbacks` support. Host allocation callbacks are refused rather than ignored. `AnvilVkImageAddress` remains a diagnostic oracle, not an application API; applications can now use `vkMapMemory`. Concurrent host calls are still unsafe — see the prerequisites. |
| Instance and device discovery | Typed generation handles reject stale handles and wrong owners across the implemented object families, including binary semaphores. All four core layer/extension enumeration calls truthfully report empty lists; named layers return `VK_ERROR_LAYER_NOT_PRESENT`, and an invalid physical-device handle leaves the count untouched. One instance, physical device, device and queue exist over whichever backend is linked. The queue derives graphics support from the backend's draw capability; compute remains absent. The format query derives linear BGRA8 colour-attachment support from that capability. The image-format query shares creation's format/type/tiling/usage/flags owner and derives extent and maximum resource bytes from backend geometry and pitch. The sparse-image format query reports zero properties because no sparse residency is supported. | General physical-device properties and limits remain absent. A production Pi build enumerates a device only when the graphics engine is up and an offscreen window is declared. UNO Q links `vk_backend_none.pbi` and enumerates nothing. |
| Command lifecycle | Pools and primary/secondary buffers across initial, recording, executable, pending, invalid, reset, free and destroy. Resources a submission references are retained until it completes, and reset, free, pool reset and pool destroy all refuse while a buffer is pending. Exactly one submission may be outstanding. A command buffer is either a transfer or a render pass and never both; a render pass holds a bounded ordered list of up to 4,096 draws. Each nonempty draw copies the current static or dynamic scissor into its immutable record; reset clears dynamic state and a draw before `vkCmdSetScissor` is refused. A render pass left open at `vkEndCommandBuffer` is refused. Whole-list preflight completes before mutation, and every abandon path discards the prepared generation ledger. | Secondary execution and inheritance, simultaneous use, externally synchronized host access, and the rest of the command set are absent. |
...
| `tools/vulkan_dispatch_check.py` / `tools/vulkan_dispatch_inventory_check.py` | PASS — 530 registry/source/emitted checks, 134 emitted rows and 3,596,637 interpreted A64 instructions. The complete inventory validates all 137 core-1.0 commands: 95 declared-callable and 42 missing, with 94 names exposed through the resolvers. | Exact registry prototypes, aliases and command domains; case-sensitive bounded lookup; null-instance/global, instance and device query rules; zero-count layer/extension, sparse-image format and sparse-image memory requirement enumeration plus linear-image row-pitch reporting and coherent mapped-range validation with caller-output safety; queue-idle lookup and live/stale handle behavior; public `vkCmdCopyBuffer`, `vkCmdFillBuffer`, `vkCmdSetScissor`, `vkCmdBindIndexBuffer` and `vkCmdDrawIndexed`; and the deliberate `vkFreeDescriptorSets` withholding boundary. |
| `tools/vulkan_resource_check.py` | PASS — 644 independent property checks over 5,263,103 executed A64 instructions. The earlier complete mutation run rejects all 30 prior mistakes; the two focused `UINT64_MAX` mutants, all ten focused map-memory mutants and the focused transfer-only attachment mutant are RED. The suite now has 44 mutants after the obsolete semaphore-refusal mutant was removed; a fresh complete run of all 44 has not been claimed. | The resource, layout, fence, retention, mapping, format-capability, submission and error-reporting contracts, executed through the public entry points, with an MMIO hard stop armed; and that every byte of the bound image still held its poison, so no processor-side image clear exists anywhere in the path; buffer fills explicitly write the coherent buffer heap. The buffer-copy section requires exact copied and untouched bytes for one or several regions, rejects within-call source/destination and destination/destination overlap, proves separate copies execute in order, proves a stale later transfer prevents an earlier write, and rejects a destroyed destination handle. It checks exact fill words and untouched bytes, `VK_WHOLE_SIZE` including the final partial-word rounding case, alignment and size refusal, and ordered copy/fill execution. Its mapping properties use the public entry points to distinguish host-visible/non-visible types, owner, live/stale handle, duplicate/unbalanced state, flags, exact/whole ranges and returned pointer, then use the mapping to poison the image. Its format property refuses colour-attachment image creation when the backend advertises no graphics draw path while the shared combination owner still accepts transfer-only and sampled-only usage. Its wait properties distinguish an unsignalled unlimited wait (explicitly unsupported) from an already-satisfied unlimited wait (success). |
| `tools/vulkan_v3d_backend_check.py` | PASS — 84 property checks over 3,092,494 executed A64 instructions; all five production board diagnostics, including `vulkanIndexedTriangleProof.pi4`, build real images. The recorded mutation run rejects all 16 desk-reachable mistakes and names 2 board-only rules it cannot reach. | That the whole closure links against the real display, V3D, QPU and Neon implementation; that a build with the engine down enumerates no device without MMIO; that geometry is planned before mutation, every arena/range/in-frame boundary is checked, and a failed rebind preserves the prior surface. It also requires full geometry restoration and the descriptor TMU mapped-range/cache-clean contracts. The source contract allows bounded CPU writes only in the coherent buffer transfer seams; image clear and draw still have no CPU or DMA fallback. It requires the pipeline/vertex/index/descriptor cleans before submission and the GPU clean plus target clean/invalidate before completion, giving the `HOST_COHERENT` advertisement teeth. |
