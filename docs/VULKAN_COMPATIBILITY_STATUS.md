...
|---|---|---|
| `Anvil/Graphics/Vulkan/Tests/vulkan_foundation_check.py` | PASS — pinned registry compared, 135 emitted ABI and lifecycle checks over 72,696 interpreted A64 instructions, production boundary in 40,368. The focused queue-capability mutant is RED. | Vocabulary against the pinned `vk.xml`, exact AArch64 layout, and that a build with no backend enumerates no device. The emitted properties prove an ordinary graphics-family scan selects family zero for a draw-capable backend and finds no graphics family for a transfer-only backend. `--mutate-queue` removes the capability-owned graphics advertisement and requires those properties to fail. The gate regenerates the twenty-four graphics-pipeline structures and, since the descriptor path landed, the nine descriptor ones and twenty more constants: every member name, order and type is compared against the registry AND against vk_core_1_0.pbi, so the two transcriptions have to agree. |
| `tools/vulkan_resource_check.py` | PASS — 394 independent property checks over 797,514 executed A64 instructions; `--mutate` rejects all 30 plausible mistakes | The resource, layout, fence, retention, submission and error-reporting contracts, executed through the public entry points, with an MMIO hard stop armed; and that every byte of the bound image still held its poison, so no processor-side clear exists anywhere in the path |
