...
lowering passed their emitted desk gates; the indexed board diagnostic builds,
but its silicon run is still owed.
...
| `tools/vulkan_dispatch_check.py` / `tools/vulkan_dispatch_inventory_check.py` | PASS — 388 registry/source/emitted checks, 64 emitted rows and 2,984,553 interpreted A64 instructions. The complete inventory validates all 137 core-1.0 commands: 77 declared-callable and 60 missing, with 76 names exposed through the resolvers. | Exact registry prototypes, aliases and command domains; case-sensitive bounded lookup; null-instance/global, instance and device query rules; public `vkCmdSetScissor`, `vkCmdBindIndexBuffer` and `vkCmdDrawIndexed`; and the deliberate `vkFreeDescriptorSets` withholding boundary. |
...
| `tools/vulkan_indexed_draw_check.py` | PASS — 32 public-path cells, 12 encoder results and 46 exact packet/refusal bytes over 4,213,868 emitted A64 instructions; `--mutate` rejects all 6 focused mistakes. | UINT16/UINT32 binding and recording, `firstIndex` applied once, maximum-index vertex bounds, stale/range/alignment refusals before counters move, in-flight index lifetime, and byte-exact V3D 4.2 `INDEX_BUFFER_SETUP` plus `INDEXED_PRIM_LIST` emission. The matching board diagnostic builds; no indexed silicon run is claimed. |
