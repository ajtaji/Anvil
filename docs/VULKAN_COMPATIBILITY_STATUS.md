...
| `vkCmdCopyBuffer` | Implements one `VkBufferCopy` region per submission on the coherent buffer heap. Recording captures both handles and offsets; submission revalidates live same-device bindings, usage and bounds, rejects overlapping source/destination memory, retains both buffers and allocations, and copies exactly the requested bytes. More than one buffer copy or a mix with a draw, clear or image copy is refused before work begins. |
