...
| `tools/vulkan_resource_check.py` | PASS — 401 independent property checks over 800,192 executed A64 instructions. The earlier complete mutation run rejects all 30 prior mistakes; `--mutate-only fence-unlimited` rejects both new `UINT64_MAX` mutants. The suite now has 32 mutants; a fresh complete run of all 32 has not been claimed. | The resource, layout, fence, retention, submission and error-reporting contracts, executed through the public entry points, with an MMIO hard stop armed; and that every byte of the bound image still held its poison, so no processor-side clear exists anywhere in the path. Its wait properties distinguish an unsignalled unlimited wait (explicitly unsupported) from an already-satisfied unlimited wait (success). |
