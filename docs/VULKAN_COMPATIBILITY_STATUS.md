...
| `vkCreateSampler` / `vkDestroySampler` | Implemented as a typed, device-owned object for normalized 2D sampling with nearest or linear min/mag filters, nearest mip mode at LOD zero, clamp-to-edge on all coordinates, and no anisotropy or comparison. It can now be retained by the bounded combined-image-sampler descriptor; shader texture fetches are not connected yet. |
