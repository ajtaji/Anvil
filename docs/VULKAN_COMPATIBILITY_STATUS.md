...
The prerequisite for honest non-native image extents is specified in
`docs/VULKAN_DYNAMIC_RENDER_GEOMETRY_DESIGN.md`. It is design-only; no
image-format maximum is inferred from it.
...
| Instance and device discovery | Typed generation handles reject stale handles and wrong owners across all nine object types. One instance, physical device, device and queue, over whichever backend is linked. The queue always reports transfer and derives graphics support from the backend's draw capability; compute remains absent. The format query derives linear BGRA8 colour-attachment support from the same capability and zeros every unsupported field. | General physical-device properties and limits, image-format properties, sparse-image format properties and extension-property enumeration remain absent. A production Pi build still enumerates a device only when the graphics engine is up and an offscreen window has been declared. UNO Q links `vk_backend_none.pbi` and enumerates nothing. |
...
|---|---|---|
| `Anvil/Graphics/Vulkan/Tests/vulkan_foundation_check.py` | PASS — pinned registry compared, 154 emitted ABI and lifecycle checks over 135,974 interpreted A64 instructions, production boundary in 40,578. The queue mutant, all three feature mutants and all seven format-query mutants are RED. | Vocabulary against the pinned `vk.xml`, exact AArch64 layout, and that a build with no backend enumerates no device. The feature properties poison then query the complete record, accept a non-null all-false record, and turn on each of its 55 members separately; `--mutate-features` rejects false query output, refusal of an all-false pointer, and an incomplete member scan. The format properties poison all three output fields and require exactly linear BGRA8 `COLOR_ATTACHMENT` on a draw-capable backend, then zero for unsupported formats and a transfer-only backend; its mutants catch invented sampled, storage, depth/stencil and optimal support as well as lost format/capability gating and stale-handle validation. The queue properties prove an ordinary graphics-family scan selects family zero for a draw-capable backend and finds no graphics family for a transfer-only backend. Every checked structure member name, order and type is compared against both the registry and `vk_core_1_0.pbi`, so the two transcriptions have to agree. |
| `tools/vulkan_resource_check.py` | PASS — 484 independent property checks over 825,022 executed A64 instructions. The earlier complete mutation run rejects all 30 prior mistakes; the two focused `UINT64_MAX` mutants, all ten focused map-memory mutants and the focused transfer-only attachment mutant are RED. The suite now has 43 mutants; a fresh complete run of all 43 has not been claimed. | The resource, layout, fence, retention, mapping, format-capability, submission and error-reporting contracts, executed through the public entry points, with an MMIO hard stop armed; and that every byte of the bound image still held its poison, so no processor-side clear exists anywhere in the path. Its mapping properties use the public entry points to distinguish host-visible/non-visible types, owner, live/stale handle, duplicate/unbalanced state, flags, exact/whole ranges and returned pointer, then use the mapping to poison the image. Its format property refuses colour-attachment image creation when the backend advertises no graphics draw path. Its wait properties distinguish an unsignalled unlimited wait (explicitly unsupported) from an already-satisfied unlimited wait (success). |
| `tools/vulkan_v3d_backend_check.py` | PASS — 51 property checks over 99,760 executed A64 instructions; `--mutate` rejects all 8 desk-reachable mistakes and names 2 board-only rules it cannot reach | That the whole closure links against the real display, V3D, QPU and Neon implementation, that a build with the engine down enumerates no device, that reaching that answer makes not one MMIO access, and that the descriptor TMU path retains its mapped-range and cache-clean contracts. The source contract requires the pipeline/vertex/descriptor cleans before submission and the GPU clean plus target clean/invalidate before completion; its eighth mutant removes vertex maintenance and goes RED, giving the `HOST_COHERENT` advertisement teeth. |
