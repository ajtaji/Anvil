...
; Registered only by the immutable loader, which is the one caller that can
; serve this prompt with storage not yet mounted. The default is a real
; procedure, never a null pointer: an unset handler that is called anyway
; branches to whatever the variable happens to hold.
Procedure.i Pi3NoMountOwner()
  ProcedureReturn 0
EndProcedure
Global pi3_ut_mount_owner.i=@Pi3NoMountOwner
Global pi3_ut_mount_registered.i
Procedure.i Pi3UpdateRegisterMount(handler.i)
  If handler=0 Or pi3_ut_mount_registered<>0 : ProcedureReturn 0 : EndIf
  pi3_ut_mount_owner=handler
  pi3_ut_mount_registered=1
  ProcedureReturn 1
EndProcedure
...
Procedure pi3ut_Command()
  ...
  If pi3ut_Match("help") <> 0 And pi3ut_End() <> 0
    pi3ut_WriteLine("update begin <length> <sha256> | status | commit | abort; mount; reset")
    ProcedureReturn
  EndIf
  pi3_ut_parse = 0
  ; Recovery is entered before storage, so a card that cannot be read can no
  ; longer keep an operator out. Mounting is then an explicit command whose
  ; cost and failure are visible, instead of an unavoidable boot step that can
  ; only be observed as silence.
  If pi3ut_Match("mount") <> 0 And pi3ut_End() <> 0
    If pi3_ut_mount_registered = 0
      pi3ut_WriteLine("!! mount is not available here; this prompt is served by a running monitor that mounted at boot")
      ProcedureReturn
    EndIf
    pi3ut_WriteLine("mounting the A/B volume; bounded cold-storage progress services the early watchdog while this work runs")
    If pi3_ut_mount_owner() = 0
      pi3ut_WriteText("!! mount refused, error ") : pi3ut_WriteInt(Pi3UpdateError())
      pi3ut_WriteLine("; the selected boot slot did not change and no byte was written")
      ProcedureReturn
    EndIf
    pi3ut_WriteLine("mounted; update and status commands are now available")
    ProcedureReturn
  EndIf
  pi3_ut_parse = 0
